...
function setLoading(v) {
  ...
  btn.disabled = v;
  if (!v) btn.textContent = authMode === 'sms' ? (otpPhone ? 'Verificar código' : 'Enviar código') : 'Iniciar sesión';
...
function redirectToApp(token, refreshToken, role, name) {
  const params = new URLSearchParams({ token, refresh: refreshToken || '', role, name: name || '' });
...
async function handleSMS() {
  ...
  if (!raw || raw.replace(/\D/g, '').length < 7) { showError('Ingresa un número válido'); return; }
  ...
  try {
    ...
    if (data.userNotFound) {
      ...
      setTimeout(() => { window.location.href = `${APP_URL}/?signup=1&phone=${encodeURIComponent(phone)}`; }, 1500);
      return;
    }
...
async function handleOTP() {
  ...
  try {
    ...
    redirectToApp(data.token, data.refreshToken, data.user.role, data.user.name);
  } catch (e) {
    showError('Error de conexión. Inténtalo de nuevo.');
  } finally { setLoading(false); }
}
...
async function handlePassword() {
  ...
  try {
    ...
    if (!data.success || !data.token) { showError(data.error || 'Credenciales incorrectas'); return; }
    redirectToApp(data.token, data.refreshToken, data.user.role, data.user.name);
  } catch (e) {
    showError('Error de conexión. Inténtalo de nuevo.');
  } finally { setLoading(false); }
