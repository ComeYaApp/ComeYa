...
export class BusinessAnalyticsService {
  ...
  static async getDashboard(
    ...
    const allOrders = startDate
      ? await db.select().from(orders).where(eq(orders.businessId, businessId))
      : periodOrders;
    ...
    const getBusinessRevenue = (o: any) =>
      o.businessEarnings ?? o.subtotal ?? 0;
    const totalRevenue = completedOrders.reduce(
      (sum, o) => sum + getBusinessRevenue(o),
      0,
    );
    ...
    return {
      ...
      dashboard: {
        ...
        todayRevenue: todayOrders
          .filter((o) => o.status === "delivered")
          .reduce((s, o) => s + getBusinessRevenue(o), 0),
    ...
    for (const order of recentOrders) {
      try {
        ...
        for (const item of items) {
          const productId =
            item.productId || item.product?.id || item.id || "unknown";
  ...
  static async getReviewStats(businessId: string) {
    ...
    const avgRating =
      businessReviews.reduce((sum, r) => sum + r.rating, 0) / totalReviews;

    const foodRatings = businessReviews.filter((r) => r.foodRating);
    const avgFoodRating =
      foodRatings.length > 0
        ? foodRatings.reduce((sum, r) => sum + (r.foodRating || 0), 0) /
          foodRatings.length
        : 0;
  ...
  static async getWeeklyComparison(businessId: string) {
    ...
    const thisWeekRevenue = thisWeekOrders.reduce(
      (sum, o) => sum + (o.businessEarnings || 0),
      0,
    );
