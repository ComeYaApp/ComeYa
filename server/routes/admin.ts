...
router.post("/businesses/geocode", authenticateToken, requireRole("admin", "super_admin"), async (req, res) => {
  try {
    const { db } = await import("../db");
    ...
    for (const biz of businesses) {
      try {
        ...
        if (geoData.status === "OK" && geoData.results[0]) {
          ...
          await db.execute(sql`UPDATE businesses SET latitude = ${String(lat)}, longitude = ${String(lng)} WHERE id = ${biz.id}`);
          updated++;
        } else {
          failed++;
        }
...
router.put("/businesses/:id", authenticateToken, requireRole("admin", "super_admin"), async (req, res) => {
  try {
    const { businesses } = await import("@shared/schema-mysql");
    const { db } = await import("../db");
    const allowed = ["name", "description", "isActive", "isOpen", "latitude", "longitude", "address", "phone"];
    const updates: any = { updatedAt: new Date() };
