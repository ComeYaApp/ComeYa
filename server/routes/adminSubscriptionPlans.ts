...
import { db } from '../db';
import { subscriptions, subscriptionPlans, subscriptionBenefits, users } from '@shared/schema-mysql';
...
router.get('/premium-subscribers', authenticateToken, requireRole('admin', 'super_admin'), async (req, res) => {
  try {
    const now = new Date();
    ...
    const totalActive = result.filter(r => r.status === 'active' && !r.isExpired).length;
    const totalExpired = result.filter(r => r.isExpired || r.status === 'expired').length;
