...
// GET /api/admin/subscription-plans — todos los planes con sus beneficios
router.get('/subscription-plans', authenticateToken, requireRole('admin', 'super_admin'), async (req, res) => {
  try {
    ...
    const result = plans.map(plan => ({
      ...plan,
      benefits: benefits.filter(b => b.plan === plan.planKey && b.isActive),
    }));
...
// PUT /api/admin/subscription-plans/:planKey — editar precio y nombre de un plan
...
// POST /api/admin/subscription-benefits — añadir beneficio a un plan
router.post('/subscription-benefits', authenticateToken, requireRole('admin', 'super_admin'), async (req, res) => {
  try {
    ...
    if (!plan || !benefitType || !benefitValue) {
      return res.status(400).json({ error: 'plan, benefitType y benefitValue son requeridos' });
    }

    const id = uuidv4();
    await db.insert(subscriptionBenefits).values({ id, plan, benefitType, benefitValue, description, isActive: true });
...
// PUT /api/admin/subscription-benefits/:id — editar beneficio
router.put('/subscription-benefits/:id', authenticateToken, requireRole('admin', 'super_admin'), async (req, res) => {
  try {
    const { benefitType, benefitValue, description, isActive } = req.body;

    await db.update(subscriptionBenefits)
      .set({ benefitType, benefitValue, description, isActive })
...
// DELETE /api/admin/subscription-benefits/:id — eliminar beneficio
