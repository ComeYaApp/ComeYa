...
const DEFAULT_CATEGORIES = [
  {
    ...
    icon: "coffee",
    color: "#F97316",
    description: "Comida preparada, menús y platos del día",
    displayOrder: 1,
  },
  {
    name: "Mercados",
  ...
  },
  {
    ...
    icon: "grid",
    color: "#6B7280",
    description: "Negocios que no encajan en otras categorías",
    displayOrder: 99,
  },
];
  ...
  async (req, res) => {
    try {
      ...
      const [created] = await db
        ...
        .from(businessCategories)
        .where(
          (await import("drizzle-orm")).eq(businessCategories.slug, cleanSlug),
        )
        .limit(1);
  ...
  async (req, res) => {
    try {
      ...
      await db
        ...
        .set(updates)
        .where(eq(businessCategories.id, req.params.id));
