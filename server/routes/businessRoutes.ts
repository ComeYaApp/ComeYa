  ...
  async (req, res) => {
    try {
      ...
      if (requestedBusinessId) {
        targetBusiness = ownerBusinesses.find(
          (b) => b.id === requestedBusinessId,
        );
        if (!targetBusiness) {
          return res
            .status(403)
            .json({ error: "No tienes acceso a este negocio" });
        }
      } else {
      ...
      const deliveredOrders = businessOrders.filter(
...
router.get(
  ...
  async (req, res) => {
    try {
      const { businesses, orders, users, addresses } = await import(
        "@shared/schema-mysql"
      );
      const { db } = await import("../db");
      ...
      if (ownerBusinesses.length === 0) {
        return res
          .status(404)
          .json({ error: "No businesses found for this user" });
      }
      ...
      const enrichedOrders = await Promise.all(
        filteredOrders.map(async (order) => {
          ...
          const business = ownerBusinesses.find(
  ...
  async (req, res) => {
    try {
      ...
      if (!ownerBusinessIds.includes(order.businessId)) {
        return res
          .status(403)
          .json({ error: "No tienes acceso a este pedido" });
      }
  ...
  async (req, res) => {
    try {
      ...
      if (!ownerBusinessIds.includes(product.businessId)) {
        return res
          .status(403)
          .json({ error: "No tienes acceso a este producto" });
      }
