...
export function NativeMap({ activeOrders, onlineDrivers }: MapProps) {
  ...
  useEffect(() => {
    ...
    onlineDrivers.forEach(driver => {
      ...
      const pos = { lat: parseFloat(driver.currentLatitude), lng: parseFloat(driver.currentLongitude) };
    ...
    activeOrders.forEach(order => {
      ...
      if (orderMarkersRef.current.has(order.id)) {
        ...
        const marker = new google.maps.Marker({
          ...
          icon: { url: ORDER_SVG(color), scaledSize: new google.maps.Size(32, 32), anchor: new google.maps.Point(16, 16) },
          zIndex: 100,
        });
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.card }]}>
      <div ref={mapRef} style={{ width: "100%", height: "100%", borderRadius: BorderRadius.lg } as any} />
      ...
      <View style={[styles.legend, { backgroundColor: theme.card }]}>
        <View style={styles.legendItem}>
          ...
          <ThemedText type="caption" style={{ color: theme.textSecondary }}>{onlineDrivers.filter(d => d.currentLatitude).length} repartidores</ThemedText>
        </View>
        <View style={styles.legendItem}>
          <View style={[styles.legendDot, { backgroundColor: "#F59E0B" }]} />
          <ThemedText type="caption" style={{ color: theme.textSecondary }}>{activeOrders.length} pedidos</ThemedText>
        </View>
      </View>
    </View>
  );
