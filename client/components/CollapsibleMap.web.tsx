...
export function CollapsibleMap({
  ...
  useEffect(() => {
    ...
    if (locations.length > 1) {
      ...
      gmap.current.fitBounds(bounds, { top: 60, right: 60, bottom: 60, left: 60 });
    }
  }, [mapsReady, isDark, isPickup]);

  // Actualizar marcador del negocio
  useEffect(() => {
    if (!mapsReady || !gmap.current || !isValidLocation(businessLocation)) return;
    const google = (window as any).google;
  ...
  useEffect(() => {
    if (!mapsReady || !gmap.current || !isValidLocation(customerLocation)) return;
    const google = (window as any).google;
  ...
  useEffect(() => {
    ...
    if (routeCoords.length >= 2) {
      polylineRef.current = new google.maps.Polyline({
        path: routeCoords.map(loc => ({ lat: loc.latitude, lng: loc.longitude })),
        geodesic: true,
  ...
  }, [mapsReady, businessLocation, deliveryPersonLocation, customerLocation, isPickup]);
  ...
  const hasAnyLocation = isValidLocation(businessLocation) || 
        ...
        {(!mapsReady || !hasAnyLocation) && (
          <View style={[styles.mapFallback, { backgroundColor: theme.backgroundSecondary }]}>
            ...
            <ThemedText type="caption" style={{ color: theme.textSecondary, marginTop: Spacing.sm, textAlign: "center" }}>
              {!mapsReady ? "Cargando mapa..." : "Esperando ubicación del pedido..."}
            </ThemedText>
          </View>
        )}
