...
const CATEGORIES: CategoryDef[] = [
  {
    ...
    color: "#10B981",
    description:
      "Control de márgenes y distribución de ingresos entre la plataforma, negocios y repartidores",
    settings: [
      {
        key: "comeya_commission",
        label: "Comisión ComeYa",
        hint: "Markup % sobre el precio base de los productos. El cliente paga este extra.",
        placeholder: "15",
        type: "number",
        icon: "trending-up",
        color: "#10B981",
      },
      {
        ...
        label: "Comisión Negocio",
        hint: "% del precio base que recibe el negocio por cada pedido entregado.",
        placeholder: "100",
        type: "number",
        icon: "briefcase",
        color: "#3B82F6",
      },
      {
        ...
        hint: "% de la tarifa de entrega que recibe el repartidor por cada entrega completada.",
        placeholder: "100",
        type: "number",
        icon: "truck",
        color: "#8B5CF6",
    ...
    settings: [
      {
        ...
        label: "Período de arrepentimiento",
        hint: "Segundos que tiene el cliente para cancelar gratis tras confirmar el pedido.",
        placeholder: "60",
        type: "number",
        icon: "clock",
        color: "#F59E0B",
      },
      {
        ...
        label: "Retraso llamada negocio",
        hint: "Minutos de espera antes de llamar automáticamente al negocio si no acepta el pedido.",
        placeholder: "3",
        type: "number",
        icon: "phone",
        color: "#F97316",
      ...
      {
        key: "fund_hold_hours",
        label: "Retención de fondos",
        hint: "Horas que se retienen los fondos antes de liberarlos al confirmar la entrega.",
        placeholder: "1",
        type: "number",
        icon: "lock",
        color: "#EF4444",
    ...
    settings: [
      {
        ...
        hint: "Número de teléfono donde los clientes envían pagos por Bizum.",
      ...
      },
      {
        key: "paypal_email",
        label: "Email PayPal",
        hint: "Email de la cuenta PayPal de ComeYa para recibir pagos.",
  ...
  },
  {
    ...
    color: "#635BFF",
    description:
      "Configuración del procesador de pagos con tarjeta. Las API keys van en variables de entorno de Render.",
    settings: [
      {
        key: "stripe_enabled",
        label: "Stripe habilitado",
        hint: "Activa o desactiva los pagos con tarjeta y Bizum via Stripe en el checkout.",
        placeholder: "true",
        type: "boolean",
        icon: "toggle-right",
        color: "#635BFF",
...
export const SettingsTab: React.FC<Props> = () => {
  ...
  const validate = (def: SettingDef, val: string): string | null => {
    if (def.type === "number") {
      ...
      if (def.key.includes("commission") && n > 100)
        return "No puede superar 100%";
    }
    if (def.type === "euro" && isNaN(parseFloat(val)))
  ...
  const save = async (def: SettingDef) => {
    ...
    try {
      ...
      if (data.success) flashSaved(def.key);
        ...
        setErrors((p) => ({
          ...p,
          [def.key]: data.error ?? "Error al guardar",
        }));
    } catch {
      setErrors((p) => ({ ...p, [def.key]: "Error de conexión" }));
    } finally {
      setSaving(null);
    }
  };
  ...
  return (
    ...
    >
      ...
      <View style={ph.row}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={[ph.title, { color: text }]}>
            Configuración del sistema
          </Text>
          <Text style={[ph.sub, { color: sub }]}>
            Parámetros globales de la plataforma · cambios en tiempo real
          </Text>
        </View>
        {!hasSettings && (
          <TouchableOpacity
            ...
            style={[ph.initBtn, { backgroundColor: PRIMARY }]}
          >
            {initializing ? (
              ...
              <>
                ...
                <Text style={ph.initTxt}>Inicializar</Text>
              </>
            )}
          </TouchableOpacity>
        )}
      ...
      >
        ...
        <Text style={[ph.infoTxt, { color: "#3B82F6" }]}>
          Las API keys (Stripe, Gemini, Twilio, Cloudinary, etc.) se configuran
          en las variables de entorno de Render — no aquí. Solo se gestionan
      ...
      {CATEGORIES.map((cat) => (
        <View
          key={cat.id}
          style={[sec.card, { backgroundColor: card, borderColor: border }]}
        >
          {/* Category header */}
          ...
          {cat.settings.map((def, i) => {
            ...
            return (
              ...
              >
                {/* Icon + label */}
                <View
                  style={[si.iconWrap, { backgroundColor: def.color + "12" }]}
                >
                ...
                {isBool ? (
                  <View style={si.boolWrap}>
                    <TouchableOpacity
                      onPress={() => {
                        ...
                        setTimeout(async () => {
                          const newVal =
                            values[def.key] === "true" ? "false" : "true";
                          try {
                            const res = await apiRequest(
                              "PUT",
                              "/api/admin/settings",
                              { key: def.key, value: newVal },
                            );
                            const data = await res.json();
                            if (data.success) flashSaved(def.key);
                          } catch {}
                    ...
                    >
                      <View
                        style={[
                          si.toggleThumb,
                          { transform: [{ translateX: boolOn ? 18 : 2 }] },
                        ]}
                      />
                    </TouchableOpacity>
                    <Text
                  ...
                  <View style={si.inputWrap}>
                    ...
                    >
                      {def.type === "euro" && (
                        <Text style={[si.prefix, { color: sub }]}>€</Text>
                      )}
                      {def.type === "number" && (
                        <Text style={[si.prefix, { color: sub }]}>
                          {def.key.includes("commission")
                            ? "%"
                            : def.key.includes("seconds")
                              ? "s"
                              : def.key.includes("minutes")
                                ? "m"
                                : "h"}
                        </Text>
                      )}
