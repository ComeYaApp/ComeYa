  ...
  if (selected) {
    return (
      <ScrollView style={{ flex: 1, backgroundColor: theme.backgroundRoot }} contentContainerStyle={{ padding: 16, paddingBottom: 100 }}>
        <TouchableOpacity style={s.backBtn} onPress={() => { setSelected(null); setRejectReason(""); }}>
        ...
        {selected.proof_image_url ? (
          ...
          <View style={[s.noImage, { backgroundColor: theme.backgroundSecondary }]}>
            <Feather name="image" size={40} color={theme.textSecondary} />
            <Text style={{ color: theme.textSecondary, marginTop: 8 }}>Sin imagen</Text>
          </View>
        )}

        {/* Datos */}
        <View style={[s.card, { backgroundColor: theme.card }]}>
          <Text style={[s.cardTitle, { color: theme.text }]}>Datos del comprobante</Text>
          <DataRow label="Cliente"       value={selected.user_name ?? "—"}    theme={theme} />
          <DataRow label="Teléfono"      value={selected.user_phone ?? "—"}   theme={theme} />
          <DataRow label="Negocio"       value={selected.business_name ?? "—"} theme={theme} />
          <DataRow label="Método"        value={METHOD_LABELS[selected.payment_provider] ?? selected.payment_provider} theme={theme} />
          <DataRow label="Referencia"    value={selected.reference_number ?? "—"} theme={theme} bold />
          <DataRow label="Importe env."  value={fmt(selected.amount)}         theme={theme} highlight />
          <DataRow label="Total pedido"  value={fmt(selected.order_total)}    theme={theme} />
          {selected.verification_notes && (
            <DataRow label="Nota" value={selected.verification_notes} theme={theme} />
          )}
          <DataRow label="Enviado" value={new Date(selected.submitted_at).toLocaleString("es-ES")} theme={theme} />
        </View>

        {/* Razón de rechazo */}
        <View style={[s.card, { backgroundColor: theme.card }]}>
          <Text style={[s.label, { color: theme.textSecondary }]}>Razón de rechazo (si rechazas):</Text>
          <TextInput
        ...
        <View style={{ flexDirection: "row", gap: 10 }}>
          ...
          >
            {processing ? <ActivityIndicator size="small" color="#FFF" /> : (
              <><Feather name="check" size={18} color="#FFF" /><Text style={s.btnText}>Aprobar</Text></>
            )}
          </TouchableOpacity>
          <TouchableOpacity
    ...
    >
      {proofs.length === 0 ? (
        <View style={[s.empty, { backgroundColor: theme.card }]}>
          ...
          <Text style={[s.emptyTitle, { color: theme.text }]}>Sin comprobantes pendientes</Text>
        ...
        proofs.map((proof: any) => (
          ...
          >
            <View style={s.row}>
              <View style={[s.badge, { backgroundColor: ComeYaColors.warning + "20" }]}>
                <Text style={[s.badgeText, { color: ComeYaColors.warning }]}>
                  {METHOD_LABELS[proof.payment_provider] ?? proof.payment_provider}
                </Text>
              </View>
              <Text style={[s.amount, { color: ComeYaColors.success }]}>{fmt(proof.amount)}</Text>
            </View>
            ...
            <View style={s.tapHint}>
              ...
              <Text style={[s.tapHintText, { color: ComeYaColors.primary }]}>Ver comprobante y verificar</Text>
            </View>
          </TouchableOpacity>
        ))
      )}
...
function DataRow({ label, value, theme, bold, highlight }: any) {
  return (
    <View style={{ flexDirection: "row", justifyContent: "space-between", paddingVertical: 5, borderBottomWidth: 0.5, borderBottomColor: theme.border }}>
      <Text style={{ color: theme.textSecondary, fontSize: 13 }}>{label}</Text>
      <Text style={{ color: highlight ? ComeYaColors.success : theme.text, fontSize: 13, fontWeight: bold || highlight ? "700" : "400", flex: 1, textAlign: "right" }}>
        {value}
      </Text>
    </View>
  );
...
const st = (theme: any) => StyleSheet.create({
  ...
  noImage:      { height: 160, borderRadius: 12, justifyContent: "center", alignItems: "center", marginBottom: 12 },
  label:        { fontSize: 13, marginBottom: 6 },
  input:        { borderWidth: 1, borderRadius: 8, padding: 10, fontSize: 14, minHeight: 44 },
  btn:          { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8, padding: 14, borderRadius: 10, marginTop: 8 },
