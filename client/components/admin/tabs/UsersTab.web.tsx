      ...
      <View style={{ flex: 1 }}>
        ...
        <View style={[kpi.bar, { backgroundColor: card, borderBottomColor: border }]}>
          {[
            ...
            { label: "Clientes",     value: counts.customer ?? 0,            color: "#6B7280"  },
            { label: "Negocios",     value: counts.business_owner ?? 0,      color: "#3B82F6"  },
            { label: "Repartidores", value: counts.delivery_driver ?? 0,     color: "#10B981"  },
        ...
        <View style={[tb.bar, { backgroundColor: card, borderBottomColor: border }]}>
          ...
          <Text style={[tb.count, { color: sub }]}>{filtered.length} usuarios</Text>
        </View>
        ...
        >
          <TouchableOpacity
            ...
            style={[tb.chip, { backgroundColor: roleFilter === "all" ? PRIMARY : inputBg, borderColor: roleFilter === "all" ? PRIMARY : border }]}
          >
            <Text style={[tb.chipTxt, { color: roleFilter === "all" ? "#fff" : text }]}>Todos</Text>
          </TouchableOpacity>
          {Object.entries(ROLE_META).map(([role, meta]) => (
            <TouchableOpacity
              key={role}
              onPress={() => setRoleFilter(role)}
              style={[tb.chip, { backgroundColor: roleFilter === role ? meta.color : inputBg, borderColor: roleFilter === role ? meta.color : border }]}
            >
              <Feather name={meta.icon as any} size={11} color={roleFilter === role ? "#fff" : meta.color} />
              <Text style={[tb.chipTxt, { color: roleFilter === role ? "#fff" : text }]}>{meta.label}</Text>
            </TouchableOpacity>
          ))}
            ...
            {filtered.map(user => {
              ...
              return (
                <TouchableOpacity
                  ...
                  style={[
                    li.card,
                    { backgroundColor: card, borderColor: isSelected ? meta.color : border },
                    isSelected && { borderLeftColor: meta.color, borderLeftWidth: 3 },
                  ]}
                >
                  <View style={li.row}>
                    {/* Avatar */}
                    <View style={[li.avatar, { backgroundColor: meta.color + "20" }]}>
                    ...
                    <View style={{ flex: 1 }}>
                      <View style={li.nameRow}>
                        <Text style={[li.name, { color: text }]} numberOfLines={1}>{user.name}</Text>
                        {!user.isActive && (
                          <View style={[li.inactivePill, { backgroundColor: "#EF444415" }]}>
                            <Text style={li.inactiveTxt}>Inactivo</Text>
                          </View>
                        )}
                      </View>
                      <Text style={[li.phone, { color: sub }]}>{user.phone}</Text>
                      {user.email && <Text style={[li.email, { color: sub }]} numberOfLines={1}>{user.email}</Text>}
                    </View>

                    {/* Role badge */}
                    <View style={[li.rolePill, { backgroundColor: meta.color + "18" }]}>
                      <Feather name={meta.icon as any} size={11} color={meta.color} />
                      <Text style={[li.roleTxt, { color: meta.color }]}>{meta.label}</Text>
                    </View>
                  </View>
                </TouchableOpacity>
              );
...
const li = StyleSheet.create({
  ...
  email:       { fontSize: 11 },
  rolePill:    { flexDirection: "row", alignItems: "center", gap: 4, paddingHorizontal: 8, paddingVertical: 4, borderRadius: 20 },
