...
export const CouponsTab: React.FC<Props> = () => {
  ...
  useEffect(() => {
    const now = new Date();
    let list = coupons;
    if (filter === "active")   list = list.filter(c => c.isActive && (!c.expiresAt || new Date(c.expiresAt) > now));
    if (filter === "inactive") list = list.filter(c => !c.isActive);
    if (filter === "expired")  list = list.filter(c => c.expiresAt && new Date(c.expiresAt) <= now);
    if (search.trim()) {
  ...
  const toggleActive = async (coupon: Coupon) => {
    try {
      const res  = await apiRequest("PUT", `/api/coupons/admin/${coupon.id}`, { isActive: !coupon.isActive });
  ...
  const remove = async (coupon: Coupon) => {
    ...
    } catch { flash(false, "Error al eliminar"); }
    ...
    <View style={{ flex: 1, backgroundColor: bg, flexDirection: "row" }}>
      <View style={{ flex: 1 }}>
        ...
        <View style={[tb.bar, { backgroundColor: card, borderBottomColor: border }]}>
          ...
          <Text style={[tb.count, { color: sub }]}>{filtered.length} cupones</Text>
          <TouchableOpacity
        ...
        {filtered.length === 0 ? (
          <View style={{ flex: 1, justifyContent: "center", alignItems: "center", gap: 12 }}>
            ...
            <TouchableOpacity onPress={() => setEditing("new")} style={[{ flexDirection: "row", alignItems: "center", gap: 6, paddingHorizontal: 16, paddingVertical: 9, borderRadius: 10, backgroundColor: PINK }]}>
              <Feather name="plus" size={14} color="#fff" />
              <Text style={{ color: "#fff", fontWeight: "700", fontSize: 13 }}>Crear primer cupón</Text>
            </TouchableOpacity>
          </View>
        ) : (
          <ScrollView
          ...
          >
            {filtered.map(coupon => {
              ...
              const statusColor = expired ? "#EF4444" : coupon.isActive ? "#10B981" : "#6B7280";
              const statusLabel = expired ? "Expirado" : coupon.isActive ? "Activo" : "Inactivo";
              const usePct      = coupon.maxUses ? Math.round((coupon.usedCount / coupon.maxUses) * 100) : null;
              const isEditing   = editing !== "new" && editing?.id === coupon.id;

              return (
                <View
                  key={coupon.id}
                  style={[li.card, { backgroundColor: card, borderColor: isEditing ? PINK : border, borderLeftColor: statusColor }]}
                >
                  <View style={li.row}>
                    {/* Code + discount */}
                    <View style={[li.codeWrap, { backgroundColor: PINK + "12" }]}>
                      <Text style={[li.code, { color: PINK }]}>{coupon.code}</Text>
                    </View>
                    <View style={{ flex: 1 }}>
                      <Text style={[li.discount, { color: text }]}>{fmtDiscount(coupon)}</Text>
                      {coupon.minOrderAmount && (
                        <Text style={[li.min, { color: sub }]}>Mín. €{(coupon.minOrderAmount / 100).toFixed(2)}</Text>
                      )}
                    </View>
                  ...
                  <View style={li.statsRow}>
                    ...
                    {coupon.maxUsesPerUser && (
                      <View style={li.statItem}>
                        ...
                        <Text style={[li.statTxt, { color: sub }]}>{coupon.maxUsesPerUser}/usuario</Text>
                      </View>
                    )}
                    {coupon.expiresAt && (
                      <View style={li.statItem}>
                  ...
                  {/* Barra de uso */}
                  {usePct !== null && (
                    <View style={[li.usageTrack, { backgroundColor: isDark ? "#333" : "#f0f0f0" }]}>
                      <View style={[li.usageFill, { width: `${Math.min(usePct, 100)}%` as any, backgroundColor: usePct >= 90 ? "#EF4444" : PINK }]} />
                    </View>
                  )}
                  ...
                  <View style={li.actions}>
                    <TouchableOpacity
                      ...
                      style={[li.actionBtn, { backgroundColor: isEditing ? PINK + "20" : "#3B82F615", borderColor: isEditing ? PINK : "#3B82F630" }]}
                    >
                      <Feather name="edit-2" size={12} color={isEditing ? PINK : "#3B82F6"} />
                      <Text style={[li.actionTxt, { color: isEditing ? PINK : "#3B82F6" }]}>{isEditing ? "Cerrar" : "Editar"}</Text>
                    </TouchableOpacity>
                    <TouchableOpacity
                      onPress={() => toggleActive(coupon)}
                      style={[li.actionBtn, { backgroundColor: coupon.isActive ? "#EF444415" : "#10B98115", borderColor: coupon.isActive ? "#EF444430" : "#10B98130" }]}
                    >
                      <Feather name={coupon.isActive ? "pause" : "play"} size={12} color={coupon.isActive ? "#EF4444" : "#10B981"} />
                      <Text style={[li.actionTxt, { color: coupon.isActive ? "#EF4444" : "#10B981" }]}>{coupon.isActive ? "Desactivar" : "Activar"}</Text>
                    </TouchableOpacity>
                    <TouchableOpacity
                      onPress={() => remove(coupon)}
                      disabled={deleting === coupon.id}
                      style={[li.actionBtn, { backgroundColor: "#EF444410", borderColor: "#EF444425" }]}
                    >
                      {deleting === coupon.id
...
const tb = StyleSheet.create({
  bar:           { flexDirection: "row", alignItems: "center", gap: 10, paddingHorizontal: 16, paddingVertical: 10, borderBottomWidth: 1 },
...
const li = StyleSheet.create({
  card:       { borderRadius: 12, padding: 14, borderWidth: 1, borderLeftWidth: 3, gap: 10 },
