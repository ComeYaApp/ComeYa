...
const ROLES = [
  ...
  { id: "business_owner",  label: "Negocio",      color: "#3B82F6", icon: "briefcase" },
  { id: "delivery_driver", label: "Repartidor",   color: "#10B981", icon: "truck"     },
...
export function UserDetail({ user, isDark, onClose, onUpdate }: Props) {
  ...
  const toggleActive = async () => {
    ...
    if (!window.confirm(`¿${action.charAt(0).toUpperCase() + action.slice(1)} a ${user.name}?`)) return;
    setProcessing(true);
    try {
      const res  = await apiRequest("PUT", `/api/admin/users/${user.id}`, { isActive: !user.isActive });
  ...
  const saveName = async () => {
    ...
    setProcessing(true);
    try {
      const res  = await apiRequest("PUT", `/api/admin/users/${user.id}`, { name: nameVal.trim() });
      const data = await res.json();
      if (data.success) {
  ...
  return (
    <View style={[det.panel, { backgroundColor: card, borderLeftColor: border }]}>
      ...
      <View style={[det.header, { borderBottomColor: border }]}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={[det.name, { color: text }]} numberOfLines={1}>{user.name}</Text>
          <View style={[det.rolePill, { backgroundColor: meta.color + "18" }]}>
            <Feather name={meta.icon as any} size={10} color={meta.color} />
            <Text style={[det.roleTxt, { color: meta.color }]}>{meta.label}</Text>
          </View>
        </View>
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 16, gap: 12 }} showsVerticalScrollIndicator={false}>
        ...
        {!user.isActive && (
          <View style={[det.alertBox, { backgroundColor: "#EF444415", borderColor: "#EF444430" }]}>
            ...
            <Text style={[det.alertTxt, { color: "#EF4444" }]}>Cuenta desactivada</Text>
          </View>
        )}
        ...
        <View style={[det.section, { backgroundColor: isDark ? "#222" : "#f8f8f8", borderColor: border }]}>
          ...
          {[
            ...
            { icon: "calendar", label: "Registrado",value: new Date(user.createdAt).toLocaleDateString("es-ES") },
            { icon: "clock",    label: "Últ. acceso",value: user.lastActiveAt ? new Date(user.lastActiveAt).toLocaleDateString("es-ES") : "—" },
          ].map(r => (
            <View key={r.label} style={det.infoRow}>
              ...
              <Text style={[det.infoVal, { color: text }]} numberOfLines={1}>{r.value}</Text>
            </View>
          ))}
        </View>
        ...
        <View style={[det.section, { backgroundColor: isDark ? "#222" : "#f8f8f8", borderColor: border }]}>
          ...
          {[
            ...
            <View key={v.label} style={det.verifyRow}>
              <Feather name={v.ok ? "check-circle" : "x-circle"} size={14} color={v.ok ? "#10B981" : "#EF4444"} />
          ...
          {editName ? (
            <View style={{ gap: 8 }}>
              ...
              <View style={{ flexDirection: "row", gap: 8 }}>
                <TouchableOpacity onPress={() => { setEditName(false); setNameVal(user.name); }} style={[det.btnSecondary, { flex: 1, borderColor: border }]}>
                  <Text style={[det.btnSecondaryTxt, { color: text }]}>Cancelar</Text>
                </TouchableOpacity>
                <TouchableOpacity onPress={saveName} disabled={processing} style={[det.btnPrimary, { flex: 1 }]}>
                  {processing ? <ActivityIndicator size="small" color="#fff" /> : <Text style={det.btnTxt}>Guardar</Text>}
                </TouchableOpacity>
              </View>
            </View>
          ) : (
        ...
        <View style={[det.section, { backgroundColor: isDark ? "#222" : "#f8f8f8", borderColor: border }]}>
          <View style={det.sectionHeaderRow}>
            ...
            {!editRole && (
              <TouchableOpacity onPress={() => { setEditRole(true); setNewRole(user.role); }}>
                <Feather name="edit-2" size={13} color="#3B82F6" />
              </TouchableOpacity>
            )}
          </View>
            ...
            <View style={{ gap: 8 }}>
              {ROLES.map(r => (
                <TouchableOpacity
                  ...
                  style={[det.roleOption, {
                    backgroundColor: newRole === r.id ? r.color + "18" : "transparent",
                    borderColor: newRole === r.id ? r.color : border,
                  }]}
                >
                  <Feather name={r.icon as any} size={14} color={newRole === r.id ? r.color : sub} />
                  <Text style={[det.roleOptionTxt, { color: newRole === r.id ? r.color : text }]}>{r.label}</Text>
                  {newRole === r.id && <Feather name="check" size={13} color={r.color} style={{ marginLeft: "auto" as any }} />}
                </TouchableOpacity>
              ))}
              <View style={{ flexDirection: "row", gap: 8, marginTop: 4 }}>
                <TouchableOpacity onPress={() => setEditRole(false)} style={[det.btnSecondary, { flex: 1, borderColor: border }]}>
                  <Text style={[det.btnSecondaryTxt, { color: text }]}>Cancelar</Text>
                </TouchableOpacity>
                <TouchableOpacity onPress={saveRole} disabled={processing} style={[det.btnPrimary, { flex: 1 }]}>
                  {processing ? <ActivityIndicator size="small" color="#fff" /> : <Text style={det.btnTxt}>Guardar rol</Text>}
                </TouchableOpacity>
              </View>
            </View>
          ) : (
            <View style={[det.rolePill, { backgroundColor: meta.color + "18", alignSelf: "flex-start" }]}>
        ...
        >
          {processing
            ...
            : <>
                <Feather name={user.isActive ? "user-x" : "user-check"} size={14} color="#fff" />
                <Text style={det.btnTxt}>{user.isActive ? "Desactivar cuenta" : "Activar cuenta"}</Text>
              </>
          }
        </TouchableOpacity>
...
const det = StyleSheet.create({
  ...
  rolePill:       { flexDirection: "row", alignItems: "center", gap: 4, paddingHorizontal: 8, paddingVertical: 3, borderRadius: 20, alignSelf: "flex-start" },
  ...
  roleOption:     { flexDirection: "row", alignItems: "center", gap: 8, padding: 10, borderRadius: 10, borderWidth: 1 },
  roleOptionTxt:  { fontSize: 13, fontWeight: "600" },
  btnPrimary:     { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 6, padding: 10, borderRadius: 10, backgroundColor: PRIMARY },
  btnDanger:      { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 6, padding: 12, borderRadius: 10, backgroundColor: "#EF4444" },
  btnSuccess:     { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 6, padding: 12, borderRadius: 10, backgroundColor: "#10B981" },
  btnSecondary:   { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 6, padding: 10, borderRadius: 10, borderWidth: 1 },
