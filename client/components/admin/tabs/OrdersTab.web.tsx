...
const ACTIVE_STATUSES  = ["pending", "confirmed", "preparing", "ready", "on_the_way", "picked_up"];
...
export const OrdersTab: React.FC<Props> = ({ mode = "active" }) => {
  ...
  // ── Detail ────────────────────────────────────────────────────────────────
  if (selected) {
    const meta = STATUS_META[selected.status] ?? { label: selected.status, color: "#888" };
    return (
      <ScrollView style={{ flex: 1, backgroundColor: bg }} contentContainerStyle={{ padding: 28 }}>
        ...
        <View style={[det.card, { backgroundColor: card, borderColor: border }]}>
          <View style={det.row}>
            <Text style={[det.orderId, { color: text }]}>#{selected.id?.slice(-8).toUpperCase()}</Text>
            <View style={[det.pill, { backgroundColor: meta.color + "20" }]}>
              <Text style={[det.pillTxt, { color: meta.color }]}>{meta.label}</Text>
            </View>
          </View>
          ...
          {[
            { icon: "briefcase",   label: "Negocio",    value: selected.businessName },
            { icon: "user",        label: "Cliente",    value: selected.customerName },
            { icon: "map-pin",     label: "Dirección",  value: selected.deliveryAddress },
            { icon: "credit-card", label: "Pago",       value: selected.paymentMethod },
            { icon: "truck",       label: "Repartidor", value: selected.deliveryPersonName ?? "Sin asignar" },
            { icon: "calendar",    label: "Creado",     value: new Date(selected.createdAt).toLocaleString("es-ES") },
          ].map(r => r.value ? (
            <View key={r.label} style={det.infoRow}>
              <View style={[det.infoIcon, { backgroundColor: ComeYaColors.primary + "15" }]}>
                <Feather name={r.icon as any} size={13} color={ComeYaColors.primary} />
              </View>
            ...
            <View style={{ marginBottom: 12 }}>
              ...
              {selected.items.map((item: any, i: number) => (
                <View key={i} style={det.itemRow}>
                  <Text style={[det.itemQty, { color: ComeYaColors.primary }]}>{item.quantity}×</Text>
                  <Text style={[det.itemName, { color: text }]} numberOfLines={1}>{item.name ?? item.productName}</Text>
                  <Text style={[det.itemPrice, { color: text }]}>€{((item.price ?? 0) / 100).toFixed(2)}</Text>
                </View>
              ))}
            </View>
          )}

          <View style={[det.totalsBox, { backgroundColor: isDark ? "#111" : "#f8f8f8", borderColor: border }]}>
            {selected.subtotal != null && (
              <View style={det.totalRow}>
                <Text style={[det.totalLabel, { color: sub }]}>Subtotal</Text>
                <Text style={[det.totalVal, { color: text }]}>€{(selected.subtotal / 100).toFixed(2)}</Text>
              </View>
            )}
            {selected.deliveryFee != null && (
              <View style={det.totalRow}>
                <Text style={[det.totalLabel, { color: sub }]}>Envío</Text>
                <Text style={[det.totalVal, { color: text }]}>€{(selected.deliveryFee / 100).toFixed(2)}</Text>
              </View>
            )}
            <View style={[det.totalRow, { marginTop: 6 }]}>
              <Text style={[det.totalLabel, { color: text, fontWeight: "700" }]}>Total</Text>
              <Text style={[det.totalVal, { color: ComeYaColors.primary, fontWeight: "800", fontSize: 18 }]}>
  ...
  return (
    <View style={{ flex: 1, backgroundColor: bg }}>
      ...
      <View style={[tb.bar, { backgroundColor: card, borderBottomColor: border }]}>
        ...
        <Text style={[tb.count, { color: sub }]}>{filtered.length} pedidos</Text>
      </View>
      ...
      >
        <TouchableOpacity
          ...
          style={[tb.chip, {
            backgroundColor: statusFilter === "all" ? ComeYaColors.primary : inputBg,
            borderColor: statusFilter === "all" ? ComeYaColors.primary : border,
          }]}
        >
          <Text style={[tb.chipTxt, { color: statusFilter === "all" ? "#fff" : text }]}>Todos</Text>
        </TouchableOpacity>
        ...
        {statuses.map(s => {
          ...
          return (
            ...
            >
              <Text style={[tb.chipTxt, { color: active ? "#fff" : text }]}>{m.label}</Text>
            </TouchableOpacity>
          );
        })}
      </ScrollView>
      ...
      {loading ? (
        ...
        <View style={{ flex: 1, justifyContent: "center", alignItems: "center", gap: 10 }}>
          ...
          <Text style={{ color: sub, fontSize: 15 }}>
            {mode === "active" ? "Sin pedidos activos" : "Sin historial de pedidos"}
          </Text>
        </View>
      ) : (
        <ScrollView
          ...
          {filtered.map(order => {
            ...
            return (
              <TouchableOpacity
                ...
                style={[li.card, { backgroundColor: card, borderColor: border, borderLeftColor: meta.color }]}
              >
                <View style={li.top}>
                  <Text style={[li.id, { color: text }]}>#{order.id?.slice(-8).toUpperCase()}</Text>
                  <View style={[li.pill, { backgroundColor: meta.color + "18" }]}>
                    <View style={[li.dot, { backgroundColor: meta.color }]} />
                    <Text style={[li.pillTxt, { color: meta.color }]}>{meta.label}</Text>
                  </View>
                ...
                <View style={li.mid}>
                  ...
                  <Text style={[li.bizName, { color: text }]} numberOfLines={1}>{order.businessName}</Text>
                  <Feather name="user" size={12} color={sub} style={{ marginLeft: 10 }} />
                  <Text style={[li.bizName, { color: sub }]} numberOfLines={1}>{order.customerName}</Text>
                </View>
                <Text style={[li.date, { color: sub }]}>{new Date(order.createdAt).toLocaleString("es-ES")}</Text>
              </TouchableOpacity>
            );
          })}
        </ScrollView>
