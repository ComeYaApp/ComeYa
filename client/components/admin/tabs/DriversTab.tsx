...
export const DriversTab: React.FC<Props> = ({ theme, showToast }) => {
      ...
      [
        { text: "Cancelar", style: "cancel" },
        {
          ...
          onPress: async () => {
            setProcessing(true);
            try {
              const res  = await apiRequest("POST", `/api/admin/drivers/${driver.id}/${driver.isBlocked ? "unblock" : "block"}`);
              const data = await res.json();
              if (data.success) {
  ...
  if (selected) {
    const ratingDisplay = selected.rating != null ? (selected.rating / 10).toFixed(1) : "N/A";
    return (
      <ScrollView style={{ flex: 1, backgroundColor: theme.backgroundRoot }} contentContainerStyle={{ padding: 16, paddingBottom: 100 }}>
        ...
        <View style={s.card}>
          <View style={s.row}>
            <View style={[s.avatar, { backgroundColor: "#9C27B020" }]}>
              <Text style={[s.avatarText, { color: "#9C27B0" }]}>{selected.name.charAt(0).toUpperCase()}</Text>
            </View>
            <View style={{ flex: 1, marginLeft: 12 }}>
              <Text style={[s.name, { color: theme.text }]}>{selected.name}</Text>
              {selected.email && <Text style={[s.sub, { color: theme.textSecondary }]}>{selected.email}</Text>}
              {selected.phone && <Text style={[s.sub, { color: theme.textSecondary }]}>{selected.phone}</Text>}
            </View>
            <View style={[s.badge, { backgroundColor: selected.isOnline ? ComeYaColors.success + "20" : theme.backgroundSecondary }]}>
              <Text style={[s.badgeText, { color: selected.isOnline ? ComeYaColors.success : theme.textSecondary }]}>
                {selected.isOnline ? "● En línea" : "Desconectado"}
              </Text>
            </View>
          </View>
          {selected.isBlocked && (
            <View style={[s.alertBox, { backgroundColor: ComeYaColors.error + "15", borderColor: ComeYaColors.error + "40" }]}>
              <Feather name="alert-triangle" size={14} color={ComeYaColors.error} />
              <Text style={[s.sub, { color: ComeYaColors.error, marginLeft: 6 }]}>
                Bloqueado{selected.blockedReason ? `: ${selected.blockedReason}` : ""}
              </Text>
            </View>
          )}
        </View>
        ...
        <View style={[s.card, { flexDirection: "row", justifyContent: "space-around" }]}>
          <Stat label="Entregas"  value={selected.totalDeliveries} color={ComeYaColors.primary} theme={theme} />
          <Stat label="Rating"    value={ratingDisplay}            color={ComeYaColors.warning} theme={theme} />
          <Stat label="Strikes"   value={selected.strikes}         color={selected.strikes > 0 ? ComeYaColors.error : ComeYaColors.success} theme={theme} />
          {selected.pendingPayouts > 0 && (
            <Stat label="Pagos pend." value={selected.pendingPayouts} color={ComeYaColors.warning} theme={theme} />
          )}
        </View>
          ...
          <View style={s.card}>
            ...
            <View style={s.infoRow}>
              ...
              <Text style={[s.sub, { color: theme.text, marginLeft: 8 }]}>
                {selected.vehicleType ?? "—"}{selected.vehiclePlate ? ` · ${selected.vehiclePlate}` : ""}
              </Text>
            </View>
          </View>
        )}
        ...
        <View style={s.card}>
          ...
          <InfoRow icon="calendar"   label="Registrado"      value={new Date(selected.createdAt).toLocaleDateString("es-ES")} theme={theme} />
          {selected.lastActiveAt && (
            <InfoRow icon="clock"    label="Última actividad" value={new Date(selected.lastActiveAt).toLocaleDateString("es-ES")} theme={theme} />
          )}
          {selected.pendingAmount > 0 && (
            <InfoRow icon="dollar-sign" label="Monto pendiente" value={`€${(selected.pendingAmount / 100).toFixed(2)}`} theme={theme} color={ComeYaColors.warning} />
          )}
          {selected.currentLatitude && selected.currentLongitude && (
            <InfoRow icon="map-pin" label="Última ubicación" value={`${parseFloat(selected.currentLatitude).toFixed(4)}, ${parseFloat(selected.currentLongitude).toFixed(4)}`} theme={theme} />
          )}
        </View>

        {/* Acciones */}
        <TouchableOpacity
          style={[s.btn, { backgroundColor: selected.isBlocked ? ComeYaColors.success : ComeYaColors.error }]}
        ...
        >
          {processing
            ...
            : <>
                <Feather name={selected.isBlocked ? "unlock" : "lock"} size={16} color="#FFF" />
                <Text style={s.btnText}>{selected.isBlocked ? "Desbloquear repartidor" : "Bloquear repartidor"}</Text>
              </>
          }
        </TouchableOpacity>
  ...
  return (
    <View style={{ flex: 1, backgroundColor: theme.backgroundRoot }}>
      {/* Resumen */}
      <View style={[s.card, { margin: 16, marginBottom: 8, flexDirection: "row", justifyContent: "space-around" }]}>
        <Stat label="Total"      value={drivers.length} color={ComeYaColors.primary} theme={theme} />
        <Stat label="En línea"   value={online}         color={ComeYaColors.success} theme={theme} />
        <Stat label="Bloqueados" value={blocked}        color={blocked > 0 ? ComeYaColors.error : theme.textSecondary} theme={theme} />
      </View>
      ...
      >
        {drivers.length === 0 ? (
          <View style={s.empty}>
            ...
            <Text style={[s.emptyText, { color: theme.textSecondary }]}>Sin repartidores registrados</Text>
          </View>
        ) : (
          drivers.map(driver => (
            <TouchableOpacity key={driver.id} style={s.card} onPress={() => setSelected(driver)}>
              <View style={s.row}>
                <View style={[s.avatarSm, { backgroundColor: "#9C27B015" }]}>
                  <Text style={{ color: "#9C27B0", fontWeight: "700", fontSize: 16 }}>{driver.name.charAt(0).toUpperCase()}</Text>
                </View>
                <View style={{ flex: 1, marginLeft: 10 }}>
                  <Text style={[s.name, { color: theme.text }]}>{driver.name}</Text>
                  {driver.email && <Text style={[s.sub, { color: theme.textSecondary }]}>{driver.email}</Text>}
                  <Text style={[s.sub, { color: theme.textSecondary }]}>{driver.phone ?? "Sin teléfono"}</Text>
                </View>
                <View style={{ alignItems: "flex-end", gap: 4 }}>
                  <View style={[s.badge, { backgroundColor: driver.isOnline ? ComeYaColors.success + "20" : theme.backgroundSecondary }]}>
                    <Text style={[s.badgeText, { color: driver.isOnline ? ComeYaColors.success : theme.textSecondary }]}>
                      {driver.isOnline ? "● Online" : "Offline"}
                    </Text>
                  </View>
                  {driver.isBlocked && (
                    <View style={[s.badge, { backgroundColor: ComeYaColors.error + "20" }]}>
                      <Text style={[s.badgeText, { color: ComeYaColors.error }]}>Bloqueado</Text>
                    </View>
                  )}
                </View>
              </View>
              <View style={[s.statsRow, { marginTop: 10, paddingTop: 10, borderTopWidth: 1, borderTopColor: theme.border }]}>
                <Text style={[s.statItem, { color: theme.textSecondary }]}>
                  <Text style={{ color: ComeYaColors.primary, fontWeight: "700" }}>{driver.totalDeliveries}</Text> entregas
                </Text>
                <Text style={[s.statItem, { color: theme.textSecondary }]}>
                  ⭐ <Text style={{ color: theme.text, fontWeight: "600" }}>{driver.rating != null ? (driver.rating / 10).toFixed(1) : "N/A"}</Text>
                </Text>
                <Text style={[s.statItem, { color: driver.strikes > 0 ? ComeYaColors.error : theme.textSecondary }]}>
...
function InfoRow({ icon, label, value, theme, color }: any) {
  return (
    <View style={{ flexDirection: "row", alignItems: "center", paddingVertical: 6 }}>
      <Feather name={icon} size={14} color={theme.textSecondary} style={{ width: 20 }} />
      <Text style={{ fontSize: 13, color: theme.textSecondary, marginLeft: 8, flex: 1 }}>{label}</Text>
      <Text style={{ fontSize: 13, fontWeight: "600", color: color ?? theme.text }}>{value}</Text>
    </View>
  );
}
...
const st = (theme: any) => StyleSheet.create({
  ...
  avatarSm:    { width: 42, height: 42, borderRadius: 21, justifyContent: "center", alignItems: "center" },
  ...
  alertBox:    { flexDirection: "row", alignItems: "center", marginTop: 10, padding: 8, borderRadius: 8, borderWidth: 1 },
  btn:         { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8, padding: 14, borderRadius: 10, marginTop: 4 },
