...
export const BusinessesTab: React.FC<Props> = () => {
  ...
  const saveCommission = async () => {
    ...
    try {
      ...
      const res = await apiRequest(
        "PUT",
        `/api/admin/businesses/${selected.id}/commission`,
        { customCommission: val },
      );
      const data = await res.json();
      if (data.success) {
        ...
        setBusinesses((prev) =>
          prev.map((b) =>
            b.id === selected.id ? { ...b, customCommission: val } : b,
          ),
        );
  ...
  const toggleActive = async (biz: any) => {
    try {
      await apiRequest("PUT", `/api/admin/businesses/${biz.id}`, {
        isActive: !biz.isActive,
      });
      setBusinesses((prev) =>
        prev.map((b) =>
          b.id === biz.id ? { ...b, isActive: !b.isActive } : b,
        ),
      );
      if (selected?.id === biz.id)
        setSelected((p: any) => ({ ...p, isActive: !p.isActive }));
    } catch {}
  };
  ...
  const toggleFeatured = async (biz: any) => {
    try {
      ...
      setBusinesses((prev) =>
        prev.map((b) =>
          b.id === biz.id ? { ...b, isFeatured: !b.isFeatured } : b,
        ),
      );
      if (selected?.id === biz.id)
      ...
      <View style={{ flex: 1, backgroundColor: bg, flexDirection: "row" }}>
        ...
        >
          ...
          >
            ...
            <Text style={[det.backTxt, { color: PRIMARY }]}>
              Todos los negocios
            </Text>
          </TouchableOpacity>
          <ScrollView showsVerticalScrollIndicator={false}>
            {filtered.map((b) => (
              <TouchableOpacity
                ...
                onPress={() => {
                  setSelected(b);
                  setCommission(b.customCommission?.toString() ?? "");
                  setSaveMsg(null);
                }}
                style={[
                  det.listItem,
              ...
              >
                <View
                  style={[
                    det.listDot,
                    { backgroundColor: b.isActive ? "#10B981" : "#EF4444" },
                  ]}
                />
                <Text style={[det.listName, { color: text }]} numberOfLines={1}>
                  {b.name}
                </Text>
              </TouchableOpacity>
            ))}
        ...
        <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 28 }}>
          ...
          <View style={det.headerRow}>
            <View style={{ flex: 1 }}>
              <Text style={[det.bizName, { color: text }]}>
                {selected.name}
              </Text>
              <Text style={[det.bizType, { color: sub }]}>
            ...
            <View style={{ flexDirection: "row", gap: 8 }}>
              <TouchableOpacity
                ...
                style={[
                  det.toggleBtn,
                  {
                    backgroundColor: selected.isFeatured
                      ? "#FFB80015"
                      : "transparent",
                    borderColor: selected.isFeatured ? "#FFB800" : border,
                  },
                ]}
              >
                <Feather
                  name="star"
                  size={12}
                  color={selected.isFeatured ? "#FFB800" : sub}
                />
                <Text
                  style={[
                    det.toggleTxt,
              ...
              <TouchableOpacity
                ...
                style={[
                  det.toggleBtn,
                  {
                    backgroundColor: selected.isActive
                      ? "#10B98115"
                      : "#EF444415",
                    borderColor: selected.isActive ? "#10B981" : "#EF4444",
                  },
                ]}
              >
                <View
                ...
                <Text
                  style={[
                    det.toggleTxt,
                    { color: selected.isActive ? "#10B981" : "#EF4444" },
                  ]}
                >
          ...
          <View style={det.infoRow}>
            {[
              ...
              >
                <View
                  style={[det.infoIcon, { backgroundColor: PRIMARY + "12" }]}
                >
          ...
          {level && (
            ...
            >
              ...
              <Text style={[det.levelTxt, { color: level.color }]}>
                Nivel {level.label}
              </Text>
            </View>
          )}
          ...
          >
            ...
            <Text style={[det.sectionSub, { color: sub }]}>
              Actual:{" "}
              {selected.customCommission != null
                ? `${selected.customCommission}% (personalizada)`
                : "Global del sistema"}
            </Text>
            <View style={det.commRow}>
              ...
              <TouchableOpacity
                ...
                style={[
                  det.saveBtn,
                  { backgroundColor: saving ? sub : PRIMARY },
                ]}
              >
                {saving ? (
                  ...
                  <>
                    ...
                    <Text style={det.saveTxt}>Guardar</Text>
                  </>
                )}
              </TouchableOpacity>
            </View>
            {saveMsg && (
              <View
                style={[
                  det.msgBox,
                  { backgroundColor: saveMsg.ok ? "#10B98115" : "#EF444415" },
                ]}
              >
                <Feather
                  name={saveMsg.ok ? "check-circle" : "alert-circle"}
                  size={13}
          ...
          {(selected.totalOrders != null || selected.totalRevenue != null) && (
            ...
            >
              <View style={det.sectionHeader}>
                <View
                  style={[det.sectionIcon, { backgroundColor: "#3B82F615" }]}
                >
              ...
              <View style={det.statsRow}>
                {[
                  ...
                  {
                    label: "Completados",
                    value: selected.completedOrders ?? 0,
                    color: "#8B5CF6",
                  },
                ].map((s) => (
                  <View
                    key={s.label}
                    style={[det.statCard, { backgroundColor: s.color + "10" }]}
                  >
                    <Text style={[det.statVal, { color: s.color }]}>
                      {s.value}
                    </Text>
                    <Text style={[det.statLabel, { color: sub }]}>
                      {s.label}
                    </Text>
                  </View>
                ))}
        ...
        {[
          ...
          <TouchableOpacity
            ...
            style={[
              tb.chip,
              {
                backgroundColor: typeFilter === f.id ? "#8B5CF6" : inputBg,
                borderColor: typeFilter === f.id ? "#8B5CF6" : border,
              },
            ]}
          >
            <Text
              style={[
                tb.chipTxt,
                { color: typeFilter === f.id ? "#fff" : text },
              ]}
            >
              {f.label}
            </Text>
          ...
          {filtered.map((biz) => {
            ...
            return (
              <TouchableOpacity
                ...
                onPress={() => {
                  setSelected(biz);
                  setCommission(biz.customCommission?.toString() ?? "");
                  setSaveMsg(null);
                }}
                style={[
                  li.card,
                  {
                    ...
                    borderColor: border,
                    borderLeftColor: biz.isActive ? "#10B981" : "#EF4444",
                  },
                ]}
              >
                <View style={li.top}>
                  <View
                    style={[
                      li.dot,
                      { backgroundColor: biz.isActive ? "#10B981" : "#EF4444" },
                    ]}
                  />
                  <Text style={[li.name, { color: text }]} numberOfLines={1}>
                    {biz.name}
                  </Text>
                  {level && (
                    <View
                  ...
                  <View style={[li.typePill, { backgroundColor: "#8B5CF615" }]}>
                    <Text style={[li.typeTxt, { color: "#8B5CF6" }]}>
                      {TYPE_LABEL[biz.type] ?? biz.type}
                    </Text>
                  </View>
                </View>
                <View style={li.mid}>
                  <Feather name="map-pin" size={11} color={sub} />
                  <Text style={[li.addr, { color: sub }]} numberOfLines={1}>
                    {biz.address ?? "Sin dirección"}
                  </Text>
                  <Feather
                    name="percent"
                    size={11}
                  ...
                  <Text style={[li.addr, { color: sub }]}>
                    {biz.customCommission != null
                      ? `${biz.customCommission}%`
                      : "Global"}
                  </Text>
                </View>
              </TouchableOpacity>
