...
export function SupportTab({}: Props) {
  ...
  const updateStatus = async (ticketId: string, status: string) => {
    ...
    try {
      await apiRequest("PATCH", `/api/support/tickets/${ticketId}/status`, {
        status,
      });
      setTickets((prev) =>
      ...
      flash(false, "Error al actualizar");
    } finally {
      setUpdating(null);
    }
  };
    ...
    <View style={{ flex: 1, backgroundColor: bg, flexDirection: "row" }}>
                ...
                >
                  <View style={li.top}>
                    ...
                    <View
                      style={[
                        li.priorityPill,
                        { backgroundColor: priorityMeta.color + "18" },
                      ]}
                    >
                      <Text
                        style={[li.priorityTxt, { color: priorityMeta.color }]}
                      >
                        {priorityMeta.label}
                      </Text>
                    </View>
                  </View>
                  <Text style={[li.user, { color: sub }]}>
                    {ticket.userName}
                  </Text>
                  <View style={li.bottom}>
                    ...
                    >
                      ...
                      />
                      <Text style={[li.statusTxt, { color: statusMeta.color }]}>
                        {statusMeta.label}
                      </Text>
                    </View>
                    ...
                    <View style={li.actions}>
                      {ticket.status === "open" && (
                        ...
                        >
                          ...
                          <Text style={[li.actionTxt, { color: "#F59E0B" }]}>
                            En proceso
                          </Text>
                        </TouchableOpacity>
                      )}
                      ...
                      >
                        ...
                        <Text style={[li.actionTxt, { color: "#10B981" }]}>
                          Cerrar
                        </Text>
                      </TouchableOpacity>
                    </View>
      ...
      {selected && (
        <View
          style={[
            chat.panel,
            { backgroundColor: card, borderLeftColor: border, flex: 1 },
          ]}
        >
          {/* Header */}
          <View style={[chat.header, { borderBottomColor: border }]}>
            <View style={{ flex: 1 }}>
              <Text style={[chat.subject, { color: text }]} numberOfLines={1}>
                {selected.subject}
              </Text>
              <View style={chat.metaRow}>
                <Text style={[chat.user, { color: sub }]}>
                  {selected.userName}
                </Text>
                <View
                  style={[
                    ...
                    {
                      backgroundColor:
                        (STATUS_META[selected.status]?.color ?? "#888") + "18",
                    },
                  ]}
                >
                  <Text
                    style={[
                      chat.statusTxt,
                      { color: STATUS_META[selected.status]?.color ?? "#888" },
                    ]}
                  >
              ...
              {selected.status !== "closed" && (
                <>
                  {selected.status === "open" && (
                    ...
                    >
                      <Text style={[chat.headerBtnTxt, { color: "#F59E0B" }]}>
                        En proceso
                      </Text>
                    </TouchableOpacity>
                  )}
                  ...
                  >
                    <Text style={[chat.headerBtnTxt, { color: "#10B981" }]}>
                      Cerrar ticket
                    </Text>
                  </TouchableOpacity>
                </>
            ...
            >
              ...
              {messages.map((m) => {
                ...
                return (
                  <View
                    key={m.id}
                    style={[msg_s.wrap, isAdmin && msg_s.wrapRight]}
                  >
                    <View
                      style={[
                        msg_s.bubble,
                    ...
                    >
                      <Text
                        style={[msg_s.sender, { color: isAdmin ? GREEN : sub }]}
                      >
                        {m.senderName}
                      </Text>
                      <Text style={[msg_s.body, { color: text }]}>
                        {m.message}
                      </Text>
                      <Text style={[msg_s.time, { color: sub }]}>
          ...
          {selected.status !== "closed" ? (
            <View
              style={[
                chat.replyBox,
                { borderTopColor: border, backgroundColor: card },
              ]}
            >
              <TextInput
                style={[
              ...
              <TouchableOpacity
                ...
                style={[
                  chat.sendBtn,
                  { backgroundColor: reply.trim() ? GREEN : sub },
                ]}
              >
                {sending ? (
            ...
            <View
              style={[
                chat.closedBar,
                { borderTopColor: border, backgroundColor: "#6B728010" },
              ]}
            >
              <Feather name="lock" size={13} color={sub} />
              <Text style={[chat.closedTxt, { color: sub }]}>
