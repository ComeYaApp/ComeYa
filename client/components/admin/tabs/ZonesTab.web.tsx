      ...
      >
        ...
        >
          ...
          <View style={det.headerRow}>
            ...
            <View style={{ flex: 1 }}>
              <Text style={[det.zoneName, { color: text }]}>
                {selected.name}
              </Text>
              {selected.description && (
                <Text style={[det.zoneSub, { color: sub }]}>
                  {selected.description}
                </Text>
              )}
            </View>
            ...
            >
              <View
                style={[
                  det.statusDot,
                  {
                    backgroundColor: selected.isActive ? "#10B981" : "#EF4444",
                  },
                ]}
              />
              <Text
            ...
            {[
              {
                ...
                value: fmt(selected.deliveryFee),
                color: "#10B981",
              },
              {
                icon: "clock",
                label: "Tiempo máximo",
                value: `${selected.maxDeliveryTime ?? 0} min`,
                color: "#F59E0B",
              },
              {
                icon: "circle",
                label: "Radio de cobertura",
                value: `${selected.radiusKm ?? 0} km`,
                color: "#3B82F6",
              },
              {
                icon: "map",
                label: "Coordenadas",
                value: selected.centerLatitude
                  ? `${parseFloat(selected.centerLatitude).toFixed(4)}, ${parseFloat(selected.centerLongitude ?? "0").toFixed(4)}`
                  : "No definidas",
                color: "#8B5CF6",
              },
            ].map((s) => (
              ...
              >
                <View
                  style={[det.statIcon, { backgroundColor: s.color + "20" }]}
                >
        ...
        >
          ...
          <Text style={[det.infoTxt, { color: "#3B82F6" }]}>
            Las zonas de entrega definen las áreas donde ComeYa opera. La tarifa
            se aplica automáticamente al calcular el costo de envío para pedidos
            dentro del radio.
          </Text>
        </View>
      </ScrollView>
  ...
  return (
    <View style={{ flex: 1, backgroundColor: bg }}>
      ...
      >
        ...
        <View style={{ flex: 1 }}>
          <Text style={[li.headerTitle, { color: text }]}>
            Zonas de entrega
          </Text>
          <Text style={[li.headerSub, { color: sub }]}>
      ...
      {loading ? (
        ...
        >
          ...
          <Text style={[li.emptyTitle, { color: text }]}>
            Sin zonas configuradas
          </Text>
          <Text style={[li.emptySub, { color: sub }]}>
            La tabla de zonas existe pero está vacía. Actualmente el coste de
            envío se calcula desde el campo{" "}
            <Text style={{ fontWeight: "700", color: text }}>deliveryFee</Text>{" "}
            de cada negocio.{"\n\n"}
            Para usar zonas dinámicas habría que añadir endpoints POST/PUT al
            backend y conectarlos al cálculo del checkout.
          </Text>
        </View>
      ) : (
        <ScrollView
            ...
            <TouchableOpacity
              ...
              style={[
                li.card,
                {
                  ...
                  borderColor: border,
                  borderLeftColor: zone.isActive ? "#10B981" : "#EF4444",
                },
              ]}
            >
              ...
              <View style={li.metaRow}>
                <View style={li.metaItem}>
                  ...
                  <Text style={[li.metaTxt, { color: sub }]}>
                    {fmt(zone.deliveryFee)}
                  </Text>
                </View>
                <View style={li.metaItem}>
                  <Feather name="clock" size={11} color="#F59E0B" />
                  <Text style={[li.metaTxt, { color: sub }]}>
                    {zone.maxDeliveryTime ?? 0} min
                  </Text>
                </View>
                <View style={li.metaItem}>
