...
export const VerificationsTab: React.FC<Props> = () => {
  ...
  useEffect(() => {
    let list = users;
    if (filter !== "all") list = list.filter(u => (u.verificationStatus ?? "pending") === filter);
    if (search.trim()) {
      const q = search.toLowerCase();
      list = list.filter(u => u.name?.toLowerCase().includes(q) || u.phone?.toLowerCase().includes(q));
    }
    setFiltered(list);
  }, [filter, search, users]);
  ...
  const handleAction = async (action: "approve" | "reject") => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setUsers(prev => prev.map(u => u.id === selected.id ? { ...u, verificationStatus: newStatus } : u));
        setSelected((p: any) => p ? { ...p, verificationStatus: newStatus } : null);
        flash(true, action === "approve" ? "✅ Usuario aprobado" : "❌ Usuario rechazado");
        setNotes("");
      } else {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: bg, flexDirection: "row" }}>
      ...
      {/* ── Lista ── */}
      <View style={{ width: selected ? 360 : undefined, flex: selected ? undefined : 1 }}>
        ...
        {filtered.length === 0 ? (
          <View style={{ flex: 1, justifyContent: "center", alignItems: "center", gap: 10 }}>
            ...
            <Text style={{ color: sub, fontSize: 15 }}>
              {filter === "pending" ? "Sin solicitudes pendientes" : "Sin resultados"}
            </Text>
          </View>
        ) : (
          <ScrollView
          ...
          >
            {filtered.map(u => {
              ...
              const rMeta     = ROLE_META[u.role]    ?? { label: u.role, color: "#888", icon: "user" };
              ...
              return (
                ...
                >
                  <View style={li.row}>
                    <View style={[li.avatar, { backgroundColor: rMeta.color + "20" }]}>
                      <Text style={[li.avatarTxt, { color: rMeta.color }]}>
                        {u.name?.charAt(0).toUpperCase() ?? "?"}
                      </Text>
                    </View>
                    <View style={{ flex: 1 }}>
                      <Text style={[li.name, { color: text }]} numberOfLines={1}>{u.name}</Text>
                      <Text style={[li.phone, { color: sub }]}>{u.phone}</Text>
                      <View style={li.badges}>
                        <View style={[li.rolePill, { backgroundColor: rMeta.color + "18" }]}>
                          <Feather name={rMeta.icon as any} size={10} color={rMeta.color} />
                          <Text style={[li.roleTxt, { color: rMeta.color }]}>{rMeta.label}</Text>
                        </View>
                        <View style={[li.statusPill, { backgroundColor: sMeta.color + "15" }]}>
                          <Feather name={sMeta.icon as any} size={10} color={sMeta.color} />
                          <Text style={[li.statusTxt, { color: sMeta.color }]}>{sMeta.label}</Text>
                        </View>
                      </View>
                      {/* Doc badges */}
                      <View style={li.docRow}>
                        <DocBadge label="DNI/NIE"  has={!!u.idDocumentUrl}       isDark={isDark} />
                        <DocBadge label="Autónomo" has={!!u.autonomoDocumentUrl} isDark={isDark} />
                      </View>
                    </View>
      ...
      {selected && (() => {
        ...
        const sMeta  = STATUS_META[status]  ?? STATUS_META.pending;
        const rMeta  = ROLE_META[selected.role] ?? { label: selected.role, color: "#888", icon: "user" };
        const dd     = selected.deliveryDriver;
        const biz    = selected.business;

        return (
          <View style={[det.panel, { backgroundColor: card, borderLeftColor: border }]}>
            {/* Header */}
            <View style={[det.header, { borderBottomColor: border }]}>
              <View style={[det.avatar, { backgroundColor: rMeta.color + "20" }]}>
                <Text style={[det.avatarTxt, { color: rMeta.color }]}>
                  {selected.name?.charAt(0).toUpperCase() ?? "?"}
                </Text>
              </View>
              <View style={{ flex: 1 }}>
                <Text style={[det.name, { color: text }]} numberOfLines={1}>{selected.name}</Text>
                <View style={[det.rolePill, { backgroundColor: rMeta.color + "18" }]}>
                  <Feather name={rMeta.icon as any} size={10} color={rMeta.color} />
                  <Text style={[det.roleTxt, { color: rMeta.color }]}>{rMeta.label}</Text>
                </View>
              </View>
            ...
            <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 16, gap: 12 }} showsVerticalScrollIndicator={false}>
              ...
              {/* Estado actual */}
              <View style={[det.statusBox, { backgroundColor: sMeta.color + "12", borderColor: sMeta.color + "30" }]}>
                <Feather name={sMeta.icon as any} size={15} color={sMeta.color} />
                <Text style={[det.statusTxt, { color: sMeta.color }]}>{sMeta.label}</Text>
                {selected.verificationNotes && (
                  <Text style={[det.statusNote, { color: sMeta.color }]}>· {selected.verificationNotes}</Text>
                )}
              </View>

              {/* Datos personales */}
              <Section title="DATOS PERSONALES" isDark={isDark}>
                <InfoRow icon="phone"    label="Teléfono"   value={selected.phone   ?? "—"} text={text} sub={sub} />
                <InfoRow icon="mail"     label="Email"      value={selected.email   ?? "—"} text={text} sub={sub} />
                <InfoRow icon="hash"     label="DNI/NIE"    value={selected.dni     ?? "—"} text={text} sub={sub} />
                <InfoRow icon="map-pin"  label="Dirección"  value={selected.address ?? "—"} text={text} sub={sub} />
                <InfoRow icon="calendar" label="Registrado" value={new Date(selected.createdAt).toLocaleDateString("es-ES")} text={text} sub={sub} />
              </Section>

              {/* Vehículo (repartidor) */}
              {dd && (
                <Section title="VEHÍCULO" isDark={isDark}>
                  <InfoRow icon="truck"    label="Tipo"      value={dd.vehicleType  ?? "—"} text={text} sub={sub} />
                  <InfoRow icon="hash"     label="Matrícula" value={dd.vehiclePlate ?? "—"} text={text} sub={sub} />
                  <InfoRow icon="box"      label="Marca"     value={dd.vehicleBrand ?? "—"} text={text} sub={sub} />
                  <InfoRow icon="settings" label="Modelo"    value={dd.vehicleModel ?? "—"} text={text} sub={sub} />
                  <InfoRow icon="droplet"  label="Color"     value={dd.vehicleColor ?? "—"} text={text} sub={sub} />
                </Section>
              )}
              ...
              {biz && (
                <Section title="NEGOCIO" isDark={isDark}>
                  <InfoRow icon="briefcase" label="Nombre"    value={biz.name    ?? "—"} text={text} sub={sub} />
                  <InfoRow icon="tag"       label="Tipo"      value={biz.type    ?? "—"} text={text} sub={sub} />
                  <InfoRow icon="map-pin"   label="Dirección" value={biz.address ?? "—"} text={text} sub={sub} />
                  <InfoRow icon="phone"     label="Teléfono"  value={biz.phone   ?? "—"} text={text} sub={sub} />
                </Section>
              )}
              ...
              {/* Nota admin */}
              <View>
                <Text style={[det.sectionTitle, { color: sub }]}>NOTA (motivo de rechazo o comentario)</Text>
                <TextInput
                  style={[det.notesInput, { backgroundColor: inputBg, borderColor: border, color: text }]}
                  value={notes}
                ...
                <View style={det.actions}>
                  ...
                  >
                    {saving ? <ActivityIndicator size="small" color="#fff" /> : (
                      <><Feather name="x-circle" size={14} color="#fff" /><Text style={det.btnTxt}>Rechazar</Text></>
                    )}
                  </TouchableOpacity>
                  <TouchableOpacity
                  ...
                  >
                    {saving ? <ActivityIndicator size="small" color="#fff" /> : (
                      <><Feather name="check-circle" size={14} color="#fff" /><Text style={det.btnTxt}>Aprobar</Text></>
                    )}
                  </TouchableOpacity>
                </View>
              )}
              ...
              {status !== "pending" && (
                <TouchableOpacity
                  onPress={() => handleAction(status === "verified" ? "reject" : "approve")}
                  disabled={saving}
                  style={[status === "verified" ? det.btnReject : det.btnApprove, { opacity: saving ? 0.6 : 1 }]}
                >
                  {saving ? <ActivityIndicator size="small" color="#fff" /> : (
                    <Text style={det.btnTxt}>{status === "verified" ? "Revocar aprobación" : "Aprobar igualmente"}</Text>
                  )}
                </TouchableOpacity>
              )}
...
function Section({ title, children, isDark }: any) {
  ...
  return (
    <View>
      <Text style={[det.sectionTitle, { color: isDark ? "#666" : "#aaa" }]}>{title}</Text>
      <View style={[det.sectionBox, { backgroundColor: bg, borderColor: border }]}>{children}</View>
    </View>
  );
}
...
function InfoRow({ icon, label, value, text, sub }: any) {
  return (
    <View style={det.infoRow}>
      ...
      <Text style={[det.infoVal, { color: text }]} numberOfLines={2}>{value}</Text>
    </View>
  );
}
...
function DocDetailRow({ label, url, text, sub }: any) {
  return (
    <View style={det.infoRow}>
      <Feather name={url ? "check-circle" : "alert-circle"} size={12} color={url ? "#10B981" : "#F59E0B"} />
      <Text style={[det.infoLbl, { color: sub }]}>{label}</Text>
      {url ? (
        // @ts-ignore
        <a href={url} target="_blank" rel="noopener noreferrer" style={{ color: "#3B82F6", fontSize: 12, fontWeight: "600", textDecoration: "none" }}>
          Ver documento ↗
        </a>
      ) : (
        <Text style={{ color: "#F59E0B", fontSize: 12, fontWeight: "600" }}>No subido</Text>
      )}
    </View>
  );
}
...
function DocBadge({ label, has, isDark }: any) {
  ...
  return (
    <View style={[li.docBadge, { backgroundColor: color + "18" }]}>
      <Feather name={has ? "file-text" : "alert-circle"} size={9} color={color} />
...
const tb = StyleSheet.create({
  ...
  chip:          { paddingHorizontal: 12, paddingVertical: 6, borderRadius: 20, borderWidth: 1, alignSelf: "flex-start" },
...
const li = StyleSheet.create({
  ...
  rolePill:  { flexDirection: "row", alignItems: "center", gap: 3, paddingHorizontal: 6, paddingVertical: 2, borderRadius: 8 },
  ...
  docBadge:  { flexDirection: "row", alignItems: "center", gap: 3, paddingHorizontal: 5, paddingVertical: 2, borderRadius: 6 },
...
const det = StyleSheet.create({
  ...
  rolePill:    { flexDirection: "row", alignItems: "center", gap: 4, paddingHorizontal: 7, paddingVertical: 3, borderRadius: 20, alignSelf: "flex-start" },
  roleTxt:     { fontSize: 10, fontWeight: "700" },
  ...
  sectionTitle:{ fontSize: 9, fontWeight: "800", letterSpacing: 1, marginBottom: 6 },
  ...
  notesInput:  { borderWidth: 1, borderRadius: 8, paddingHorizontal: 12, paddingVertical: 9, fontSize: 13, minHeight: 70 },
  actions:     { flexDirection: "row", gap: 10 },
  btnApprove:  { flex: 1, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 6, padding: 12, borderRadius: 10, backgroundColor: "#10B981" },
  btnReject:   { flex: 1, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 6, padding: 12, borderRadius: 10, backgroundColor: "#EF4444" },
