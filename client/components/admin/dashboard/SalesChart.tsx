...
export function SalesChart() {
  ...
  const yLabels = [0, 0.5, 1].map(f => ({ y: PAD_T + chartH * (1 - f), val: maxVal * f }));
  ...
  return (
    <View style={[chart.card, { backgroundColor: bg, borderColor: border }]}>
      ...
      <View style={chart.header}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={[chart.title, { color: text }]}>Ingresos últimos 7 días</Text>
          <Text style={[chart.sub, { color: subTxt }]}>Comisiones ComeYa · total {fmt(total)}</Text>
        </View>
