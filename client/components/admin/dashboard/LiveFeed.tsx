  ...
  const load = async () => {
    try {
      const res = await apiRequest(
        "GET",
        "/api/admin/orders?status=active&limit=12",
      );
    ...
    <View style={[lf.card, { backgroundColor: bg, borderColor: border }]}>
      <View style={lf.header}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={[lf.sub, { color: sub }]}>
            Pedidos activos · actualiza cada 15s
          </Text>
        </View>
        <View
          style={[
            lf.livePill,
            { backgroundColor: pulse ? "#10B98130" : "#10B98115" },
          ]}
        >
        ...
        orders.map((o, i) => {
          ...
          return (
            <View
              ...
              style={[
                lf.row,
                { backgroundColor: rowBg, borderLeftColor: meta.color },
              ]}
            >
              {/* Status icon */}
              <View
                style={[lf.statusIcon, { backgroundColor: meta.color + "20" }]}
              >
              ...
              <View style={{ flex: 1 }}>
                <View style={lf.topRow}>
                  ...
                  >
                    <Text style={[lf.statusTxt, { color: meta.color }]}>
                      {meta.label}
                    </Text>
                  </View>
                </View>
              ...
              {/* Amount + time */}
              <View style={lf.right}>
                <Text style={[lf.amount, { color: text }]}>
                  {fmt(o.total ?? 0)}
                </Text>
                <Text style={[lf.time, { color: sub }]}>
                  {timeAgo(o.createdAt)}
                </Text>
              </View>
            </View>
