  ...
  const DefaultSidebar = (
    <View style={[s.sidebar, { backgroundColor: card, borderRightColor: border }]}>
      ...
      {(user?.role === "business_owner" || user?.role === "admin") && (
        <>
          <Text style={[s.sideTitle, { color: sub, marginTop: 20 }]}>NEGOCIO</Text>
          <Pressable onPress={() => navigation.navigate("BusinessDashboard" as any)} style={s.sideItem}>
            <Feather name="bar-chart-2" size={15} color={sub} />
            <Text style={[s.sideItemText, { color: text }]}>Dashboard</Text>
          </Pressable>
          <Pressable onPress={() => navigation.navigate("BusinessOrders" as any)} style={s.sideItem}>
      ...
      {user?.role === "delivery_driver" && (
        <>
          <Text style={[s.sideTitle, { color: sub, marginTop: 20 }]}>REPARTIDOR</Text>
          <Pressable onPress={() => navigation.navigate("DriverDashboard" as any)} style={s.sideItem}>
