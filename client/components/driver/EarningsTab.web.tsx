...
export function EarningsTab({ mode }: Props) {
  ...
  const last7: { label: string; value: number }[] = Array.from(
    { length: 7 },
    (_, i) => {
      const d = new Date();
      ...
      const total =
        dayDeliveries.reduce(
          (s, dl) => s + (dl.deliveryEarnings ?? dl.deliveryFee ?? 0),
          0,
        ) / 100;
        ...
        >
          ...
          <View style={s.statsGrid}>
            {[
              ...
              },
              {
                icon: "star",
                label: "Calificación",
                value: stats?.rating ? (stats.rating / 10).toFixed(1) : "—",
                color: AMBER,
              },
              {
                icon: "check-circle",
              ...
              >
                <View
                  style={[s.statIcon, { backgroundColor: st.color + "18" }]}
                >
          ...
          >
            ...
            {(
              ...
              <View
                key={row.label}
                style={[s.breakdownRow, { borderBottomColor: border }]}
              >
                <View
                  style={[s.breakdownDot, { backgroundColor: row.color }]}
                />
                <Text style={[s.breakdownLabel, { color: text }]}>
                  {row.label}
                </Text>
          ...
          >
            ...
            <View style={{ flex: 1, marginLeft: 10 }}>
              <Text style={[s.infoTitle, { color: text }]}>
                ¿Cómo recibes tus pagos?
              </Text>
              <Text style={[s.infoBody, { color: sub }]}>
                Cuando el cliente confirme la entrega, tus fondos se liberan
                automáticamente. ComeYa te transfiere vía Bizum o IBAN.
                Configura tus cuentas en Mi Perfil → Cuentas de pago.
              </Text>
            </View>
          </View>
        </ScrollView>
    ...
    <View style={[s.root, { backgroundColor: bg }]}>
      ...
      >
        {[
          {
            label: "Total",
            value: fmtEur(data?.stats?.totalEarnings ?? 0),
            color: GREEN,
          },
          {
            label: "Este mes",
            value: fmtEur(data?.stats?.monthEarnings ?? 0),
            color: BLUE,
          },
          {
            label: "Esta semana",
            value: fmtEur(data?.stats?.weekEarnings ?? 0),
            color: AMBER,
          },
          {
            label: "Entregas",
        ...
        {deliveries.length === 0 ? (
          ...
          deliveries.map((dl, i) => {
            ...
            return (
              ...
              >
                <View
                  style={[s.historyIcon, { backgroundColor: GREEN + "18" }]}
                >
                  <Feather name="check-circle" size={18} color={GREEN} />
                </View>
                <View style={{ flex: 1, marginLeft: 12 }}>
                  <Text
                    style={[s.historyBusiness, { color: text }]}
                    numberOfLines={1}
                  >
                    {dl.businessName}
                  </Text>
