...
// ── Campo de formulario ───────────────────────────────────────────────────────
function Field({ label, value, onChange, placeholder, type = "text", hint, required, disabled, card, border, sub, text, inputBg }: any) {
  return (
    <View style={f.wrap}>
      <Text style={[f.label, { color: sub }]}>{label}{required && <Text style={{ color: RED }}> *</Text>}</Text>
      <TextInput
        style={[f.input, { backgroundColor: disabled ? card : inputBg, color: text, borderColor: border, opacity: disabled ? 0.6 : 1 }] as any}
        value={value}
...
if (pd.success) {
  ...
  const handleFileChange = async (e: any) => {
    ...
    try {
      ...
      const r = await apiRequest("POST", "/api/users/profile-image", { image: b64 });
  ...
  const handleSaveProfile = async () => {
    if (!name.trim()) { flash(false, "El nombre es obligatorio"); return; }
    if (!dni.trim())  { flash(false, "El DNI/NIE es obligatorio para operar en España"); return; }
    setSaving(true);
    try {
  ...
  const handleChangePwd = async () => {
    if (!curPwd || !newPwd || !confPwd) { flash(false, "Rellena todos los campos de contraseña"); return; }
    if (newPwd.length < 8) { flash(false, "La nueva contraseña debe tener al menos 8 caracteres"); return; }
    if (newPwd !== confPwd) { flash(false, "Las contraseñas no coinciden"); return; }
    setSavingPwd(true);
    try {
      ...
      const data = await res.json();
      if (!data.success) throw new Error(data.error ?? "Contraseña actual incorrecta");
  ...
  const verColor = verStatus === "verified" ? GREEN : verStatus === "rejected" ? RED : AMBER;
  const verLabel = verStatus === "verified" ? "Cuenta verificada" : verStatus === "rejected" ? "Verificación rechazada" : "Pendiente de verificación";
  const verIcon  = verStatus === "verified" ? "check-circle" : verStatus === "rejected" ? "x-circle" : "clock";
  ...
  return (
    <View style={[s.root, { backgroundColor: bg }]}>
      ...
      <View style={[s.header, { backgroundColor: card, borderBottomColor: border }]}>
        ...
        <Text style={[s.subtitle, { color: sub }]}>Datos personales y configuración de cuenta</Text>
      </View>
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={[s.content, { backgroundColor: bg }]} showsVerticalScrollIndicator={false}>
        ...
        {/* ── Avatar + estado ── */}
        <View style={[s.avatarCard, { backgroundColor: card, borderColor: border }]}>
          <TouchableOpacity onPress={handlePickPhoto} disabled={uploadingImg} style={s.avatarWrap}>
            {profileImg ? (
              <img src={profileImg} style={{ width: 88, height: 88, borderRadius: 44, objectFit: "cover" } as any} alt="perfil" />
            ) : (
              <View style={[s.avatarPlaceholder, { backgroundColor: GREEN + "20" }]}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={[s.avatarName, { color: text }]}>{user?.name ?? "Repartidor"}</Text>
            <Text style={[s.avatarPhone, { color: sub }]}>{user?.phone ?? ""}</Text>
          ...
          {/* Normativa española */}
          <View style={[s.normBox, { backgroundColor: BLUE + "10", borderColor: BLUE + "30" }]}>
            <Feather name="info" size={14} color={BLUE} />
            <Text style={[s.normTxt, { color: text }]}>
              <Text style={{ fontWeight: "700" }}>Normativa España · Soria{"\n"}</Text>
              Para operar como repartidor autónomo en Soria debes estar dado de alta en el RETA (Régimen Especial de Trabajadores Autónomos), tener el DNI/NIE en vigor y el vehículo con ITV y seguro al día. ComeYa verifica estos datos antes de activar tu cuenta.
            </Text>
          </View>
        </View>
        ...
        <View style={[s.card, { backgroundColor: card, borderColor: border }]}>
          <Section title="Datos personales" icon="user" color={GREEN}>
            <Field label="Nombre completo" value={name} onChange={setName} placeholder="Nombre y apellidos" required
            ...
            <Field label="DNI / NIE" value={dni} onChange={(t: string) => setDni(t.toUpperCase())} placeholder="12345678A"
              required hint="Obligatorio para operar como autónomo en España"
              card={card} border={border} sub={sub} text={text} inputBg={inputBg} />
            <Field label="Teléfono" value={phone} onChange={setPhone} placeholder="+34 6XX XXX XXX" type="phone" required
              card={card} border={border} sub={sub} text={text} inputBg={inputBg} />
...
<Field label="Dirección en Soria" value={address} onChange={setAddress} placeholder="Calle Mayor 12, 42001 Soria"
            ...
            >
              ...
              <Text style={s.saveBtnTxt}>{saving ? "Guardando..." : "Guardar datos personales"}</Text>
            </TouchableOpacity>
          </Section>
        </View>
...
{/* ── Cambiar contraseña ── */}
          ...
          <Section title="Cambiar contraseña" icon="lock" color={PURPLE}>
            <View style={[s.pwdInfo, { backgroundColor: AMBER + "12", borderColor: AMBER + "30" }]}>
              ...
              <Text style={[s.pwdInfoTxt, { color: text }]}>
                Usa una contraseña de al menos 8 caracteres con letras y números. No la compartas con nadie.
              </Text>
            </View>
            <Field label="Contraseña actual" value={curPwd} onChange={setCurPwd} placeholder="••••••••" type="password"
              card={card} border={border} sub={sub} text={text} inputBg={inputBg} />
            <Field label="Nueva contraseña" value={newPwd} onChange={setNewPwd} placeholder="Mínimo 8 caracteres" type="password"
              card={card} border={border} sub={sub} text={text} inputBg={inputBg} />
            <Field label="Confirmar nueva contraseña" value={confPwd} onChange={setConfPwd} placeholder="Repite la contraseña" type="password"
              card={card} border={border} sub={sub} text={text} inputBg={inputBg} />
            ...
            {newPwd.length > 0 && (
              <View style={s.strengthRow}>
                {[1, 2, 3, 4].map(i => {
                  ...
                  const color = strength >= 3 ? GREEN : strength >= 2 ? AMBER : RED;
                  return <View key={i} style={[s.strengthBar, { backgroundColor: i <= strength ? color : border }]} />;
                })}
                <Text style={[s.strengthTxt, { color: sub }]}>
                  {newPwd.length < 6 ? "Débil" : newPwd.length < 9 ? "Media" : newPwd.length < 12 ? "Fuerte" : "Muy fuerte"}
                </Text>
              </View>
            )}
            ...
            <TouchableOpacity
              ...
              disabled={savingPwd}
              style={[s.saveBtn, { backgroundColor: PURPLE, opacity: savingPwd ? 0.7 : 1 }]}
            >
              {savingPwd
                ? <ActivityIndicator size="small" color="#fff" />
                : <Feather name="lock" size={15} color="#fff" />
              }
              <Text style={s.saveBtnTxt}>{savingPwd ? "Cambiando..." : "Cambiar contraseña"}</Text>
            </TouchableOpacity>
          </Section>
        </View>
        ...
        <View style={[s.card, { backgroundColor: card, borderColor: border }]}>
          <Section title="Normativa repartidor autónomo · Soria" icon="file-text" color={BLUE}>
            {[
              { icon: "user-check", color: GREEN,  title: "Alta en RETA",           body: "Debes estar dado de alta como autónomo en la Seguridad Social (RETA) para poder operar legalmente como repartidor en España." },
              { icon: "credit-card", color: BLUE,  title: "DNI/NIE en vigor",       body: "Tu documento de identidad debe estar vigente. ComeYa lo verifica antes de activar tu cuenta." },
              { icon: "truck",       color: AMBER, title: "Vehículo homologado",    body: "El vehículo debe tener ITV en vigor, seguro de responsabilidad civil y, si es moto, el permiso de conducir correspondiente (A1, A2 o A)." },
              { icon: "shield",      color: PURPLE, title: "Seguro de accidentes",  body: "Se recomienda contratar un seguro de accidentes personal. ComeYa no cubre accidentes laborales al ser autónomo." },
              { icon: "file-text",   color: RED,   title: "Facturación",            body: "Como autónomo debes emitir factura a ComeYa por tus servicios. Puedes usar el modelo simplificado si facturas menos de 3.000€/año a un mismo cliente." },
              { icon: "map-pin",     color: GREEN, title: "Zona de operación",      body: "ComeYa opera en Soria capital y municipios del área metropolitana. Las entregas fuera de zona deben ser acordadas previamente." },
            ].map((item, i) => (
              <View key={i} style={[s.normItem, { borderBottomColor: border, borderBottomWidth: i < 5 ? 1 : 0 }]}>
                <View style={[s.normIcon, { backgroundColor: item.color + "15" }]}>
                  <Feather name={item.icon as any} size={16} color={item.color} />
                </View>
                <View style={{ flex: 1 }}>
                  <Text style={[s.normTitle, { color: text }]}>{item.title}</Text>
                  <Text style={[s.normBody, { color: sub }]}>{item.body}</Text>
                </View>
              </View>
            ))}

            <View style={[s.legalNote, { backgroundColor: BLUE + "10", borderColor: BLUE + "30" }]}>
              ...
              <Text style={[s.legalNoteTxt, { color: sub }]}>
                ...
                <Text style={{ color: BLUE, fontWeight: "700" }}>Seguridad Social</Text> o el{" "}
                <Text style={{ color: BLUE, fontWeight: "700" }}>Ayuntamiento de Soria</Text>.
                ComeYa no es responsable de las obligaciones fiscales del autónomo.
              </Text>
            </View>
          </Section>
        </View>
        ...
        {/* ── Zona de peligro ── */}
        <View style={[s.card, { backgroundColor: card, borderColor: RED + "30", borderWidth: 1 }]}>
          <Section title="Zona de peligro" icon="alert-triangle" color={RED}>
            <Text style={[s.dangerTxt, { color: sub }]}>
              Si deseas eliminar tu cuenta o tienes problemas con tu verificación, contacta con soporte. La eliminación de cuenta es irreversible y borrará todos tus datos y historial de entregas.
            </Text>
            <TouchableOpacity
              style={[s.dangerBtn, { borderColor: RED + "40" }]}
              onPress={() => flash(false, "Contacta con soporte en support@comeya.es para eliminar tu cuenta.")}
            >
              <Feather name="trash-2" size={14} color={RED} />
              <Text style={[s.dangerBtnTxt, { color: RED }]}>Solicitar eliminación de cuenta</Text>
            </TouchableOpacity>
          </Section>
        </View>
...
const s = StyleSheet.create({
  ...
  avatarCard:      { borderRadius: 14, borderWidth: 1, padding: 20, marginBottom: 16, gap: 14 },
  ...
  cameraBadge:     { position: "absolute" as any, bottom: 0, right: 0, width: 26, height: 26, borderRadius: 13, justifyContent: "center", alignItems: "center", borderWidth: 2, borderColor: "#fff" },
  avatarName:      { fontSize: 17, fontWeight: "800" },
  avatarPhone:     { fontSize: 13, marginTop: 2, marginBottom: 8 },
  verBadge:        { flexDirection: "row", alignItems: "center", gap: 5, paddingHorizontal: 10, paddingVertical: 4, borderRadius: 20, alignSelf: "flex-start" },
  verTxt:          { fontSize: 11, fontWeight: "700" },
  normBox:         { borderRadius: 10, borderWidth: 1, padding: 12, flexDirection: "row", gap: 10, alignItems: "flex-start" },
  normTxt:         { fontSize: 12, lineHeight: 18, flex: 1 },
  card:            { borderRadius: 14, borderWidth: 1, padding: 20, marginBottom: 16 },
  saveBtn:         { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8, paddingVertical: 13, borderRadius: 10, marginTop: 6 },
  saveBtnTxt:      { fontSize: 14, fontWeight: "700", color: "#fff" },
  pwdInfo:         { flexDirection: "row", alignItems: "flex-start", gap: 8, borderRadius: 8, borderWidth: 1, padding: 10, marginBottom: 12 },
  pwdInfoTxt:      { fontSize: 12, lineHeight: 17, flex: 1 },
  strengthRow:     { flexDirection: "row", alignItems: "center", gap: 4, marginBottom: 12 },
  strengthBar:     { flex: 1, height: 4, borderRadius: 2 },
  strengthTxt:     { fontSize: 11, marginLeft: 4 },
  normItem:        { flexDirection: "row", alignItems: "flex-start", gap: 12, paddingVertical: 12 },
  normIcon:        { width: 34, height: 34, borderRadius: 8, justifyContent: "center", alignItems: "center", flexShrink: 0 },
