...
export function AvailableOrdersTab({ isOnline, onToggleOnline, togglingOnline, showToast }: Props) {
  ...
  const handleAccept = async (orderId: string) => {
    if (!isOnline) { flash(false, "Debes estar en línea para aceptar pedidos"); return; }
    setAccepting(orderId);
    try {
      const res  = await apiRequest("POST", `/api/delivery/accept/${orderId}`, {});
      const data = await res.json();
      if (data.success) {
    ...
    } catch { flash(false, "Error al aceptar el pedido"); }
  ...
  const avgDist = orders.length
    ? (orders.reduce((s, o) => s + (o.distanceKm ?? 0), 0) / orders.length).toFixed(1)
    : "—";
    ...
    <View style={[s.root, { backgroundColor: bg }]}>
      ...
      <View style={[s.header, { backgroundColor: card, borderBottomColor: border }]}>
        ...
        <View style={[s.toggleWrap, {
          ...
          <View style={[s.pulse, { backgroundColor: isOnline ? GREEN : "#9ca3af" }]} />
          <Text style={[s.toggleTxt, { color: isOnline ? GREEN : sub }]}>
            {togglingOnline ? "Cambiando..." : isOnline ? "En línea" : "Desconectado"}
          </Text>
          <Switch
            value={isOnline}
      ...
      <View style={[s.kpiStrip, { backgroundColor: card, borderBottomColor: border }]}>
        {[
          { label: "Disponibles",    value: orders.length,      color: BLUE,   icon: "package"     },
          { label: "Ganancia total", value: fmtEur(totalEarnings), color: GREEN,  icon: "trending-up" },
          { label: "Dist. media",    value: `${avgDist} km`,    color: PURPLE, icon: "map-pin"     },
          { label: "Estado",         value: isOnline ? "Activo" : "Inactivo", color: isOnline ? GREEN : "#9ca3af", icon: "radio" },
        ].map(k => (
      ...
      {msg && (
        <View style={[s.msgBar, { backgroundColor: msg.ok ? "#16A34A15" : "#EF444415" }]}>
          <Feather name={msg.ok ? "check-circle" : "alert-circle"} size={14} color={msg.ok ? GREEN : "#EF4444"} />
          <Text style={[s.msgTxt, { color: msg.ok ? GREEN : "#EF4444" }]}>{msg.text}</Text>
        </View>
      )}

      {/* ── Offline banner ── */}
      {!isOnline && (
        <View style={[s.offlineBanner, { backgroundColor: AMBER + "15", borderColor: AMBER + "40" }]}>
      ...
      {loading ? (
        <View style={s.center}>
          ...
          <Text style={[s.loadingTxt, { color: sub }]}>Buscando pedidos...</Text>
        </View>
      ) : (
        <ScrollView
          style={{ flex: 1 }}
        ...
        >
          {orders.length === 0 ? (
            <View style={s.empty}>
              ...
              <Text style={[s.emptyTitle, { color: text }]}>Sin pedidos disponibles</Text>
            ...
            orders.map(order => {
              ...
              const distKm   = order.distanceKm ? (order.distanceKm / 1000).toFixed(1) : null;
              const eta      = order.estimatedMinutes ?? (distKm ? Math.round(parseFloat(distKm) * 3) : null);

              return (
                <View key={order.id} style={[s.card, { backgroundColor: card, borderColor: border }]}>

                  {/* Card header */}
                  <View style={s.cardHeader}>
                    <View style={[s.businessBadge, { backgroundColor: BLUE + "15" }]}>
                  ...
                  {/* Route */}
                  <View style={[s.routeBox, { backgroundColor: isDark ? "#222" : "#f8f9fa", borderColor: border }]}>
                    <View style={s.routeRow}>
                      <View style={[s.routeDot, { backgroundColor: AMBER }]} />
                      <View style={{ flex: 1 }}>
                        <Text style={[s.routeLabel, { color: sub }]}>RECOGIDA</Text>
                        <Text style={[s.routeAddr, { color: text }]} numberOfLines={1}>
                    ...
                    <View style={s.routeRow}>
                      ...
                      <View style={{ flex: 1 }}>
                        <Text style={[s.routeLabel, { color: sub }]}>ENTREGA</Text>
                        <Text style={[s.routeAddr, { color: text }]} numberOfLines={1}>
                  ...
                  <View style={s.chips}>
                    {distKm && (
                      <View style={[s.chip, { backgroundColor: chipBg }]}>
                        ...
                        <Text style={[s.chipTxt, { color: sub }]}>{distKm} km</Text>
                      </View>
                    )}
                    {eta && (
                      <View style={[s.chip, { backgroundColor: chipBg }]}>
                        <Feather name="clock" size={11} color={sub} />
                        <Text style={[s.chipTxt, { color: sub }]}>~{eta} min</Text>
                      </View>
                    )}
                    <View style={[s.chip, { backgroundColor: chipBg }]}>
                      <Feather name="package" size={11} color={sub} />
                      <Text style={[s.chipTxt, { color: sub }]}>{items.length} prod.</Text>
                    </View>
                    <View style={[s.chip, { backgroundColor: chipBg }]}>
                      <Feather name={order.paymentMethod === "cash" ? "dollar-sign" : "credit-card"} size={11} color={sub} />
                  ...
                  {items.length > 0 && (
                    <View style={[s.itemsPreview, { borderTopColor: border }]}>
                      <Text style={[s.itemsTitle, { color: sub }]}>PRODUCTOS</Text>
                      ...
                      {items.length > 3 && (
                        <Text style={[s.itemMore, { color: sub }]}>+{items.length - 3} más</Text>
                      )}
                    </View>
                  )}
                  ...
                  <View style={[s.cardFooter, { borderTopColor: border }]}>
                    <View>
                      <Text style={[s.earningLabel, { color: sub }]}>Tu ganancia</Text>
                      <Text style={[s.earningVal, { color: GREEN }]}>{fmtEur(order.deliveryFee ?? 0)}</Text>
                    </View>
                    <View style={s.footerRight}>
                      <View style={s.totalWrap}>
                        <Text style={[s.totalLabel, { color: sub }]}>Total pedido</Text>
                        <Text style={[s.totalVal, { color: text }]}>{fmtEur(order.total ?? 0)}</Text>
                      </View>
                      <TouchableOpacity
...
const s = StyleSheet.create({
  ...
  toggleWrap:    { flexDirection: "row", alignItems: "center", gap: 8, paddingHorizontal: 14, paddingVertical: 8, borderRadius: 24, borderWidth: 1 },
  ...
  offlineBanner: { flexDirection: "row", alignItems: "center", gap: 10, marginHorizontal: 20, marginTop: 12, padding: 12, borderRadius: 10, borderWidth: 1 },
  ...
  businessBadge: { width: 38, height: 38, borderRadius: 10, justifyContent: "center", alignItems: "center" },
  ...
  routeBox:      { marginHorizontal: 16, marginBottom: 12, borderRadius: 10, borderWidth: 1, padding: 12, gap: 6 },
  routeRow:      { flexDirection: "row", alignItems: "flex-start", gap: 10 },
  routeDot:      { width: 10, height: 10, borderRadius: 5, marginTop: 3, flexShrink: 0 },
  ...
  chips:         { flexDirection: "row", flexWrap: "wrap", gap: 6, paddingHorizontal: 16, marginBottom: 12 },
  ...
  itemsPreview:  { paddingHorizontal: 16, paddingTop: 10, paddingBottom: 12, borderTopWidth: 1, gap: 3 },
  itemsTitle:    { fontSize: 9, fontWeight: "700", letterSpacing: 0.5, marginBottom: 4 },
  itemRow:       { fontSize: 12 },
  ...
  acceptBtn:     { flexDirection: "row", alignItems: "center", gap: 8, paddingHorizontal: 20, paddingVertical: 12, borderRadius: 10 },
