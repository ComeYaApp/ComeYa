    ...
    <View style={[s.sidebar, { backgroundColor: card, borderRightColor: border }]}>
      ...
      <View style={s.header}>
        <Image
          ...
          contentFit="contain"
        />
        <Pressable onPress={() => setShowPicker(v => !v)} style={[s.bizSelector, { borderColor: showPicker ? PRIMARY : border, backgroundColor: showPicker ? PRIMARY + "08" : "transparent" }]}>
          <Feather name="briefcase" size={13} color={showPicker ? PRIMARY : sub} />
          <Text style={[s.bizName, { color: text }]} numberOfLines={1}>
            {selectedBusiness?.name || "Mi Negocio"}
          </Text>
          <Feather name={showPicker ? "chevron-up" : "chevron-down"} size={14} color={showPicker ? PRIMARY : sub} />
        </Pressable>
      ...
      {showPicker && (
        <View style={[s.picker, { backgroundColor: dropdownBg, borderColor: dropdownBorder }]}>
          {businesses.length === 0 && (
            <Text style={[s.pickerEmpty, { color: sub }]}>Sin negocios registrados</Text>
          )}
          {businesses.map((biz: any) => (
            <Pressable
              key={biz.id}
              onPress={() => { selectBusiness(biz); setShowPicker(false); }}
              style={[s.pickerItem, selectedBusiness?.id === biz.id && { backgroundColor: PRIMARY + "15" }]}
            >
              <Feather name="briefcase" size={13} color={selectedBusiness?.id === biz.id ? PRIMARY : sub} />
              <Text style={[s.pickerText, { color: selectedBusiness?.id === biz.id ? PRIMARY : text }]} numberOfLines={1}>
                {biz.name}
              </Text>
              {selectedBusiness?.id === biz.id && <Feather name="check" size={13} color={PRIMARY} />}
            </Pressable>
          ))}
          <View style={[s.pickerDivider, { backgroundColor: dropdownBorder }]} />
          <Pressable
            onPress={() => { setShowPicker(false); navigation.navigate("MyBusinesses" as any); }}
            style={s.pickerItem}
          >
            <Feather name="settings" size={13} color={sub} />
            <Text style={[s.pickerText, { color: text }]}>Gestionar negocios</Text>
          </Pressable>
          <Pressable
            onPress={() => { setShowPicker(false); navigation.navigate("MyBusinesses" as any, { openAddModal: true }); }}
            style={s.pickerItem}
          >
            <Feather name="plus-circle" size={13} color={PRIMARY} />
            <Text style={[s.pickerText, { color: PRIMARY }]}>Añadir negocio</Text>
          </Pressable>
        </View>
      )}
    </View>
...
const s = StyleSheet.create({
  sidebar:      { width: 240, borderRightWidth: 1, flexDirection: "column" as any, position: "relative" as any },
  header:       { padding: 24, paddingBottom: 16 },
  logo:         { width: 100, height: 32, marginBottom: 12 },
  bizSelector:  { flexDirection: "row", alignItems: "center", gap: 6, marginBottom: 4, paddingHorizontal: 10, paddingVertical: 7, borderRadius: 8, borderWidth: 1 },
  ...
  overlay:      { position: "absolute" as any, top: 0, left: 0, right: 0, bottom: 0, zIndex: 998 },
  picker:       {
  ...
  pickerEmpty:   { fontSize: 12, paddingHorizontal: 14, paddingVertical: 10, fontStyle: "italic" },
  pickerDivider: { height: 1, marginVertical: 4, marginHorizontal: 10 },
  pickerItem:    { flexDirection: "row", alignItems: "center", gap: 8, paddingHorizontal: 14, paddingVertical: 10, borderRadius: 8, marginHorizontal: 4 },
