...
function SectionHeader({ icon, title }: { icon: any; title: string }) {
  ...
  return (
    <View style={sh.row}>
      <View
        style={[sh.iconWrap, { backgroundColor: ComeYaColors.primary + "18" }]}
      >
...
export default function AdminDashboardScreen() {
    ...
    try {
      const [metricsRes, salesRes, earningsRes, payoutsRes] = await Promise.all(
        [
          ...
          apiRequest(
            "GET",
            "/api/admin/finance/platform-earnings?period=month",
          ),
      ...
      const pendingPayoutAmount =
        p?.payouts?.reduce((acc: number, x: any) => acc + (x.amount ?? 0), 0) ??
        0;
      setFinance({
  ...
  return (
    <ScrollView
      style={{
        flex: 1,
        backgroundColor: theme.backgroundRoot ?? theme.background,
      }}
    ...
    >
      ...
      <View style={s.header}>
        ...
        <View
          style={[s.liveBadge, { backgroundColor: ComeYaColors.error + "18" }]}
        >
          ...
          {Platform.OS !== "web" ? (
            <LineChart
              data={{
                ...
                datasets: [
                  {
                    data: salesChart.chartData.map((d: any) => d.amount / 100),
                  },
                ],
              }}
              width={chartWidth}
      ...
      <View style={[s.row2, { gap, marginTop: gap }]}>
        ...
        <View style={[s.card, s.flex1, { backgroundColor: theme.card }]}>
          ...
          <StatRow
            label="En camino"
            value={metrics?.onTheWayOrders ?? 0}
            color={ComeYaColors.success}
            icon="truck"
          />
          <StatRow
        ...
        <View style={[s.card, s.flex1, { backgroundColor: theme.card }]}>
          ...
          <StatRow
            ...
            value={fmt(finance?.weekRevenue ?? metrics?.weekRevenue ?? 0)}
            color={ComeYaColors.success}
            icon="trending-up"
          />
          <StatRow
            label="Ingresos mes"
            value={fmt(finance?.monthRevenue ?? metrics?.monthRevenue ?? 0)}
            color={ComeYaColors.primary}
            icon="bar-chart-2"
          />
          <StatRow
            label="Payouts pendientes"
            value={finance?.pendingPayouts ?? metrics?.pendingPayouts ?? 0}
            color={ComeYaColors.warning}
            icon="send"
          />
          <StatRow
            label="Monto a pagar"
            value={fmt(finance?.pendingPayoutAmount ?? 0)}
            color={ComeYaColors.warning}
            icon="credit-card"
          />
          <StatRow
            label="Fraudes detectados"
            value={finance?.fraudCount ?? metrics?.fraudCount ?? 0}
            color={ComeYaColors.error}
            icon="shield"
          />
        </View>
      ...
      <View style={[s.card, { backgroundColor: theme.card, marginTop: gap }]}>
        ...
        <View style={[s.row2, { gap: 8, marginTop: 8 }]}>
          ...
          >
            ...
            <Text style={[s.miniLabel, { color: theme.textSecondary }]}>
              Ticket promedio
            </Text>
          </View>
          <View
          ...
          <View
            style={[
              s.flex1,
              s.miniStat,
              { backgroundColor: ComeYaColors.warning + "12" },
            ]}
          >
          ...
          <View
            style={[s.flex1, s.miniStat, { backgroundColor: "#6C5CE7" + "12" }]}
          >
            <Text style={[s.miniVal, { color: "#6C5CE7" }]}>
              {metrics?.avgRating
                ? `${(metrics.avgRating / 100).toFixed(1)}★`
                : "—"}
            </Text>
            <Text style={[s.miniLabel, { color: theme.textSecondary }]}>
              Rating promedio
