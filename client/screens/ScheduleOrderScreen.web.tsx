...
export default function ScheduleOrderScreen() {
  ...
  const scheduleMutation = useMutation({
    ...
    onSuccess: () => {
      showToast(
        `Pedido programado para ${new Date(scheduledDate).toLocaleDateString("es-ES")}`,
        "success",
      );
  ...
  const handleSchedule = () => {
    if (isRecurring) {
      if (recurringDays.length === 0) {
        showToast("Selecciona al menos un día", "warning");
        return;
      }
      ...
      >
        <View style={[s.sideHeader, { borderBottomColor: border }]}>
          ...
          <View
            style={[
              s.typeBadge,
              {
                backgroundColor: isRecurring ? "#8B5CF620" : PRIMARY + "20",
                borderColor: isRecurring ? "#8B5CF640" : PRIMARY + "40",
              },
            ]}
          >
            <Feather
              ...
              size={13}
              color={isRecurring ? "#8B5CF6" : PRIMARY}
            />
            <Text
              style={{
                color: isRecurring ? "#8B5CF6" : PRIMARY,
        ...
        <View style={s.sideNav}>
          <Pressable
            ...
            style={[s.navItem, !isRecurring && s.navItemActive]}
          >
            <Feather
              name="calendar"
              size={18}
              color={!isRecurring ? PRIMARY : sub}
            />
            <Text
              style={[s.navItemText, { color: !isRecurring ? PRIMARY : text }]}
            >
              Una vez
            </Text>
          </Pressable>
      ...
      >
        ...
        <View style={[s.card, { backgroundColor: card, borderColor: border }]}>
          ...
          <View style={s.toggleRow}>
            {[
              { id: false, label: "Una vez", icon: "calendar" },
              { id: true, label: "Recurrente", icon: "repeat" },
            ].map((opt) => (
              <Pressable
                key={String(opt.id)}
                onPress={() => setIsRecurring(opt.id)}
                style={[
                  s.toggleBtn,
                  {
                    backgroundColor: isRecurring === opt.id ? PRIMARY : cardBg,
                    borderColor: isRecurring === opt.id ? PRIMARY : border,
                  },
                ]}
              >
                <Feather
                  ...
                  size={16}
                  color={isRecurring === opt.id ? "#fff" : sub}
                />
                <Text
                  style={[
                    s.toggleBtnText,
                    { color: isRecurring === opt.id ? "#fff" : text },
                  ]}
                >
                  {opt.label}
                </Text>
                    ...
                    style={[
                      s.dayBtn,
                      {
                        backgroundColor: recurringDays.includes(day.id)
                          ? PRIMARY
                          : cardBg,
                        borderColor: recurringDays.includes(day.id)
                          ? PRIMARY
                          : border,
                      },
                    ]}
                  >
                    <Text
                      style={[
                        s.dayBtnText,
                        {
                          color: recurringDays.includes(day.id) ? "#fff" : text,
                        },
                      ]}
                    >
                      {day.label}
        ...
        <Pressable
          ...
          style={[
            s.ctaBtn,
            { backgroundColor: PRIMARY, opacity: isPending ? 0.6 : 1 },
          ]}
        >
          {isPending ? (
