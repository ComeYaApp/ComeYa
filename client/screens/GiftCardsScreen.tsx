  ...
  const handlePickProof = async () => {
    ...
    try {
      if (Platform.OS === "web") {
        ...
        if (status !== "granted") {
          showToast("Permiso de galería requerido", "error");
          return;
        }
          ...
          if (Platform.OS === "web") {
            ...
            if (result.success) {
              const confirmRes = await apiRequest(
                "POST",
                `/api/gift-cards/${data.giftCard.id}/stripe-success`,
                {},
              );
              ...
              if (confirmData.success) {
                ...
                showToast(
                  "Gift Card pagada y activada correctamente",
                  "success",
                );
              } else {
                showToast(
                  confirmData.error ||
                    "Pago realizado, pero no se pudo activar la gift card",
                  "warning",
                );
              }
            } else if (result.error !== "Pago cancelado") {
              showToast(result.error || "No se pudo procesar el pago", "error");
            }
        ...
        {activeTab === "buy" ? (
          <View>
            ...
            <View
              style={[
                styles.section,
                { backgroundColor: "#3B82F615", flexDirection: "row", gap: 8 },
              ]}
            >
                ...
                {PRESET_AMOUNTS.map((preset) => (
                  ...
                  >
                    <ThemedText
                      ...
                      style={{
                        color:
                          amount === preset.toString() ? "#fff" : theme.text,
                        fontWeight: "600",
                      }}
                    >
                      {preset} €
              ...
              return (
                <View
                  ...
                  style={[
                    ...
                    {
                      backgroundColor: theme.card,
                      borderWidth: card.status === "pending_payment" ? 1.5 : 0,
                      borderColor: "#F59E0B",
                    },
                    Shadows.sm,
                  ]}
                >
                  ...
                  {card.status === "pending_payment" && (
                    ...
                    >
                      <Pressable
                        onPress={() =>
                          setProofCardId(isProofOpen ? null : card.id)
                        }
                        style={{
                          flexDirection: "row",
                          alignItems: "center",
                      ...
                      {isProofOpen && (
                        <View style={{ marginTop: 10, gap: 8 }}>
                            ...
                            {PROVIDERS.map((p) => (
                              ...
                              >
                                <ThemedText
                                  ...
                                  style={{
                                    color:
                                      proofProvider === p ? "#fff" : theme.text,
                                    fontWeight: "600",
                                  }}
                                >
                                  {p}
                          ...
                          <TextInput
                            ...
                            placeholder="Referencia / nº operación"
                            placeholderTextColor={theme.textSecondary}
                            style={[
                              styles.input,
                              {
                          ...
                          <Pressable
                            onPress={() => proofMutation.mutate(card.id)}
                            disabled={
                              !proofUrl.trim() || proofMutation.isPending
                            }
                            style={[
                              styles.purchaseButton,
                              {
                                backgroundColor: "#F59E0B",
                                opacity: proofUrl.trim() ? 1 : 0.5,
                              },
                            ]}
                          >
