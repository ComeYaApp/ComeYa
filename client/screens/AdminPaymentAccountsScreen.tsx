...
const PROVIDERS = [
  {
    ...
    fields: [
      { key: 'phone', label: 'Numero de telefono', placeholder: '+34 600 000 000', keyboard: 'phone-pad' as const },
      { key: 'name',  label: 'Nombre del titular', placeholder: 'ComeYa SL',       keyboard: 'default' as const },
    ],
  },
  {
    key: 'transferencia',
    ...
    fields: [
      { key: 'iban',    label: 'IBAN',             placeholder: 'ES00 0000 0000 0000 0000 0000', keyboard: 'default' as const },
      { key: 'titular', label: 'Titular',           placeholder: 'ComeYa SL',                    keyboard: 'default' as const },
      { key: 'banco',   label: 'Nombre del banco',  placeholder: 'Banco Santander',               keyboard: 'default' as const },
    ],
  },
  {
    key: 'paypal',
...
export default function AdminPaymentAccountsScreen() {
  ...
  const loadAccounts = async () => {
    try {
      const res = await apiRequest('GET', '/api/payment-accounts/admin/receiving-accounts');
  ...
  const save = async (provider: string) => {
    setSaving(provider);
    try {
      const res = await apiRequest('PUT', `/api/payment-accounts/admin/receiving-accounts/${provider}`, {
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.backgroundRoot }]}>
      <View style={[styles.header, { paddingTop: insets.top + Spacing.md, backgroundColor: theme.card, borderBottomColor: theme.border }]}>
        ...
        <Text style={[styles.headerTitle, { color: theme.text }]}>Cuentas Receptoras</Text>
        <View style={{ width: 40 }} />
      </View>

      <ScrollView contentContainerStyle={{ padding: Spacing.lg, paddingBottom: insets.bottom + 40 }}>
        <View style={[styles.infoBanner, { backgroundColor: ComeYaColors.primary + '15' }]}>
          <Feather name="info" size={18} color={ComeYaColors.primary} />
          <Text style={[styles.infoText, { color: ComeYaColors.primary }]}>
            Configura las cuentas donde los clientes enviaran sus pagos manuales (Bizum y Transferencia).
          </Text>
        </View>

        {PROVIDERS.map(provider => (
          <View key={provider.key} style={[styles.card, { backgroundColor: theme.card }, Shadows.sm]}>
            <View style={styles.cardHeader}>
              <View style={[styles.iconBox, { backgroundColor: provider.color + '20' }]}>
                <Feather name={provider.icon} size={22} color={provider.color} />
              </View>
              <Text style={[styles.cardTitle, { color: theme.text }]}>{provider.label}</Text>
            </View>
...
const styles = StyleSheet.create({
  ...
  label: { fontSize: 13, fontWeight: '500', marginBottom: 6, marginTop: Spacing.md },
  input: {
