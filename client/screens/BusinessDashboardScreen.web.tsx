...
export default function BusinessDashboardScreen() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      apiRequest(
        "GET",
        `/api/analytics/dashboard/${selectedBusiness.id}?period=${selectedPeriod}`,
      ).then((r) => r.json()),
  ...
  const metrics = stats
    ? [
        ...
        {
          label: "Rating",
          value: `${((stats.averageRating || 0) / 10).toFixed(1)}★`,
          icon: "star",
          color: "#F59E0B",
        },
        {
          ...
          <>
              ...
              {(["today", "week", "month", "all"] as Period[]).map((p) => (
                <Pressable
                  ...
                  style={[
                    s.periodBtn,
                    {
                      backgroundColor: selectedPeriod === p ? PRIMARY : card,
                      borderColor: selectedPeriod === p ? PRIMARY : border,
                    },
                  ]}
                >
                  <Text
                    style={[
                      s.periodBtnText,
                      { color: selectedPeriod === p ? "#fff" : text },
                    ]}
                  >
                    {periodLabels[p]}
                  </Text>
            ...
            >
              <View style={s.toggleLeft}>
                <View
                  style={[
                    s.toggleDot,
                    { backgroundColor: isOpen ? "#4CAF50" : "#EF4444" },
                  ]}
                />
                <View>
            ...
            <View style={s.metricsGrid}>
              {metrics.map((m) => (
                ...
                >
                  <View
                    style={[s.metricIcon, { backgroundColor: m.color + "15" }]}
                  >
                    <Feather name={m.icon as any} size={20} color={m.color} />
                  </View>
                  <Text style={[s.metricValue, { color: text }]}>
                    {m.value}
                  </Text>
            ...
            {topProducts.length > 0 && (
              <>
                ...
                {topProducts.slice(0, 8).map((p, i) => (
                  ...
                  >
                    <View
                      style={[s.rankBadge, { backgroundColor: PRIMARY + "15" }]}
                    >
                      <Text style={[s.rankText, { color: PRIMARY }]}>
                        {i + 1}
                      </Text>
                    </View>
                    <Text
                    ...
                    <Text style={[s.productQty, { color: sub }]}>
                      {p.quantity} uds.
                    </Text>
            ...
            <View style={s.sectionHeader}>
              ...
              <Pressable onPress={() => navigateTo("BusinessOrders")}>
                <Text style={[s.sectionLink, { color: PRIMARY }]}>
                  Ver todos →
                </Text>
              </Pressable>
            </View>
            ...
            >
              {orders.length === 0 ? (
                ...
                orders.map((order) => (
                  <View
                    key={order.id}
                    style={[s.orderRow, { borderBottomColor: border }]}
                  >
                    <View
                      style={[
                        s.orderDot,
                        {
                          backgroundColor: STATUS_COLOR[order.status] || "#999",
                        },
                      ]}
                    />
                    <View style={s.orderInfo}>
                      <Text style={[s.orderCustomer, { color: text }]}>
                    ...
                    <View
                      style={[
                        ...
                        {
                          backgroundColor:
                            (STATUS_COLOR[order.status] || "#999") + "20",
                        },
                      ]}
                    >
                      <Text
            ...
            <View style={s.actionsGrid}>
              {[
                ...
                <Pressable
                  ...
                  onPress={() => navigateTo(a.screen)}
                >
                  <View
                    style={[s.actionIcon, { backgroundColor: a.color + "15" }]}
                  >
                    <Feather name={a.icon as any} size={22} color={a.color} />
                  </View>
                  <Text style={[s.actionLabel, { color: text }]}>
                    {a.label}
                  </Text>
                </Pressable>
              ))}
