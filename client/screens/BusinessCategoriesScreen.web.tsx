...
export default function BusinessCategoriesScreen() {
  ...
  const handleSave = async () => {
    try {
      if (editing)
        await apiRequest("PUT", `/api/business/categories/${editing.id}`, form);
      ...
      >
        ...
        {loading ? (
          ...
          categories.map((cat) => (
            <View
              key={cat.id}
              style={[s.catRow, { backgroundColor: card, borderColor: border }]}
            >
              <View style={{ flex: 1 }}>
                <Text style={[s.catName, { color: text }]}>{cat.name}</Text>
                {cat.description ? (
                  <Text style={[s.catDesc, { color: sub }]}>
                    {cat.description}
                  </Text>
                ) : null}
              </View>
              <Pressable onPress={() => openEdit(cat)} style={s.editBtn}>
                <Feather name="edit-2" size={16} color={ComeYaColors.primary} />
              </Pressable>
              <Switch
                value={cat.isActive}
                onValueChange={() => toggleActive(cat.id, cat.isActive)}
