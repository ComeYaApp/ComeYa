...
const STATUS_LABELS: Record<string, { label: string; color: string; icon: string }> = {
  pending_payment:    { label: 'Pendiente de pago',     color: '#F59E0B', icon: 'clock' },
  pending_verification: { label: 'Verificando pago',    color: '#3B82F6', icon: 'search' },
    ...
    <View style={[s.root, { backgroundColor: bg }]}>
      <MobileSidebarWrapper title="Gift Cards" sidebarStyle={[s.sidebar, { backgroundColor: card, borderRightColor: border }]}>
        <View style={[s.sideHeader, { borderBottomColor: border }]}>
          ...
          <View style={[s.countBadge, { backgroundColor: cardBg }]}>
            <Text style={[s.countText, { color: text }]}>{myCards.length} tarjeta{myCards.length !== 1 ? 's' : ''}</Text>
          </View>
        </View>
      ...
      <ScrollView style={s.main} contentContainerStyle={s.content} showsVerticalScrollIndicator={false}>
        {activeTab === 'buy' ? (
          <>
            {/* Info flujo */}
            <View style={[s.infoBanner, { backgroundColor: '#3B82F615', borderColor: '#3B82F630' }]}>
          ...
          <>
            <Text style={[s.sectionTitle, { color: text }]}>Mis Tarjetas ({myCards.length})</Text>

            {myCards.length === 0 ? (
              <View style={[s.empty, { backgroundColor: card, borderColor: border }]}>
                <Feather name="gift" size={44} color={sub} />
                <Text style={[s.emptyTitle, { color: text }]}>No tienes gift cards aún</Text>
                <Pressable onPress={() => setActiveTab('buy')} style={[s.ctaBtn, { backgroundColor: PRIMARY, marginTop: 8 }]}>
              ...
              myCards.map((c: any) => {
                ...
                return (
                  <View key={c.id} style={[s.giftCardItem, { backgroundColor: card, borderColor: needsProof ? '#F59E0B' : border }]}>
                    <View style={s.gcRow}>
                      <View style={[s.giftCardIcon, { backgroundColor: st.color + '15' }]}>
                    ...
                    {needsProof && (
                      <View style={[s.proofSection, { borderTopColor: border }]}>
                        <Pressable onPress={() => setProofCardId(isProofOpen ? null : c.id)}
                          style={[s.proofToggle, { backgroundColor: '#F59E0B15' }]}>
                          <Feather name="upload" size={15} color="#F59E0B" />
                          <Text style={[s.proofToggleText, { color: '#F59E0B' }]}>
                        ...
                        {isProofOpen && (
                          <View style={{ marginTop: 12, gap: 10 }}>
                            <Text style={[s.proofLabel, { color: sub }]}>Método de pago</Text>
                            <View style={s.providersRow}>
                              {PROVIDERS.map(p => (
                                <Pressable key={p} onPress={() => setProofProvider(p)}
                                  style={[s.providerBtn, { backgroundColor: proofProvider === p ? PRIMARY : cardBg, borderColor: proofProvider === p ? PRIMARY : border }]}>
                                  <Text style={[s.providerBtnText, { color: proofProvider === p ? '#fff' : text }]}>{p}</Text>
                                </Pressable>
                              ))}
                            </View>
                            <TextInput value={proofRef} onChangeText={setProofRef}
                            ...
                            <Pressable onPress={() => proofMutation.mutate(c.id)} disabled={!canProof}
                              ...
                              {proofMutation.isPending
                                ? <ActivityIndicator color="#fff" />
                                : <><Feather name="send" size={16} color="#fff" /><Text style={s.ctaBtnText}>Enviar comprobante</Text></>
                              }
                            </Pressable>
                          </View>
                        )}
...
const s = StyleSheet.create({
  ...
  infoBanner:       { flexDirection: 'row', alignItems: 'flex-start', gap: 10, borderWidth: 1, borderRadius: 12, padding: 14, marginBottom: 16 },
  ...
  giftCardItem:     { borderRadius: 14, borderWidth: 1.5, padding: 16, marginBottom: 12 },
  ...
  statusBadge:      { alignSelf: 'flex-start', paddingHorizontal: 8, paddingVertical: 3, borderRadius: 8, marginBottom: 4 },
  ...
  proofToggle:      { flexDirection: 'row', alignItems: 'center', gap: 8, paddingVertical: 10, paddingHorizontal: 14, borderRadius: 10 },
  proofToggleText:  { fontSize: 13, fontWeight: '700' },
  proofLabel:       { fontSize: 12, fontWeight: '600' },
  providersRow:     { flexDirection: 'row', gap: 8 },
  providerBtn:      { paddingHorizontal: 14, paddingVertical: 8, borderRadius: 8, borderWidth: 1 },
  providerBtnText:  { fontSize: 13, fontWeight: '600', textTransform: 'capitalize' as any },
});
