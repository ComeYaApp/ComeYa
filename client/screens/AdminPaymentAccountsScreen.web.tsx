...
const PROVIDERS = [
  ...
  { key: "transferencia", label: "Transferencia SEPA", color: "#1A56DB", icon: "credit-card" as const, fields: [{ key: "iban", label: "IBAN", placeholder: "ES00 0000 0000 0000 0000 0000" }, { key: "titular", label: "Titular", placeholder: "ComeYa S.L." }, { key: "banco", label: "Banco", placeholder: "Banco Santander" }] },
  { key: "paypal", label: "PayPal", color: "#003087", icon: "dollar-sign" as const, fields: [{ key: "email", label: "Email de PayPal", placeholder: "pagos@comeya.es" }] },
];
...
export default function AdminPaymentAccountsScreen() {
  ...
  useEffect(() => {
    apiRequest("GET", "/api/payment-accounts/admin/receiving-accounts").then(r => r.json()).then(data => {
      if (data.success) {
        const map: Record<string, any> = {};
        data.accounts.forEach((acc: any) => { map[acc.provider] = acc.accountData; });
        setAccounts(map);
      }
  ...
  const save = async (provider: string) => {
    setSaving(provider);
    try {
      const res = await apiRequest("PUT", `/api/payment-accounts/admin/receiving-accounts/${provider}`, { accountData: accounts[provider], isActive: true });
  ...
  return (
    <View style={[s.root, { backgroundColor: bg }]}>
      {!isMobile && <View style={[s.sidebar, { backgroundColor: card, borderRightColor: border }]}>
        ...
        <Text style={[s.sideSub, { color: sub }]}>Configura dónde reciben los pagos manuales de clientes</Text>

        <View style={[s.infoBanner, { backgroundColor: ComeYaColors.primary + "10" }]}>
          <Feather name="info" size={16} color={ComeYaColors.primary} />
          <Text style={[s.infoText, { color: ComeYaColors.primary }]}>
            Los clientes enviarán sus pagos a estas cuentas al hacer pedidos con Bizum o Transferencia.
          </Text>
        </View>

        {PROVIDERS.map(p => (
          <Pressable key={p.key} onPress={() => setActiveProvider(p.key)} style={[s.providerTab, { backgroundColor: activeProvider === p.key ? p.color + "15" : "transparent", borderColor: activeProvider === p.key ? p.color : border }]}>
            <Feather name={p.icon} size={18} color={activeProvider === p.key ? p.color : sub} />
            <Text style={[s.providerTabText, { color: activeProvider === p.key ? p.color : text }]}>{p.label}</Text>
            {accounts[p.key] && <View style={[s.dot, { backgroundColor: ComeYaColors.success }]} />}
          </Pressable>
        ))}
        ...
        {loading ? (
          ...
          <View style={[s.card, { backgroundColor: card, borderColor: border }]}>
            <View style={s.cardHeader}>
              <View style={[s.iconBox, { backgroundColor: activeConfig.color + "20" }]}>
                <Feather name={activeConfig.icon} size={22} color={activeConfig.color} />
              </View>
              <Text style={[s.cardTitle, { color: text }]}>{activeConfig.label}</Text>
            </View>

            {activeConfig.fields.map(field => (
              <View key={field.key}>
                <Text style={[s.fieldLabel, { color: sub }]}>{field.label}</Text>
                <TextInput
                  style={[s.input, { backgroundColor: inputBg, color: text, borderColor: accounts[activeProvider]?.[field.key] ? activeConfig.color : border }]}
            ...
            <Pressable
              ...
              style={[s.saveBtn, { backgroundColor: activeConfig.color, opacity: saving === activeProvider ? 0.6 : 1 }]}
            >
...
const s = StyleSheet.create({
  ...
  infoBanner: { flexDirection: "row", alignItems: "flex-start", gap: 8, padding: 12, borderRadius: 10, marginBottom: 16 },
  infoText: { flex: 1, fontSize: 12, lineHeight: 17 },
  providerTab: { flexDirection: "row", alignItems: "center", gap: 10, paddingVertical: 10, paddingHorizontal: 14, borderRadius: 10, borderWidth: 1.5, marginBottom: 8 },
