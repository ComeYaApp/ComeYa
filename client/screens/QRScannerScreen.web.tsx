...
export default function QRScannerScreen() {
  ...
  useEffect(() => {
    loadJsQR().then(setJsQR).catch(() => setHasPermission(false));
    startCamera();
    return () => stopCamera();
  }, []);
  ...
  const handleQRDetected = async (data: string) => {
    ...
    try {
      ...
      Alert.alert(
        ...
        [
          { text: "Reintentar", onPress: () => { setScanned(false); setProcessing(false); startCamera(); } },
  ...
  if (hasPermission === false) {
    return (
      <View style={[s.container, { backgroundColor: theme.backgroundRoot, paddingTop: insets.top }]}>
        ...
        <Pressable style={[s.btn, { backgroundColor: ComeYaColors.primary }]} onPress={() => navigation.goBack()}>
  ...
  return (
    <View style={s.container}>
      ...
      <View style={s.overlay}>
        {/* Botón cerrar */}
        <Pressable style={[s.closeBtn, { top: insets.top + Spacing.md }]} onPress={() => { stopCamera(); navigation.goBack(); }}>
        ...
        <View style={s.instructions}>
          {processing ? (
            <>
              ...
              <ThemedText type="h4" style={{ color: "#fff", marginTop: Spacing.md }}>Procesando...</ThemedText>
            </>
          ) : (
            <>
              ...
              <ThemedText type="body" style={{ color: "rgba(255,255,255,0.8)", textAlign: "center", marginTop: Spacing.sm }}>
...
const s = StyleSheet.create({
  ...
  corner: { position: "absolute", width: 44, height: 44, borderColor: ComeYaColors.primary },
  tl: { top: 0, left: 0, borderTopWidth: 4, borderLeftWidth: 4, borderTopLeftRadius: BorderRadius.lg },
  tr: { top: 0, right: 0, borderTopWidth: 4, borderRightWidth: 4, borderTopRightRadius: BorderRadius.lg },
  bl: { bottom: 0, left: 0, borderBottomWidth: 4, borderLeftWidth: 4, borderBottomLeftRadius: BorderRadius.lg },
  br: { bottom: 0, right: 0, borderBottomWidth: 4, borderRightWidth: 4, borderBottomRightRadius: BorderRadius.lg },
  scanLine: {
    position: "absolute", left: 4, right: 4, height: 2,
    backgroundColor: ComeYaColors.primary, opacity: 0.8,
  },
  instructions: { position: "absolute", bottom: -120, paddingHorizontal: Spacing.xl, alignItems: "center" },
  btn: { marginTop: Spacing.xl, paddingHorizontal: Spacing.xl, paddingVertical: Spacing.md, borderRadius: BorderRadius.lg },
});
