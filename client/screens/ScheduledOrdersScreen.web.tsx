...
export default function ScheduledOrdersScreen() {
  ...
  const upcomingOrders = (scheduledOrders as any[]).filter(o => o.status === 'pending');
  const historyOrders  = (scheduledOrders as any[]).filter(o => ['executed', 'cancelled', 'failed'].includes(o.status));

  const TABS: { id: Tab; label: string; icon: string; count: number }[] = [
    { id: 'upcoming', label: 'Próximos',  icon: 'clock',   count: upcomingOrders.length },
    { id: 'history',  label: 'Historial', icon: 'archive', count: historyOrders.length  },
  ];
    ...
    <View style={[s.root, { backgroundColor: bg }]}>
      ...
      >
        <View style={[s.sideHeader, { borderBottomColor: border }]}>
          ...
          <Text style={[s.sideTitle, { color: text }]}>Pedidos Programados</Text>
          <Text style={[s.sideSub, { color: sub }]}>Gestiona tus pedidos automáticos</Text>

          <View style={s.sideStats}>
            <View style={[s.statBox, { backgroundColor: cardBg }]}>
              <Text style={[s.statNum, { color: PRIMARY }]}>{upcomingOrders.length}</Text>
              <Text style={[s.statLabel, { color: sub }]}>Próximos</Text>
            </View>
            <View style={[s.statBox, { backgroundColor: cardBg }]}>
              <Text style={[s.statNum, { color: text }]}>{historyOrders.length}</Text>
        ...
        <View style={s.sideNav}>
          {TABS.map(tab => (
            ...
            >
              ...
              <View style={[s.navBadge, { backgroundColor: activeTab === tab.id ? PRIMARY : cardBg }]}>
                <Text style={[s.navBadgeText, { color: activeTab === tab.id ? '#fff' : sub }]}>{tab.count}</Text>
              </View>
            </Pressable>
          ))}
        </View>
      ...
      <ScrollView style={s.main} contentContainerStyle={s.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={[s.tabRow, { backgroundColor: card, borderColor: border }]}>
          {TABS.map(tab => (
            ...
            >
              ...
              <View style={[s.tabBadge, { backgroundColor: activeTab === tab.id ? PRIMARY + '20' : cardBg }]}>
                <Text style={[s.tabBadgeText, { color: activeTab === tab.id ? PRIMARY : sub }]}>{tab.count}</Text>
              </View>
            </Pressable>
          ))}
        </View>
        ...
        {isLoading ? (
          <View style={s.loadingWrap}>
            ...
            <Text style={[s.loadingText, { color: sub }]}>Cargando pedidos...</Text>
          </View>
        ) : activeTab === 'upcoming' ? (
          upcomingOrders.length === 0 ? (
            <View style={[s.empty, { backgroundColor: card, borderColor: border }]}>
              <Feather name="calendar" size={44} color={sub} />
              <Text style={[s.emptyTitle, { color: text }]}>Sin pedidos programados</Text>
            ...
            upcomingOrders.map((order: any) => {
              ...
              return (
                <View key={order.id} style={[s.orderCard, { backgroundColor: card, borderColor: border }]}>
                  <View style={s.orderCardHeader}>
                    <View style={[s.orderIconWrap, { backgroundColor: PRIMARY + '15' }]}>
                      <Feather name="shopping-bag" size={20} color={PRIMARY} />
                    </View>
                    <View style={{ flex: 1, marginLeft: 12 }}>
                      <Text style={[s.businessName, { color: text }]}>{order.businessName || 'Negocio'}</Text>
                      {order.recurringPattern && (
                        <View style={s.recurringBadge}>
                          ...
                          <Text style={[s.recurringText, { color: PRIMARY }]}>
                            {RECURRENCE_LABELS[order.recurringPattern] || order.recurringPattern}
                          </Text>
                        </View>
                      )}
                    </View>
                    <Text style={[s.totalText, { color: PRIMARY }]}>{formatCurrency(order.total || 0)}</Text>
                  </View>
                  ...
                  {items.length > 0 && (
                    <View style={[s.itemsWrap, { borderTopColor: border }]}>
                      ...
                      {items.length > 3 && (
                        <Text style={[s.moreItems, { color: sub }]}>+{items.length - 3} productos más</Text>
                      )}
                    </View>
                  )}

                  <View style={[s.orderCardFooter, { borderTopColor: border }]}>
                    <Pressable
                    ...
                    >
                      {cancelMutation.isPending
                        ...
                        : <>
                            ...
                            <Text style={{ color: '#EF4444', fontSize: 13, fontWeight: '600', marginLeft: 6 }}>Cancelar pedido</Text>
                          </>
                      }
                    </Pressable>
                  </View>
          ...
          historyOrders.length === 0 ? (
            <View style={[s.empty, { backgroundColor: card, borderColor: border }]}>
              ...
              <Text style={[s.emptyText, { color: sub }]}>Aquí aparecerán tus pedidos ejecutados y cancelados</Text>
            </View>
          ) : (
            historyOrders.map((order: any) => {
              ...
              return (
                <View key={order.id} style={[s.historyCard, { backgroundColor: card, borderColor: border }]}>
                  <View style={[s.historyIconWrap, { backgroundColor: cfg.color + '15' }]}>
                    <Feather name={cfg.icon as any} size={20} color={cfg.color} />
                  </View>
                  <View style={{ flex: 1, marginLeft: 12 }}>
                    <Text style={[s.businessName, { color: text }]}>{order.businessName || 'Negocio'}</Text>
                    <Text style={[s.historyDate, { color: sub }]}>{date} · {time}</Text>
                  </View>
...
const s = StyleSheet.create({
  ...
  tabRow:          { flexDirection: 'row', borderRadius: 14, borderWidth: 1, marginBottom: 24, overflow: 'hidden' as any },
  ...
  orderCard:       { borderRadius: 16, borderWidth: 1, marginBottom: 16, overflow: 'hidden' as any },
  orderCardHeader: { flexDirection: 'row', alignItems: 'center', padding: 16 },
  orderIconWrap:   { width: 44, height: 44, borderRadius: 22, justifyContent: 'center', alignItems: 'center' },
  ...
  orderCardBody:   { flexDirection: 'row', gap: 20, paddingHorizontal: 16, paddingVertical: 12, borderTopWidth: 1 },
  dateRow:         { flexDirection: 'row', alignItems: 'center', gap: 6 },
  dateText:        { fontSize: 13 },
  itemsWrap:       { paddingHorizontal: 16, paddingVertical: 12, borderTopWidth: 1, gap: 4 },
  itemText:        { fontSize: 13 },
  moreItems:       { fontSize: 12, fontStyle: 'italic' },
  orderCardFooter: { flexDirection: 'row', justifyContent: 'flex-end', padding: 12, borderTopWidth: 1 },
  cancelBtn:       { flexDirection: 'row', alignItems: 'center', borderWidth: 1, borderRadius: 8, paddingHorizontal: 14, paddingVertical: 8 },
  historyCard:     { flexDirection: 'row', alignItems: 'center', borderRadius: 14, borderWidth: 1, padding: 16, marginBottom: 12 },
  historyIconWrap: { width: 44, height: 44, borderRadius: 22, justifyContent: 'center', alignItems: 'center' },
