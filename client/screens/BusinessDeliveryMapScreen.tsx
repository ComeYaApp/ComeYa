...
import { View, StyleSheet, Pressable, ActivityIndicator, ScrollView, Text as RNText, Alert } from "react-native";
...
export default function BusinessDeliveryMapScreen() {
  ...
  return (
    <View style={[styles.root, { backgroundColor: bg }]}>
      {/* Header */}
      <View style={[styles.header, { backgroundColor: card, paddingTop: insets.top + 8 }]}>
        ...
        <Pressable onPress={fetchDeliveries} style={[styles.refreshBtn, { backgroundColor: ComeYaColors.primary + "15" }]}>
      ...
      <View style={styles.mapContainer}>
        ...
        {loading && (
          <View style={[styles.loadingOverlay, { backgroundColor: card }]}>
            ...
            <ThemedText type="body" style={{ marginTop: 12, color: sub }}>Cargando entregas...</ThemedText>
          </View>
        )}

        {/* Stats bar */}
        {stats && !loading && (
          <View style={[styles.statsBar, { backgroundColor: card, borderColor: bord }]}>
            <View style={styles.statItem}>
              <ThemedText type="h4" style={{ color: ComeYaColors.primary }}>{stats.totalActive}</ThemedText>
              <ThemedText type="caption" style={{ color: sub }}>Activos</ThemedText>
            </View>
            <View style={styles.statItem}>
              <ThemedText type="h4" style={{ color: "#F59E0B" }}>{stats.pending}</ThemedText>
              <ThemedText type="caption" style={{ color: sub }}>Esperando</ThemedText>
            </View>
            <View style={styles.statItem}>
              <ThemedText type="h4" style={{ color: "#DC2626" }}>{stats.onTheWay}</ThemedText>
              <ThemedText type="caption" style={{ color: sub }}>En camino</ThemedText>
            </View>
            <View style={styles.statItem}>
              <ThemedText type="h4" style={{ color: "#8B5CF6" }}>{stats.avgMinutes}m</ThemedText>
              <ThemedText type="caption" style={{ color: sub }}>Tiempo med</ThemedText>
            </View>
          </View>
        )}
      </View>
      ...
      {/* Deliveries list */}
      <View style={[styles.listContainer, { backgroundColor: bg, borderTopColor: bord }]}>
            ...
            deliveries.map((d) => {
              ...
              return (
                <Pressable
                  ...
                  style={[
                    styles.deliveryCard,
                    { backgroundColor: card, borderColor: bord, borderLeftColor: cfg.color },
                  ]}
                >
                  <View style={{ flex: 1 }}>
                    <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
                      ...
                      <View style={[styles.timeBadge, { backgroundColor: d.minutesActive > 30 ? "#EF444420" : "#10B98120" }]}>
                    ...
                    <ThemedText type="caption" style={{ color: sub, marginTop: 2 }}>
                      👤 {d.customer.name}
                    </ThemedText>
                    
                    {d.driver ? (
                      <ThemedText type="caption" style={{ color: cfg.color, marginTop: 2 }}>
                        🛵 {d.driver.name}
                      </ThemedText>
                    ) : (
