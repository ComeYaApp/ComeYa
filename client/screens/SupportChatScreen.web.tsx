...
const WELCOME: Message = {
  ...
  content: '¡Hola! Soy el asistente virtual de ComeYa. Estoy aquí para ayudarte con:\n\n• Información sobre tus pedidos\n• Consultas sobre productos y negocios\n• Tiempos de entrega\n• Métodos de pago\n• Cualquier otra duda\n\n¿En qué puedo ayudarte hoy?',
};
...
export default function SupportChatScreen() {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const res = await fetch(new URL('/api/support/chat', getApiUrl()).toString(), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify({
          userId: user?.id, message: userMsg.content,
      ...
      setMessages(prev => [...prev, { id: (Date.now() + 1).toString(), role: 'assistant', content: 'Lo siento, hubo un problema. Intenta de nuevo o contáctanos por WhatsApp.', timestamp: new Date() }]);
    } finally { setLoading(false); }
  };

  const handleKey = (e: any) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); sendMessage(); } };

  return (
    <View style={[s.root, { backgroundColor: bg }]}>
      <MobileSidebarWrapper title="Chat de Soporte" sidebarStyle={[s.sidebar, { backgroundColor: card, borderRightColor: border }]}>
        <View style={[s.sideHeader, { borderBottomColor: border }]}>
          ...
          <Text style={[s.sideSub, { color: sub }]}>Respuestas instantáneas con IA</Text>
          <View style={[s.onlineBadge, { backgroundColor: '#10B98120', borderColor: '#10B98140' }]}>
            <View style={[s.onlineDot, { backgroundColor: '#10B981' }]} />
            <Text style={{ color: '#10B981', fontSize: 12, fontWeight: '600' }}>En línea</Text>
          </View>
        </View>
        <View style={s.sideInfo}>
          <Text style={[s.sideInfoTitle, { color: text }]}>Puedo ayudarte con:</Text>
          {['Estado de pedidos', 'Métodos de pago', 'Tiempos de entrega', 'Problemas con productos', 'Información general'].map(item => (
        ...
        <View style={[s.sideFooter, { borderTopColor: border }]}>
          <Pressable onPress={() => navigation.navigate('Support')} style={s.backBtn}>
      ...
      <View style={s.chatArea}>
        ...
        <ScrollView ref={scrollRef} style={s.messages} contentContainerStyle={s.messagesContent} showsVerticalScrollIndicator={false}>
          {messages.map(msg => {
            ...
            return (
              <View key={msg.id} style={[s.bubbleWrap, isUser ? s.bubbleWrapUser : s.bubbleWrapBot]}>
                {!isUser && (
                  <View style={[s.botAvatarSmall, { backgroundColor: PRIMARY + '15' }]}>
                    <Feather name="message-circle" size={14} color={PRIMARY} />
                  </View>
                )}
                <View style={[s.bubble, isUser ? [s.bubbleUser, { backgroundColor: PRIMARY }] : [s.bubbleBot, { backgroundColor: card, borderColor: border }]]}>
                  {!isUser && <Text style={[s.bubbleName, { color: PRIMARY }]}>ComeYa</Text>}
                  <Text style={[s.bubbleText, { color: isUser ? '#fff' : text }]}>{msg.content}</Text>
                  <Text style={[s.bubbleTime, { color: isUser ? 'rgba(255,255,255,0.7)' : sub }]}>{time}</Text>
                </View>
              </View>
            );
          })}
          {loading && (
            <View style={[s.bubbleWrap, s.bubbleWrapBot]}>
              ...
              <View style={[s.bubble, s.bubbleBot, { backgroundColor: card, borderColor: border }]}>
        ...
        {/* Input */}
        <View style={[s.inputArea, { backgroundColor: card, borderTopColor: border }]}>
          <TextInput
            value={input}
          ...
          <Pressable
            ...
            style={[s.sendBtn, { backgroundColor: input.trim() && !loading ? PRIMARY : cardBg }]}
          >
            {loading
              ? <ActivityIndicator size="small" color={sub} />
              : <Feather name="send" size={18} color={input.trim() ? '#fff' : sub} />
            }
          </Pressable>
        </View>
      </View>
...
const s = StyleSheet.create({
  ...
  botAvatar:      { width: 64, height: 64, borderRadius: 32, justifyContent: 'center', alignItems: 'center', marginBottom: 10 },
  sideTitle:      { fontSize: 16, fontWeight: '700', marginBottom: 4, textAlign: 'center' },
  sideSub:        { fontSize: 12, textAlign: 'center', marginBottom: 10 },
  onlineBadge:    { flexDirection: 'row', alignItems: 'center', gap: 6, paddingHorizontal: 12, paddingVertical: 5, borderRadius: 20, borderWidth: 1 },
  onlineDot:      { width: 7, height: 7, borderRadius: 4 },
  sideInfo:       { flex: 1, padding: 20 },
  sideInfoTitle:  { fontSize: 13, fontWeight: '700', marginBottom: 10 },
  sideInfoRow:    { flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 8 },
  ...
  botAvatarSmall: { width: 28, height: 28, borderRadius: 14, justifyContent: 'center', alignItems: 'center' },
  ...
  inputArea:      { flexDirection: 'row', alignItems: 'flex-end', gap: 10, padding: 16, borderTopWidth: 1 },
  input:          { flex: 1, borderWidth: 1, borderRadius: 12, paddingHorizontal: 16, paddingVertical: 10, fontSize: 15, maxHeight: 100 },
