...
const PAYMENT_INFO: Record<string, {
  ...
  instructions: string[]; fields: { label: string; value: string; copyable: boolean }[];
}> = {
  bizum: {
...
export default function PaymentProofScreen() {
  ...
  const analyzeWithOCR = async (base64: string) => {
    ...
    try {
      ...
      if (data.success && data.extracted) {
        ...
        if (data.extracted.referenceNumber) setReferenceNumber(data.extracted.referenceNumber);
    ...
    return (
      <View style={[styles.container, { backgroundColor: theme.backgroundRoot }]}>
        <View style={[styles.successContainer]}>
          <View style={[styles.successIcon, { backgroundColor: ComeYaColors.success + "20" }]}>
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.backgroundRoot }]}>
      {/* Header */}
      <View style={[styles.header, { paddingTop: insets.top + Spacing.md, backgroundColor: theme.card, borderBottomColor: theme.border }]}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 120 }}>
        ...
        <View style={[styles.amountCard, { backgroundColor: config.color }]}>
          <ThemedText type="caption" style={{ color: "rgba(255,255,255,0.8)" }}>Importe a transferir</ThemedText>
          <ThemedText style={{ color: "#fff", fontSize: 48, fontWeight: "800" }}>€{amountEur}</ThemedText>
        ...
        <View style={[styles.section, { backgroundColor: theme.card }, Shadows.sm]}>
          <View style={styles.sectionHeader}>
            ...
            <ThemedText type="h4" style={{ marginLeft: Spacing.sm }}>Datos de pago</ThemedText>
          </View>
          {config.fields.map((field, i) => (
            <View key={i} style={[styles.fieldRow, { borderBottomColor: theme.border, borderBottomWidth: i < config.fields.length - 1 ? 1 : 0 }]}>
              <View style={{ flex: 1 }}>
                <ThemedText type="caption" style={{ color: theme.textSecondary }}>{field.label}</ThemedText>
                <ThemedText type="body" style={{ fontWeight: "700", marginTop: 2 }}>{field.value}</ThemedText>
              </View>
              {field.copyable && (
                <Pressable onPress={() => handleCopy(field.value, field.label)} style={[styles.copyBtn, { backgroundColor: config.color + "15" }]}>
          ...
          {/* Concepto */}
          <View style={[styles.fieldRow, { backgroundColor: ComeYaColors.warning + "15", borderRadius: BorderRadius.md, marginTop: Spacing.sm }]}>
            <View style={{ flex: 1 }}>
              <ThemedText type="caption" style={{ color: ComeYaColors.warning }}>⚠️ Concepto / Referencia</ThemedText>
              <ThemedText type="body" style={{ fontWeight: "800", marginTop: 2 }}>COMEYA-{shortId}</ThemedText>
            </View>
            <Pressable onPress={() => handleCopy(`COMEYA-${shortId}`, "Concepto")} style={[styles.copyBtn, { backgroundColor: ComeYaColors.warning + "20" }]}>
        ...
        <View style={[styles.section, { backgroundColor: theme.card }, Shadows.sm]}>
          <View style={styles.sectionHeader}>
            ...
            <ThemedText type="h4" style={{ marginLeft: Spacing.sm }}>Cómo pagar</ThemedText>
          </View>
          {config.instructions.map((instruction, i) => (
            <View key={i} style={styles.instructionRow}>
              <View style={[styles.stepBadge, { backgroundColor: ComeYaColors.primary }]}>
                <ThemedText type="caption" style={{ color: "#fff", fontWeight: "700", fontSize: 11 }}>{i + 1}</ThemedText>
              </View>
              <ThemedText type="body" style={{ flex: 1, marginLeft: Spacing.md, color: theme.textSecondary, lineHeight: 20 }}>
                {instruction}
              </ThemedText>
            </View>
          ))}
        ...
        <View style={[styles.section, { backgroundColor: theme.card }, Shadows.sm]}>
          <View style={styles.sectionHeader}>
            ...
            <ThemedText type="h4" style={{ marginLeft: Spacing.sm }}>Subir comprobante</ThemedText>
          </View>

          {!proofImage ? (
            <View style={styles.uploadButtons}>
              <Pressable onPress={handleTakePhoto} style={[styles.uploadBtn, { backgroundColor: theme.backgroundSecondary, borderColor: theme.border }]}>
                <Feather name="camera" size={28} color={ComeYaColors.primary} />
                <ThemedText type="small" style={{ color: theme.text, marginTop: Spacing.xs, fontWeight: "600" }}>Tomar foto</ThemedText>
              </Pressable>
              <Pressable onPress={handlePickImage} style={[styles.uploadBtn, { backgroundColor: theme.backgroundSecondary, borderColor: theme.border }]}>
                <Feather name="image" size={28} color={ComeYaColors.primary} />
                <ThemedText type="small" style={{ color: theme.text, marginTop: Spacing.xs, fontWeight: "600" }}>Galería</ThemedText>
              </Pressable>
            </View>
          ) : (
            <View>
              <View style={styles.imagePreviewContainer}>
                <Image source={{ uri: proofImage }} style={styles.imagePreview} resizeMode="cover" />
                <Pressable onPress={() => { setProofImage(null); setOcrResult(null); }} style={styles.removeImageBtn}>
                  <Feather name="x" size={16} color="#fff" />
                </Pressable>
                {isAnalyzing && (
                  <View style={styles.analyzingOverlay}>
                    <ActivityIndicator color="#fff" />
                    <ThemedText type="caption" style={{ color: "#fff", marginTop: 4 }}>Analizando...</ThemedText>
                  </View>
                )}
              </View>
              {ocrResult && (
                <View style={[styles.ocrResult, { backgroundColor: ComeYaColors.success + "15", borderColor: ComeYaColors.success }]}>
                  <Feather name="zap" size={16} color={ComeYaColors.success} />
                  <ThemedText type="caption" style={{ color: ComeYaColors.success, marginLeft: Spacing.xs, fontWeight: "600" }}>
          ...
          <View style={{ marginTop: Spacing.lg }}>
            ...
            <TextInput
              style={[styles.input, { backgroundColor: theme.backgroundSecondary, borderColor: referenceNumber ? ComeYaColors.primary : theme.border, color: theme.text }]}
            ...
            <TextInput
              style={[styles.input, { backgroundColor: theme.backgroundSecondary, borderColor: senderName ? ComeYaColors.primary : theme.border, color: theme.text }]}
              value={senderName}
      ...
      <View style={[styles.footer, { backgroundColor: theme.card, borderTopColor: theme.border, paddingBottom: insets.bottom + Spacing.md }]}>
        <Pressable
          ...
          style={[styles.primaryBtn, {
            backgroundColor: proofImage && referenceNumber.trim() ? ComeYaColors.primary : theme.border,
...
const styles = StyleSheet.create({
  ...
  uploadBtn: {
    flex: 1, height: 100, borderRadius: BorderRadius.lg, borderWidth: 1.5,
    borderStyle: "dashed", justifyContent: "center", alignItems: "center",
  },
  imagePreviewContainer: { position: "relative", borderRadius: BorderRadius.lg, overflow: "hidden" },
  imagePreview: { width: "100%", height: 200, borderRadius: BorderRadius.lg },
  removeImageBtn: {
    position: "absolute", top: 8, right: 8,
    backgroundColor: "rgba(0,0,0,0.6)", width: 28, height: 28,
  ...
  input: {
    height: 48, borderWidth: 1.5, borderRadius: BorderRadius.md,
