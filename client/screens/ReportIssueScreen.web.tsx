...
export default function ReportIssueScreen() {
  ...
  const reportMutation = useMutation({
    mutationFn: async () => apiRequest('POST', new URL(`/api/orders/${orderId}/report-issue`, getApiUrl()).toString(), {
      orderId, reporterId: user?.id, issueType: selectedType, description: description.trim(), priority,
    }),
    onSuccess: () => {
      showToast('Problema reportado exitosamente', 'success');
      queryClient.invalidateQueries({ queryKey: ['/api/users', user?.id, 'issues'] });
      navigation.goBack();
    },
    ...
    <View style={[s.root, { backgroundColor: bg }]}>
      <MobileSidebarWrapper title="Reportar Problema" sidebarStyle={[s.sidebar, { backgroundColor: card, borderRightColor: border }]}>
        <View style={[s.sideHeader, { borderBottomColor: border }]}>
          ...
          {selectedIssue && (
            <View style={[s.selBadge, { backgroundColor: PRIMARY + '15', borderColor: PRIMARY + '30' }]}>
              <Feather name={selectedIssue.icon as any} size={13} color={PRIMARY} />
              <Text style={{ color: PRIMARY, fontSize: 12, fontWeight: '600' }}>{selectedIssue.label}</Text>
            </View>
          )}
          {selectedPriority && (
            <View style={[s.selBadge, { backgroundColor: selectedPriority.color + '15', borderColor: selectedPriority.color + '30', marginTop: 6 }]}>
              <Text style={{ color: selectedPriority.color, fontSize: 12, fontWeight: '600' }}>Prioridad: {selectedPriority.label}</Text>
            </View>
          )}
        </View>
      ...
      <ScrollView style={s.main} contentContainerStyle={s.content} showsVerticalScrollIndicator={false}>
            ...
            {ISSUE_TYPES.map(type => {
              ...
              return (
                <Pressable key={type.id} onPress={() => setSelectedType(type.id)}
                  style={[s.typeCard, { backgroundColor: active ? PRIMARY + '10' : cardBg, borderColor: active ? PRIMARY : border }]}>
                  <View style={[s.typeIcon, { backgroundColor: active ? PRIMARY + '20' : border + '40' }]}>
                    <Feather name={type.icon as any} size={22} color={active ? PRIMARY : sub} />
                  </View>
                  <Text style={[s.typeLabel, { color: active ? PRIMARY : text }]}>{type.label}</Text>
                </Pressable>
              );
            })}
          </View>
          ...
          <View style={s.priorityRow}>
            {PRIORITIES.map(p => (
              <Pressable key={p.id} onPress={() => setPriority(p.id)}
                style={[s.priorityBtn, { backgroundColor: priority === p.id ? p.color : cardBg, borderColor: p.color }]}>
                <Text style={[s.priorityBtnText, { color: priority === p.id ? '#fff' : p.color }]}>{p.label}</Text>
              </Pressable>
            ))}
          </View>
        </View>
        ...
        <View style={[s.card, { backgroundColor: card, borderColor: border }]}>
          <View style={s.cardHeader}>
            ...
            <Text style={[s.cardTitle, { color: text }]}>Describe el problema</Text>
          </View>
          <TextInput
            value={description}
        ...
        <View style={[s.card, { backgroundColor: card, borderColor: border }]}>
          <View style={s.cardHeader}>
            ...
            <Text style={[s.cardTitle, { color: text }]}>Fotos (opcional, máx. 4)</Text>
          </View>
          <View style={s.photosRow}>
            ...
            {photos.length < 4 && (
              <Pressable onPress={handlePickPhoto} style={[s.addPhoto, { borderColor: border, backgroundColor: cardBg }]}>
        ...
        {/* Submit */}
        <Pressable onPress={handleSubmit} disabled={reportMutation.isPending}
          style={[s.submitBtn, { backgroundColor: PRIMARY, opacity: reportMutation.isPending ? 0.6 : 1 }]}>
          {reportMutation.isPending
            ? <ActivityIndicator color="#fff" />
            : <><Feather name="send" size={18} color="#fff" /><Text style={s.submitBtnText}>Enviar reporte</Text></>
          }
        </Pressable>
      </ScrollView>
    </View>
...
const s = StyleSheet.create({
  ...
  selBadge:      { flexDirection: 'row', alignItems: 'center', gap: 6, paddingHorizontal: 12, paddingVertical: 5, borderRadius: 20, borderWidth: 1 },
  ...
  typeCard:      { width: '30%', padding: 14, borderRadius: 12, borderWidth: 2, alignItems: 'center', gap: 8 },
  ...
  priorityBtn:   { flex: 1, paddingVertical: 12, borderRadius: 10, borderWidth: 1.5, alignItems: 'center' },
  priorityBtnText:{ fontSize: 14, fontWeight: '700' },
  textarea:      { borderWidth: 1, borderRadius: 10, padding: 14, fontSize: 15, minHeight: 120, textAlignVertical: 'top' as any },
  ...
  addPhoto:      { width: 90, height: 90, borderRadius: 10, borderWidth: 2, borderStyle: 'dashed' as any, justifyContent: 'center', alignItems: 'center', gap: 4 },
