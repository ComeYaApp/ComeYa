...
type Route = RouteProp<{ Review: { orderId: string; businessId: string; businessName: string; deliveryPersonId?: string } }, 'Review'>;

function Stars({ rating, onRate, label }: { rating: number; onRate: (n: number) => void; label: string }) {
  const { isDark } = useTheme();
  const sub = isDark ? '#aaa' : '#666';
  return (
    <View style={s.starBlock}>
      <Text style={[s.starLabel, { color: isDark ? '#fff' : '#1a1a1a' }]}>{label}</Text>
      <View style={s.starRow}>
        {[1,2,3,4,5].map(n => (
          <Pressable key={n} onPress={() => onRate(n)} style={s.starBtn}>
            <Feather name="star" size={28} color={n <= rating ? '#FFD700' : sub} />
          </Pressable>
        ))}
      </View>
    </View>
...
export default function ReviewScreenEnhanced() {
  ...
  const handlePickPhoto = async () => {
    if (photos.length >= 3) { showToast('Máximo 3 fotos', 'warning'); return; }
  ...
  const submitMutation = useMutation({
    mutationFn: async () => (await apiRequest('POST', '/api/reviews', {
      orderId, userId: user?.id, businessId, deliveryPersonId,
      foodRating: foodRating || undefined, deliveryRating: deliveryRating || undefined,
      packagingRating: packagingRating || undefined, driverRating: driverRating || undefined,
  ...
  const avgRating = [foodRating, packagingRating, deliveryRating, driverRating].filter(Boolean);
    ...
    <View style={[s.root, { backgroundColor: bg }]}>
      <MobileSidebarWrapper title="Calificar pedido" sidebarStyle={[s.sidebar, { backgroundColor: card, borderRightColor: border }]}>
        <View style={[s.sideHeader, { borderBottomColor: border }]}>
          ...
          <Text style={[s.sideSub, { color: sub }]}>Califica tu experiencia</Text>
          {avg !== '—' && (
            <View style={[s.avgBadge, { backgroundColor: '#FFD70020', borderColor: '#FFD70040' }]}>
              <Feather name="star" size={14} color="#FFD700" />
              <Text style={[s.avgText, { color: '#B8860B' }]}>{avg} promedio</Text>
            </View>
          )}
        </View>
        <View style={s.sideInfo}>
          {[
            ...
            ...(deliveryPersonId ? [{ label: 'Entrega', val: deliveryRating }, { label: 'Repartidor', val: driverRating }] : []),
          ].map(item => (
            <View key={item.label} style={s.sideRatingRow}>
              <Text style={[s.sideRatingLabel, { color: sub }]}>{item.label}</Text>
              <View style={s.sideStars}>
                {[1,2,3,4,5].map(n => (
                  <Feather key={n} name="star" size={12} color={n <= item.val ? '#FFD700' : border} />
                ))}
              </View>
            </View>
          ))}
      ...
      <ScrollView style={s.main} contentContainerStyle={s.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={[s.card, { backgroundColor: card, borderColor: border }]}>
          <View style={s.cardHeader}>
            ...
            <Text style={[s.cardTitle, { color: text }]}>Calidad del pedido</Text>
          </View>
          <Stars rating={foodRating} onRate={setFoodRating} label="Comida" />
          <Stars rating={packagingRating} onRate={setPackagingRating} label="Empaque" />
        </View>
        ...
        {deliveryPersonId && (
          <View style={[s.card, { backgroundColor: card, borderColor: border }]}>
            ...
            <Stars rating={deliveryRating} onRate={setDeliveryRating} label="Velocidad" />
            <Stars rating={driverRating} onRate={setDriverRating} label="Repartidor" />
          </View>
        )}
        ...
        {tags.length > 0 && (
          <View style={[s.card, { backgroundColor: card, borderColor: border }]}>
            <View style={s.cardHeader}>
              ...
              <Text style={[s.cardTitle, { color: text }]}>Etiquetas (opcional)</Text>
            </View>
              ...
              {tags.map((tag: any) => {
                ...
                return (
                  <Pressable
                    ...
                    style={[s.tag, { backgroundColor: active ? color + '20' : cardBg, borderColor: active ? color : border }]}
                  >
                    <Feather name={tag.icon as any} size={13} color={active ? color : sub} />
                    <Text style={[s.tagText, { color: active ? color : text }]}>{tag.tagName}</Text>
                  </Pressable>
                );
              })}
            </View>
        ...
        <View style={[s.card, { backgroundColor: card, borderColor: border }]}>
          <View style={s.cardHeader}>
            ...
            <Text style={[s.cardTitle, { color: text }]}>Fotos (opcional, máx. 3)</Text>
          </View>
          <View style={s.photosRow}>
            {photos.map((uri, i) => (
              <View key={i} style={s.photoWrap}>
                <Image source={{ uri }} style={s.photo} contentFit="cover" />
                <Pressable onPress={() => setPhotos(photos.filter((_, j) => j !== i))} style={s.removePhoto}>
                  <Feather name="x" size={14} color="#fff" />
                </Pressable>
              </View>
            ))}
            {photos.length < 3 && (
              <Pressable onPress={handlePickPhoto} style={[s.addPhoto, { borderColor: border }]}>
        ...
        <View style={[s.card, { backgroundColor: card, borderColor: border }]}>
          <View style={s.cardHeader}>
            ...
            <Text style={[s.cardTitle, { color: text }]}>Comentario (opcional)</Text>
          </View>
          <TextInput
            value={comment}
        ...
        <Pressable
          ...
          style={[s.submitBtn, { backgroundColor: PRIMARY, opacity: submitMutation.isPending ? 0.6 : 1 }]}
        >
          {submitMutation.isPending
            ? <ActivityIndicator color="#fff" />
            : <><Feather name="send" size={18} color="#fff" /><Text style={s.submitText}>Enviar calificación</Text></>
          }
        </Pressable>
      </ScrollView>
    </View>
...
const s = StyleSheet.create({
  ...
  sideTitle:      { fontSize: 16, fontWeight: '700', textAlign: 'center', marginBottom: 4 },
  sideSub:        { fontSize: 12, textAlign: 'center', marginBottom: 10 },
  avgBadge:       { flexDirection: 'row', alignItems: 'center', gap: 5, paddingHorizontal: 12, paddingVertical: 5, borderRadius: 20, borderWidth: 1 },
  avgText:        { fontSize: 13, fontWeight: '700' },
  sideInfo:       { flex: 1, padding: 16, gap: 10 },
  sideRatingRow:  { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  ...
  removePhoto:    { position: 'absolute' as any, top: -6, right: -6, width: 22, height: 22, borderRadius: 11, backgroundColor: '#EF4444', justifyContent: 'center', alignItems: 'center' },
  addPhoto:       { width: 90, height: 90, borderRadius: 10, borderWidth: 2, borderStyle: 'dashed' as any, justifyContent: 'center', alignItems: 'center' },
  textarea:       { borderRadius: 10, padding: 12, fontSize: 15, minHeight: 100, textAlignVertical: 'top' as any },
  submitBtn:      { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 10, paddingVertical: 16, borderRadius: 12 },
