...
export default function BusinessManageScreen() {
  ...
  const settingItems = [
    ...
    },
    {
      ...
      onPress: () => navigation.navigate("BusinessStats" as any),
    },
    {
      icon: "credit-card",
      label: "Cuentas de cobro",
      sub: "Bizum, IBAN, PayPal",
            ...
            {unavailable.length > 0 && (
              <>
                <View style={s.sectionHeader}>
                  ...
                  />
                  <Text style={[s.sectionTitle, { color: ComeYaColors.error }]}>
...
function ProductRow({ product, onToggle, card, border, text, sub }: any) {
  return (
    <View
      style={[s.productRow, { backgroundColor: card, borderColor: border }]}
    >
      <Image
      ...
      />
      <View style={{ flex: 1, marginLeft: 14 }}>
        <Text style={[s.productName, { color: text }]} numberOfLines={1}>
          {product.name}
        </Text>
