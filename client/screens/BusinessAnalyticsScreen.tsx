...
export default function BusinessAnalyticsScreen() {
  ...
  const { data: dashboardData, refetch: refetchDashboard } = useQuery({
    ...
    queryFn: async () => {
      const response = await apiRequest(
        "GET",
        `/api/analytics/dashboard/${businessId}?period=${period}`,
      );
  ...
  const { data: topProductsData } = useQuery({
    ...
    queryFn: async () => {
      const response = await apiRequest(
        "GET",
        `/api/analytics/top-products/${businessId}?limit=5`,
      );
      return response.json();
    },
  });
  ...
  const { data: peakHoursData } = useQuery({
    ...
    queryFn: async () => {
      const response = await apiRequest(
        "GET",
        `/api/analytics/peak-hours/${businessId}`,
      );
      return response.json();
    },
  });
  ...
  const { data: salesChartData } = useQuery({
    ...
    queryFn: async () => {
      const response = await apiRequest(
        "GET",
        `/api/analytics/sales-chart/${businessId}?days=7`,
      );
      return response.json();
    },
  });
  ...
  const { data: comparisonData } = useQuery({
    ...
    queryFn: async () => {
      const response = await apiRequest(
        "GET",
        `/api/analytics/weekly-comparison/${businessId}`,
      );
      return response.json();
    },
  });
      ...
      >
        ...
        <View style={styles.metricsGrid}>
          ...
          >
            ...
            />
            <ThemedText type="h3" style={{ marginTop: Spacing.sm }}>
              {formatCurrency(dashboard?.avgOrderValue || 0)}
            </ThemedText>
            <ThemedText type="caption" style={{ color: theme.textSecondary }}>
              Ticket Promedio
            </ThemedText>
            ...
            <View style={styles.comparisonRow}>
              <View style={styles.comparisonCol}>
                ...
                <ThemedText type="body">
                  {formatEuros(comparison.thisWeek.revenue)}
                </ThemedText>
              </View>
              <View style={styles.comparisonCol}>
                <ThemedText
                ...
                <ThemedText type="body">
                  {formatEuros(comparison.lastWeek.revenue)}
                </ThemedText>
              </View>
            </View>
          </View>
              ...
              <View key={product.productId} style={styles.productRow}>
                ...
                <View style={styles.productInfo}>
                  ...
                  <ThemedText
                    ...
                    style={{ color: theme.textSecondary }}
                  >
                    {product.unitsSold} unidades • {formatEuros(product.revenue)}
                  </ThemedText>
                </View>
              </View>
            ))}
          ...
          >
            ...
            <BarChart
              data={{
                ...
                datasets: [
                  { data: peakHours.slice(0, 6).map((h: any) => h.orderCount) },
                ],
              }}
              width={width - 64}
              height={200}
