...
import {
  View, StyleSheet, ScrollView, Pressable, Text, ActivityIndicator,
} from "react-native";
...
export default function BusinessDetailScreen() {
  ...
  useEffect(() => {
    apiRequest("GET", `/api/business/${route.params.businessId}`).then(r => r.json()).then(data => {
      ...
      const prods = (data.business?.products || []).map((p: any) => ({
        ...
        isAvailable: p.isAvailable === true || p.isAvailable === 1 || p.is_available === true || p.is_available === 1,
    ...
    <View style={[s.root, { backgroundColor: bg }]}>
      ...
      <View style={s.body}>
        ...
        <ScrollView style={s.main} showsVerticalScrollIndicator={false}>
          {/* Hero */}
          <View style={s.hero}>
            <Image source={{ uri: business?.image }} style={s.heroImg} contentFit="cover" />
            <View style={[s.heroOverlay, { backgroundColor: card }]}>
              <View style={s.heroInfo}>
                <Text style={[s.heroName, { color: text }]}>{business?.name}</Text>
                <Text style={[s.heroDesc, { color: sub }]}>{business?.description}</Text>
                <View style={s.heroMeta}>
                  <View style={s.metaChip}>
                    <Feather name="star" size={14} color="#FFB800" />
                    <Text style={[s.metaChipText, { color: text }]}>{((business?.rating || 0) / 100).toFixed(1)}</Text>
                  </View>
                  <View style={s.metaChip}>
                    <Feather name="clock" size={14} color={sub} />
                    <Text style={[s.metaChipText, { color: sub }]}>{business?.deliveryTime || "30-45 min"}</Text>
                  </View>
                  <View style={s.metaChip}>
                    <Feather name="truck" size={14} color={sub} />
                    <Text style={[s.metaChipText, { color: sub }]}>€{((business?.deliveryFee || 300) / 100).toFixed(2)} envío</Text>
                  </View>
                  <View style={[s.statusBadge, { backgroundColor: business?.isOpen ? "#4CAF5020" : "#9E9E9E20" }]}>
                    <Text style={{ color: business?.isOpen ? "#4CAF50" : "#9E9E9E", fontSize: 12, fontWeight: "700" }}>
          ...
          {/* Categorías */}
          <View style={[s.catBar, { backgroundColor: card, borderBottomColor: border }]}>
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.catScroll}>
              {categories.map(cat => (
                <Pressable key={cat} onPress={() => setActiveCategory(cat)} style={[s.catBtn, activeCategory === cat && s.catBtnActive]}>
                  <Text style={[s.catBtnText, activeCategory === cat && s.catBtnTextActive]}>{cat}</Text>
                </Pressable>
              ))}
            </ScrollView>
          </View>
                ...
                return (
                  <View key={p.id} style={[s.productCard, { backgroundColor: card }]}>
                    ...
                    <View style={s.productBody}>
                      <Text style={[s.productName, { color: text }]} numberOfLines={2}>{p.name}</Text>
                      <Text style={[s.productDesc, { color: sub }]} numberOfLines={2}>{p.description}</Text>
            ...
            <>
              ...
              <View style={[s.cartSummary, { borderTopColor: border }]}>
                <View style={s.cartRow}>
                  <Text style={[s.cartRowLabel, { color: sub }]}>Subtotal</Text>
                  <Text style={[s.cartRowValue, { color: text }]}>€{cartTotal.toFixed(2)}</Text>
                </View>
                <View style={s.cartRow}>
                  <Text style={[s.cartRowLabel, { color: sub }]}>Envío</Text>
                  <Text style={[s.cartRowValue, { color: text }]}>€{((business?.deliveryFee || 300) / 100).toFixed(2)}</Text>
                </View>
                <View style={[s.cartRow, s.cartTotal]}>
                  <Text style={[s.cartTotalLabel, { color: text }]}>Total</Text>
                  <Text style={[s.cartTotalValue, { color: PRIMARY }]}>€{(cartTotal + (business?.deliveryFee || 300) / 100).toFixed(2)}</Text>
                </View>
                <Pressable style={s.checkoutBtn} onPress={() => navigation.navigate("Checkout")}>
                  <Text style={s.checkoutBtnText}>Ir al checkout ({cartCount})</Text>
                </Pressable>
              </View>
            </>
          )}
...
const s = StyleSheet.create({
  ...
  metaChip: { flexDirection: "row", alignItems: "center", gap: 5, paddingHorizontal: 10, paddingVertical: 6, borderRadius: 20, backgroundColor: "#f0f0f0" },
  ...
  productCard: { width: 220, borderRadius: 14, overflow: "hidden", shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.07, shadowRadius: 6, elevation: 2 },
  ...
  addBtn: { width: 32, height: 32, borderRadius: 16, backgroundColor: PRIMARY, justifyContent: "center", alignItems: "center" },
  qtyRow: { flexDirection: "row", alignItems: "center", gap: 8 },
  qtyBtn: { width: 28, height: 28, borderRadius: 14, borderWidth: 1.5, borderColor: PRIMARY, justifyContent: "center", alignItems: "center" },
  qtyText: { fontSize: 14, fontWeight: "700", minWidth: 16, textAlign: "center" },
  // Carrito
  cartPanel: { width: 320, borderLeftWidth: 1, padding: 20, flexDirection: "column" },
  cartTitle: { fontSize: 18, fontWeight: "800", marginBottom: 16 },
  cartEmpty: { flex: 1, justifyContent: "center", alignItems: "center", gap: 12, paddingVertical: 40 },
  ...
  cartItemQty: { width: 28, height: 28, borderRadius: 14, backgroundColor: PRIMARY + "15", justifyContent: "center", alignItems: "center" },
