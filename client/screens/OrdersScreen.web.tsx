      ...
      <View style={s.body}>
        ...
        <View style={[s.list, { borderRightColor: border }]}>
            ...
            {(["active", "history"] as const).map(t => (
              <Pressable key={t} onPress={() => setTab(t)} style={[s.tab, tab === t && s.tabActive]}>
                <Text style={[s.tabText, { color: tab === t ? PRIMARY : sub }]}>
                  {t === "active" ? `Activos (${activeOrders.length})` : `Historial (${historyOrders.length})`}
                </Text>
              </Pressable>
            ))}
          </View>
          ...
          <ScrollView showsVerticalScrollIndicator={false}>
            {loading ? (
              <View style={s.loadingBox}><ActivityIndicator color={PRIMARY} /></View>
            ) : displayOrders.length === 0 ? (
              <View style={s.emptyBox}>
                <Text style={{ fontSize: 40 }}>📦</Text>
                <Text style={[s.emptyText, { color: sub }]}>
                  {tab === "active" ? "No tienes pedidos activos" : "Sin historial de pedidos"}
                </Text>
              </View>
              ...
              return (
                <Pressable
                  ...
                  style={[s.orderCard, { backgroundColor: isSelected ? PRIMARY + "08" : card, borderColor: isSelected ? PRIMARY : border }]}
                >
                  <View style={[s.orderStatus, { backgroundColor: st.color + "20" }]}>
        ...
        <View style={[s.detail, { backgroundColor: card }]}>
          {!selected ? (
            <View style={s.detailEmpty}>
              ...
              <Text style={[s.detailEmptyText, { color: sub }]}>Selecciona un pedido para ver los detalles</Text>
            </View>
          ) : (() => {
            const st = STATUS[selected.status] || STATUS.pending;
            const items = (() => { try { return JSON.parse(selected.items || "[]"); } catch { return []; } })();
            return (
              <ScrollView contentContainerStyle={s.detailContent} showsVerticalScrollIndicator={false}>
                ...
                <View style={[s.statusCard, { backgroundColor: st.color + "12", borderColor: st.color + "30" }]}>
                  ...
                  <View style={{ flex: 1, marginLeft: 12 }}>
                    <Text style={[s.statusCardTitle, { color: st.color }]}>{st.label}</Text>
                    <Text style={[s.statusCardSub, { color: sub }]}>
                      {selected.status === "on_the_way" ? "Tu repartidor está en camino" :
                       selected.status === "delivered" ? "Pedido entregado con éxito" :
                       "Actualizando estado..."}
                    </Text>
                  </View>
                  {["pending", "confirmed", "preparing", "ready", "on_the_way"].includes(selected.status) && (
                    <Pressable
                      style={[s.trackBtn, { backgroundColor: PRIMARY }]}
                      onPress={() => navigation.navigate("OrderTracking", { orderId: selected.id })}
                    >
                ...
                {items.map((item: any, i: number) => (
                  <View key={i} style={[s.detailItem, { borderBottomColor: border }]}>
                    <Text style={[s.detailItemQty, { color: PRIMARY }]}>{item.quantity}x</Text>
                    <Text style={[s.detailItemName, { color: text, flex: 1 }]}>{item.name}</Text>
                    <Text style={[s.detailItemPrice, { color: text }]}>€{((item.price || 0) * item.quantity / 100).toFixed(2)}</Text>
                  </View>
                ))}

                <View style={[s.detailTotals, { borderTopColor: border }]}>
                  <View style={s.detailRow}>
                    <Text style={[s.detailRowLabel, { color: sub }]}>Subtotal</Text>
                    <Text style={[s.detailRowValue, { color: text }]}>€{((selected.subtotal || 0) / 100).toFixed(2)}</Text>
                  </View>
                  <View style={s.detailRow}>
                    <Text style={[s.detailRowLabel, { color: sub }]}>Envío</Text>
                    <Text style={[s.detailRowValue, { color: text }]}>€{((selected.deliveryFee || 0) / 100).toFixed(2)}</Text>
                  </View>
                  <View style={[s.detailRow, s.detailTotalRow]}>
                    <Text style={[s.detailTotalLabel, { color: text }]}>Total</Text>
                    <Text style={[s.detailTotalValue, { color: PRIMARY }]}>€{((selected.total || 0) / 100).toFixed(2)}</Text>
                  </View>
                </View>

                <Text style={[s.detailSectionTitle, { color: text }]}>Entrega</Text>
                <Text style={[s.detailAddress, { color: sub }]}>{selected.deliveryAddress}</Text>
...
const s = StyleSheet.create({
  ...
  orderCard: { flexDirection: "row", alignItems: "center", gap: 12, padding: 16, borderBottomWidth: 1, borderWidth: 0, borderLeftWidth: 3 },
  orderStatus: { width: 40, height: 40, borderRadius: 20, justifyContent: "center", alignItems: "center" },
  ...
  detail: { flex: 1 },
  detailEmpty: { flex: 1, justifyContent: "center", alignItems: "center", gap: 12 },
  ...
  statusCard: { flexDirection: "row", alignItems: "center", padding: 16, borderRadius: 14, borderWidth: 1, marginBottom: 24 },
  ...
  detailSectionTitle: { fontSize: 16, fontWeight: "700", marginBottom: 12, marginTop: 8 },
  detailItem: { flexDirection: "row", alignItems: "center", paddingVertical: 10, borderBottomWidth: 1, gap: 8 },
