...
const EMPTY_FORM = { name: "", description: "", type: "restaurant", address: "", phone: "", image: "" };
      ...
      <View style={s.main}>
        ...
        <View style={[s.toolbar, { backgroundColor: card, borderBottomColor: border }]}>
          ...
          <Pressable
            ...
            style={[s.addBtn, { backgroundColor: showForm ? border : ComeYaColors.primary }]}
          >
          ...
          {showForm && (
            <View style={[s.formCard, { backgroundColor: card, borderColor: ComeYaColors.primary + "40" }]}>
              ...
              <View style={s.typeGrid}>
                {BUSINESS_TYPES.map(t => (
                  <Pressable
                    ...
                    style={[s.typeChip, {
                      backgroundColor: form.type === t.id ? ComeYaColors.primary + "15" : inputBg,
                      borderColor: form.type === t.id ? ComeYaColors.primary : border,
                    }]}
                  >
                    <Feather name={t.icon as any} size={14} color={form.type === t.id ? ComeYaColors.primary : sub} />
                    <Text style={[s.typeChipText, { color: form.type === t.id ? ComeYaColors.primary : text }]}>{t.name}</Text>
                  </Pressable>
                ))}
              </View>

              <Pressable
                onPress={() => pickImage(url => setForm(p => ({ ...p, image: url })))}
                style={[s.imagePicker, { backgroundColor: inputBg, borderColor: border }]}
              >
                {form.image ? (
                  <Image source={{ uri: getImageUrl(form.image) }} style={s.imagePreview} contentFit="cover" />
                ) : (
                  <>
              ...
              <View style={s.formBtns}>
                <Pressable onPress={closeForm} style={[s.formBtn, { borderColor: border, borderWidth: 1 }]}>
                  <Text style={{ color: text }}>Cancelar</Text>
                </Pressable>
                <Pressable
                  onPress={mode === "edit" ? handleEdit : handleCreate}
                  disabled={submitting}
                  style={[s.formBtn, { backgroundColor: ComeYaColors.primary, opacity: submitting ? 0.6 : 1 }]}
                >
                  {submitting
                    ? <ActivityIndicator color="#fff" size="small" />
                    : <Text style={{ color: "#fff", fontWeight: "700" }}>{mode === "edit" ? "Guardar cambios" : "Crear negocio"}</Text>
                  }
                </Pressable>
              </View>
            </View>
          ...
          {isLoading ? (
            ...
            businesses.map((biz: any) => (
              <View key={biz.id} style={[s.bizCard, {
                ...
                borderColor: selectedBusiness?.id === biz.id ? ComeYaColors.primary : border,
                ...
                <View style={s.bizInfo}>
                  <View style={s.bizHeader}>
                    ...
                    <View style={[s.statusBadge, { backgroundColor: biz.isOpen ? ComeYaColors.success + "15" : border }]}>
                      <Text style={[s.statusText, { color: biz.isOpen ? ComeYaColors.success : sub }]}>
                        {biz.isOpen ? "Abierto" : "Cerrado"}
                      </Text>
                    </View>
                  </View>
                  <Text style={[s.bizType, { color: sub }]}>
                    {BUSINESS_TYPES.find(t => t.id === biz.type)?.name || "Negocio"}
                  </Text>
                  {biz.stats && (
                    <View style={[s.statsRow, { borderTopColor: border }]}>
                      <View style={s.statItem}>
                        <Text style={[s.statValue, { color: text }]}>{biz.stats.pendingOrders}</Text>
                        <Text style={[s.statLabel, { color: sub }]}>Pendientes</Text>
                      </View>
                      <View style={s.statItem}>
                        <Text style={[s.statValue, { color: text }]}>{biz.stats.totalOrders}</Text>
                        <Text style={[s.statLabel, { color: sub }]}>Completados</Text>
                      </View>
                      <View style={s.statItem}>
                        <Text style={[s.statValue, { color: ComeYaColors.success }]}>
                          €{((biz.stats.totalRevenue || 0) / 100).toFixed(0)}
                        </Text>
                        <Text style={[s.statLabel, { color: sub }]}>Ingresos</Text>
                      </View>
                    </View>
                  )}
                  <View style={[s.bizActions, { borderTopColor: border }]}>
                    <Pressable onPress={() => selectBusiness(biz)} style={s.bizAction}>
                      <Feather
                        name={selectedBusiness?.id === biz.id ? "check-circle" : "circle"}
                        size={16}
                        color={selectedBusiness?.id === biz.id ? ComeYaColors.primary : sub}
                      />
                      <Text style={[s.bizActionText, { color: selectedBusiness?.id === biz.id ? ComeYaColors.primary : sub }]}>
                        {selectedBusiness?.id === biz.id ? "Seleccionado" : "Seleccionar"}
                      </Text>
                    </Pressable>
                    <Pressable
                      onPress={() => openEdit(biz)}
                      style={[s.bizAction, { borderLeftWidth: 1, borderLeftColor: border }]}
                    >
                      <Feather name="edit-2" size={16} color={ComeYaColors.primary} />
                      <Text style={[s.bizActionText, { color: ComeYaColors.primary }]}>Editar</Text>
                    </Pressable>
                    <Pressable
...
const s = StyleSheet.create({
  ...
  typeGrid: { flexDirection: "row", flexWrap: "wrap", gap: 8, marginBottom: 16, marginTop: 6 },
