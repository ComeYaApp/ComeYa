  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.backgroundRoot, paddingTop: insets.top }]}>
      <View style={styles.header}>
        ...
        <Text style={[styles.title, { color: theme.text }]}>Pedidos Programados</Text>
        <View style={{ width: 44 }} />
      </View>
      ...
      <View style={[styles.tabRow, { backgroundColor: theme.card }]}>
        {([
          { id: 'upcoming', label: `Próximos (${upcomingOrders.length})`, icon: 'clock' },
          { id: 'history',  label: `Historial (${historyOrders.length})`,  icon: 'archive' },
        ] as { id: Tab; label: string; icon: string }[]).map(tab => (
          <Pressable
            key={tab.id}
            onPress={() => setActiveTab(tab.id)}
            style={[styles.tab, activeTab === tab.id && { borderBottomColor: PRIMARY, borderBottomWidth: 2 }]}
          >
            <Feather name={tab.icon as any} size={15} color={activeTab === tab.id ? PRIMARY : theme.textSecondary} />
            <Text style={[styles.tabText, { color: activeTab === tab.id ? PRIMARY : theme.textSecondary }]}>
              {tab.label}
            </Text>
          </Pressable>
        ))}
      ...
      {isLoading ? (
        <View style={styles.loadingWrap}>
          ...
          <Text style={[styles.loadingText, { color: theme.textSecondary }]}>Cargando pedidos...</Text>
        </View>
      ) : displayOrders.length === 0 ? (
        <View style={styles.emptyWrap}>
          <Feather name={activeTab === 'upcoming' ? 'calendar' : 'archive'} size={48} color={theme.textSecondary} />
          <Text style={[styles.emptyTitle, { color: theme.text }]}>
            {activeTab === 'upcoming' ? 'Sin pedidos programados' : 'Sin historial'}
          </Text>
          {activeTab === 'upcoming' && (
        ...
        <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
          {activeTab === 'upcoming' ? (
            upcomingOrders.map((order: any) => {
              ...
              return (
                <View key={order.id} style={[styles.orderCard, { backgroundColor: theme.card }, Shadows.sm]}>
                  <View style={styles.orderHeader}>
                    <View style={[styles.orderIcon, { backgroundColor: PRIMARY + '15' }]}>
                      <Feather name="shopping-bag" size={20} color={PRIMARY} />
                    </View>
                    <View style={{ flex: 1, marginLeft: 12 }}>
                      <Text style={[styles.businessName, { color: theme.text }]}>{order.businessName || 'Negocio'}</Text>
                      {order.recurringPattern && (
                        <View style={styles.recurringBadge}>
                          <Feather name="repeat" size={11} color={PRIMARY} />
                          <Text style={[styles.recurringText, { color: PRIMARY }]}>
                            {RECURRENCE_LABELS[order.recurringPattern] || order.recurringPattern}
                          </Text>
                        </View>
                      )}
                    </View>
                    <Text style={[styles.totalText, { color: PRIMARY }]}>{formatCurrency(order.total || 0)}</Text>
                  </View>

                  <View style={[styles.dateSection, { borderTopColor: theme.border }]}>
                    ...
                    <View style={styles.dateRow}>
                      ...
                      <Text style={[styles.dateText, { color: theme.textSecondary }]}>{time}</Text>
                    </View>
                  </View>
                  ...
                  {items.length > 0 && (
                    <View style={[styles.itemsSection, { borderTopColor: theme.border }]}>
                      ...
                      {items.length > 3 && (
                        <Text style={[styles.moreItems, { color: theme.textSecondary }]}>+{items.length - 3} productos más</Text>
                      )}
                    </View>
                  )}
            ...
            historyOrders.map((order: any) => {
              ...
              return (
                <View key={order.id} style={[styles.historyCard, { backgroundColor: theme.card }, Shadows.sm]}>
                  <View style={[styles.historyIcon, { backgroundColor: cfg.color + '15' }]}>
                    <Feather name={cfg.icon as any} size={20} color={cfg.color} />
                  </View>
                  <View style={{ flex: 1, marginLeft: 12 }}>
                    <Text style={[styles.businessName, { color: theme.text }]}>{order.businessName || 'Negocio'}</Text>
                    <Text style={[styles.dateText, { color: theme.textSecondary }]}>{date} · {time}</Text>
                  </View>
...
const styles = StyleSheet.create({
  ...
  emptyWrap:      { flex: 1, justifyContent: 'center', alignItems: 'center', padding: Spacing.xl, gap: 12 },
  emptyTitle:     { fontSize: 18, fontWeight: '700', textAlign: 'center' },
  emptyText:      { fontSize: 14, textAlign: 'center', lineHeight: 20 },
  content:        { padding: Spacing.lg, paddingBottom: 80 },
  orderCard:      { borderRadius: BorderRadius.xl, marginBottom: Spacing.md, overflow: 'hidden' },
  orderHeader:    { flexDirection: 'row', alignItems: 'center', padding: Spacing.lg },
  orderIcon:      { width: 44, height: 44, borderRadius: 22, justifyContent: 'center', alignItems: 'center' },
  ...
  dateSection:    { flexDirection: 'row', gap: 20, paddingHorizontal: Spacing.lg, paddingVertical: Spacing.md, borderTopWidth: 1 },
  dateRow:        { flexDirection: 'row', alignItems: 'center', gap: 6 },
  dateText:       { fontSize: 13 },
  itemsSection:   { paddingHorizontal: Spacing.lg, paddingVertical: Spacing.md, borderTopWidth: 1, gap: 4 },
  itemText:       { fontSize: 13 },
  moreItems:      { fontSize: 12, fontStyle: 'italic' },
  orderFooter:    { flexDirection: 'row', justifyContent: 'flex-end', padding: Spacing.md, borderTopWidth: 1 },
  cancelBtn:      { flexDirection: 'row', alignItems: 'center', gap: 6, borderWidth: 1, borderRadius: 8, paddingHorizontal: 14, paddingVertical: 8 },
  cancelBtnText:  { color: '#EF4444', fontSize: 13, fontWeight: '600' },
  historyCard:    { flexDirection: 'row', alignItems: 'center', borderRadius: BorderRadius.lg, padding: Spacing.lg, marginBottom: Spacing.md },
