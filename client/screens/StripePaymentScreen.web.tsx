...
function PaymentForm({
  ...
  useEffect(() => {
    const init = async () => {
      try {
        console.log("[Stripe] Init params:", {
          orderId,
          amount,
          businessId,
          giftCardId,
        ...
        if (isGiftCard && (!giftCardId || !amount || amount <= 0)) {
          showToast("Datos de gift card incompletos", "error");
          return;
        }
        if (
          !isSubscription &&
          !isGiftCard &&
          (!orderId || !amount || amount <= 0 || !businessId)
        ) {
        ...
        const body = isSubscription
          ...
          : {
              orderId: isGiftCard ? undefined : orderId,
              giftCardId: isGiftCard ? giftCardId : undefined,
              amount: Math.round(amount),
              businessId: isGiftCard ? undefined : businessId,
        ...
        if (data.clientSecret) {
          ...
          showToast(
            data.message || data.error || "Error al inicializar el pago",
            "error",
          );
        }
      } catch (e: any) {
  ...
  return (
    <form onSubmit={handleSubmit} style={{ width: "100%" }}>
      {/* Card element */}
      <View
        style={[s.cardWrap, { borderColor: border, backgroundColor: inputBg }]}
      >
        <CardElement
          options={{
            style: {
      ...
      {!clientSecret && (
        <View
          style={[
            s.infoBox,
            { backgroundColor: PRIMARY + "10", borderColor: PRIMARY + "30" },
          ]}
        >
          <ActivityIndicator size="small" color={PRIMARY} />
          <span style={{ color: PRIMARY, marginLeft: 10, fontSize: 14 }}>
      ...
      <button
        ...
        style={{
          ...
          height: 52,
          backgroundColor:
            !stripe || loading || !clientSecret ? "#f87171" : PRIMARY,
          borderRadius: 12,
          display: "flex",
          alignItems: "center",
          ...
          border: "none",
          cursor:
            !stripe || loading || !clientSecret ? "not-allowed" : "pointer",
          marginTop: 8,
          gap: 8,
        }}
      >
      ...
      <Pressable onPress={onCancel} style={s.cancelBtn}>
        <span style={{ color: isDark ? "#aaa" : "#6b7280", fontSize: 14 }}>
          Cancelar y volver
        </span>
      </Pressable>
    </form>
  ...
  const handleSuccess = async (paymentIntentId?: string) => {
    ...
    if (isGiftCard && giftCardId) {
      try {
        await apiRequest(
          "POST",
          `/api/gift-cards/${giftCardId}/stripe-success`,
          { paymentIntentId },
        );
      } catch {}
      navigation.reset({
        index: 0,
        routes: [{ name: "Main" }, { name: "GiftCards" }],
      });
      return;
    }
    await clearCart();
  ...
  if (!stripeReady) {
    return (
      ...
      >
        ...
        <span style={{ color: sub, marginTop: 12, fontSize: 14 }}>
            ...
            >
              ...
              >
                ...
                {!isSubscription && !isGiftCard && (
                  <>
                    <View
                      style={[s.summaryDivider, { backgroundColor: border }]}
                    />
                    <View style={s.summaryRow}>
              ...
              <View style={s.badges}>
                {[
                  ["shield", "SSL 256-bit"],
                  ["check-circle", "Sin cargos ocultos"],
                  ["refresh-cw", "Cancelación fácil"],
                ].map(([ic, label]) => (
          ...
          <View style={[s.right, isMobile && { padding: 20 }]}>
            ...
            >
              ...
              <View style={[s.stripeNote, { borderTopColor: border }]}>
                ...
                <span style={{ fontSize: 12, color: sub, marginLeft: 6 }}>
                  Procesado por Stripe · Tus datos nunca se almacenan en
                  nuestros servidores
                </span>
              </View>
            </View>
