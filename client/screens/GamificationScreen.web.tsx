...
export default function GamificationScreen() {
  ...
  const { data: pointsData } = useQuery({
    ...
    queryFn: async () => (await apiRequest('GET', '/api/gamification/points')).json(),
  });
  const { data: rewardsData } = useQuery({
    queryKey: ['/api/gamification/rewards'],
    queryFn: async () => (await apiRequest('GET', '/api/gamification/rewards')).json(),
  });
  const { data: achievementsData } = useQuery({
    queryKey: ['/api/gamification/achievements'],
    queryFn: async () => (await apiRequest('GET', '/api/gamification/achievements')).json(),
  });
  const { data: leaderboardData } = useQuery({
    queryKey: ['/api/gamification/leaderboard'],
    queryFn: async () => (await apiRequest('GET', '/api/gamification/leaderboard?limit=50')).json(),
  });

  const redeemMutation = useMutation({
    mutationFn: async (rewardId: string) =>
      (await apiRequest('POST', `/api/gamification/redeem/${rewardId}`, {})).json(),
    onSuccess: (data) => {
      if (data.success) {
    ...
    <View style={[s.root, { backgroundColor: bg }]}>
      ...
      <MobileSidebarWrapper title="Mis puntos" sidebarStyle={[s.sidebar, { backgroundColor: card, borderRightColor: border }]}>
        <View style={[s.sideHeader, { borderBottomColor: border }]}>
          ...
          <Text style={[s.sidePoints, { color: text }]}>{points?.currentPoints || 0}</Text>
          <Text style={[s.sidePointsLabel, { color: sub }]}>puntos disponibles</Text>
      ...
      <ScrollView style={s.main} contentContainerStyle={s.content} showsVerticalScrollIndicator={false}>
            ...
            <View style={s.heroRight}>
              ...
              <View style={s.progressBar}>
                <View style={[s.progressFill, {
                  width: `${Math.min(100, 100 - (points.pointsToNextTier / (points.pointsToNextTier + points.currentPoints)) * 100)}%` as any,
        ...
        <View style={[s.tabRow, { backgroundColor: card, borderColor: border }]}>
          {TABS.map(tab => (
            <Pressable
              key={tab.id}
              onPress={() => setActiveTab(tab.id)}
              style={[s.tab, activeTab === tab.id && { borderBottomColor: PRIMARY, borderBottomWidth: 2 }]}
            >
              <Feather name={tab.icon as any} size={16} color={activeTab === tab.id ? PRIMARY : sub} />
              <Text style={[s.tabText, { color: activeTab === tab.id ? PRIMARY : sub }]}>{tab.label}</Text>
            </Pressable>
          ))}
        </View>
          ...
          <View>
            {rewards.length === 0 ? (
              <View style={[s.empty, { backgroundColor: card, borderColor: border }]}>
                ...
                <Text style={[s.emptyText, { color: sub }]}>No hay recompensas disponibles</Text>
              </View>
            ) : rewards.map((reward: any) => (
              <View key={reward.id} style={[s.rewardCard, { backgroundColor: card, borderColor: border }]}>
                <View style={[s.rewardIconWrap, { backgroundColor: PRIMARY + '15' }]}>
                  <Feather name="gift" size={22} color={PRIMARY} />
                </View>
                <View style={{ flex: 1, marginLeft: 14 }}>
                  <Text style={[s.rewardTitle, { color: text }]}>{reward.title}</Text>
                  <Text style={[s.rewardDesc, { color: sub }]}>{reward.description}</Text>
                  <View style={s.rewardMeta}>
                    <Feather name="star" size={13} color="#F59E0B" />
                    <Text style={[s.rewardPts, { color: '#F59E0B' }]}>{reward.pointsCost} pts</Text>
                  </View>
                </View>
                <Pressable
                  onPress={() => redeemMutation.mutate(reward.id)}
                  disabled={!reward.canAfford || redeemMutation.isPending}
                  style={[s.redeemBtn, { backgroundColor: reward.canAfford ? PRIMARY : cardBg, opacity: reward.canAfford ? 1 : 0.5 }]}
                >
                  {redeemMutation.isPending
                    ? <ActivityIndicator size="small" color="#fff" />
                    : <Text style={[s.redeemBtnText, { color: reward.canAfford ? '#fff' : sub }]}>Canjear</Text>
                  }
                </Pressable>
              </View>
            ))}
        ...
        {activeTab === 'achievements' && (
          <View>
            <Text style={[s.groupTitle, { color: text }]}>Desbloqueados ({achievements.unlocked.length})</Text>
            {achievements.unlocked.map((a: any) => (
              <View key={a.id} style={[s.achievCard, { backgroundColor: card, borderColor: '#10B98130' }]}>
                <View style={[s.achievIcon, { backgroundColor: '#10B98120' }]}>
                  <Feather name="award" size={22} color="#10B981" />
                </View>
                <View style={{ flex: 1, marginLeft: 14 }}>
                  <Text style={[s.achievTitle, { color: text }]}>{a.name}</Text>
                  <Text style={[s.achievDesc, { color: sub }]}>{a.description}</Text>
                </View>
                <Feather name="check-circle" size={20} color="#10B981" />
              </View>
            ))}
            <Text style={[s.groupTitle, { color: text, marginTop: 24 }]}>Bloqueados ({achievements.locked.length})</Text>
            {achievements.locked.map((a: any) => (
              <View key={a.id} style={[s.achievCard, { backgroundColor: card, borderColor: border, opacity: 0.6 }]}>
        ...
        {activeTab === 'leaderboard' && (
          <View>
            {leaderboard.map((entry: any, i: number) => {
              const rankColor = i === 0 ? '#FFD700' : i === 1 ? '#C0C0C0' : i === 2 ? '#CD7F32' : cardBg;
              const isMe = entry.userId === user?.id;
              return (
                <View key={entry.userId} style={[s.rankCard, { backgroundColor: isMe ? PRIMARY + '10' : card, borderColor: isMe ? PRIMARY : border }]}>
                  <View style={[s.rankBadge, { backgroundColor: rankColor }]}>
                    <Text style={[s.rankNum, { color: i < 3 ? '#fff' : text }]}>{i + 1}</Text>
                  </View>
                  <View style={{ flex: 1, marginLeft: 14 }}>
                    <Text style={[s.rankName, { color: text }]}>{entry.userName}{isMe ? ' (tú)' : ''}</Text>
                    <Text style={[s.rankTier, { color: sub }]}>{TIER_LABELS[(entry.tier as keyof typeof TIER_LABELS)] || entry.tier}</Text>
                  </View>
                  <Text style={[s.rankPts, { color: PRIMARY }]}>{entry.totalEarned} pts</Text>
                </View>
              );
            })}
          </View>
...
const s = StyleSheet.create({
  ...
  roleBadge:      { paddingHorizontal: 10, paddingVertical: 4, borderRadius: 12, marginBottom: 12 },
  roleBadgeText:  { fontSize: 11, fontWeight: '700' },
  tierBadge:      { flexDirection: 'row', alignItems: 'center', gap: 6, paddingHorizontal: 12, paddingVertical: 6, borderRadius: 20, borderWidth: 1, marginBottom: 8 },
  ...
  heroCard:       { borderRadius: 20, padding: 28, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 },
  heroLeft:       { flexDirection: 'row', alignItems: 'center', gap: 16 },
  heroIcon:       { fontSize: 52 },
  heroPoints:     { fontSize: 40, fontWeight: '900', color: '#fff' },
  heroPointsLabel:{ fontSize: 13, color: 'rgba(255,255,255,0.85)', marginTop: 2 },
  heroTier:       { fontSize: 13, color: 'rgba(255,255,255,0.85)', fontWeight: '600', marginTop: 4 },
  heroRight:      { alignItems: 'flex-end' },
  heroNextLabel:  { fontSize: 11, color: 'rgba(255,255,255,0.8)', marginBottom: 4 },
  heroNextPts:    { fontSize: 20, fontWeight: '800', color: '#fff', marginBottom: 6 },
  progressBar:    { width: 120, height: 6, backgroundColor: 'rgba(255,255,255,0.3)', borderRadius: 3, overflow: 'hidden' },
  progressFill:   { height: '100%', borderRadius: 3 },
  tabRow:         { flexDirection: 'row', borderRadius: 14, borderWidth: 1, marginBottom: 24, overflow: 'hidden' },
  ...
  empty:          { borderRadius: 16, borderWidth: 1, padding: 48, alignItems: 'center', gap: 12 },
  emptyText:      { fontSize: 15 },
  rewardCard:     { flexDirection: 'row', alignItems: 'center', borderRadius: 14, borderWidth: 1, padding: 16, marginBottom: 12 },
  rewardIconWrap: { width: 48, height: 48, borderRadius: 24, justifyContent: 'center', alignItems: 'center' },
  ...
  achievCard:     { flexDirection: 'row', alignItems: 'center', borderRadius: 14, borderWidth: 1, padding: 16, marginBottom: 10 },
  achievIcon:     { width: 48, height: 48, borderRadius: 24, justifyContent: 'center', alignItems: 'center' },
  achievTitle:    { fontSize: 14, fontWeight: '700', marginBottom: 3 },
  achievDesc:     { fontSize: 13 },
  rankCard:       { flexDirection: 'row', alignItems: 'center', borderRadius: 14, borderWidth: 1, padding: 16, marginBottom: 10 },
