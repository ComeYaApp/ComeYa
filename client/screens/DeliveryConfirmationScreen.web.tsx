...
export default function DeliveryConfirmationScreen({ orderId, orderDetails, onConfirmed, onDisputed }: Props) {
  ...
  const handleConfirmDelivery = async () => {
    if (window.confirm("¿Confirmar entrega? Los fondos serán liberados al negocio y repartidor.")) {
      setLoading(true);
      try {
        // Obtener GPS
        let location = { latitude: 0, longitude: 0 };
        try { location = await getGPSLocation(); } catch {}

        const res = await apiRequest("POST", "/api/fund-release/confirm-delivery", {
          orderId,
  ...
  const handleSubmitDispute = async () => {
    if (!selectedIssue) { Alert.alert("Error", "Por favor selecciona el tipo de problema"); return; }
    if (selectedIssue === "other" && !disputeReason.trim()) { Alert.alert("Error", "Por favor describe el problema"); return; }
    ...
    try {
      ...
      if (data.success) {
        ...
        Alert.alert("Disputa registrada", "Tu caso será revisado por nuestro equipo.", [{ text: "OK", onPress: onDisputed }]);
      } else {
  ...
  return (
    <View style={[s.container, { backgroundColor: theme.backgroundRoot, paddingTop: insets.top }]}>
      <ScrollView contentContainerStyle={s.content} showsVerticalScrollIndicator={false}>

        <View style={s.headerIcon}>
          <View style={[s.iconCircle, { backgroundColor: ComeYaColors.primary + "20" }]}>
            <Feather name="check-circle" size={64} color={ComeYaColors.primary} />
          </View>
        </View>

        <ThemedText type="h2" style={s.title}>¿Recibiste tu pedido?</ThemedText>
        <ThemedText type="body" style={[s.subtitle, { color: theme.textSecondary }]}>
          Confirma que todo está correcto para liberar el pago al negocio y repartidor
        </ThemedText>

        {/* Resumen */}
        <View style={[s.orderCard, { backgroundColor: theme.card }, Shadows.sm]}>
          <View style={[s.orderHeader, { borderBottomColor: theme.border }]}>
            <View style={[s.businessIcon, { backgroundColor: ComeYaColors.primary + "20" }]}>
          ...
          <View style={s.orderDetail}>
            ...
            <ThemedText type="h4" style={{ color: ComeYaColors.primary }}>€{(orderDetails.total / 100).toFixed(2)}</ThemedText>
          </View>
          <View style={s.orderDetail}>
        ...
        <View style={[s.photoSection, { backgroundColor: theme.card }]}>
          ...
          {proofPhoto ? (
            <View style={s.photoPreview}>
              <img src={proofPhoto} style={{ width: "100%", height: 160, objectFit: "cover", borderRadius: 12 } as any} />
              <Pressable style={s.removePhoto} onPress={() => setProofPhoto(null)}>
                <Feather name="x" size={16} color="#fff" />
              </Pressable>
            </View>
          ) : (
            <Pressable style={[s.photoBtn, { borderColor: theme.border }]} onPress={handleTakePhoto}>
              <Feather name="camera" size={24} color={theme.textSecondary} />
              <ThemedText type="small" style={{ color: theme.textSecondary, marginTop: 6 }}>Tomar foto</ThemedText>
            </Pressable>
          )}
        </View>

        {/* Info */}
        <View style={[s.infoCard, { backgroundColor: ComeYaColors.success + "15" }]}>
        ...
        <Pressable
          style={[s.confirmButton, { backgroundColor: ComeYaColors.primary, opacity: loading ? 0.7 : 1 }]}
          onPress={handleConfirmDelivery}
          disabled={loading}
        >
          {loading
            ? <ActivityIndicator color="#fff" />
            : <><Feather name="check-circle" size={22} color="#fff" /><ThemedText type="body" style={s.confirmButtonText}>Sí, todo está bien</ThemedText></>
          }
        </Pressable>

        <Pressable
          style={[s.issueButton, { backgroundColor: theme.card, borderColor: ComeYaColors.error }]}
        ...
        <View style={s.modalOverlay}>
          <View style={[s.modalContent, { backgroundColor: theme.card }]}>
            ...
            <ScrollView style={s.modalBody}>
              <ThemedText type="body" style={{ fontWeight: "600", marginBottom: Spacing.md }}>¿Qué problema tuviste?</ThemedText>
              {ISSUES.map((issue) => {
                const selected = selectedIssue === issue.id;
                return (
                  <Pressable
                    key={issue.id}
                    style={[s.issueOption, { backgroundColor: selected ? ComeYaColors.primary + "15" : theme.backgroundSecondary, borderColor: selected ? ComeYaColors.primary : theme.border }]}
                    onPress={() => setSelectedIssue(issue.id)}
                  >
                    <Feather name={issue.icon as any} size={22} color={selected ? ComeYaColors.primary : theme.textSecondary} />
                    <ThemedText type="body" style={{ flex: 1, marginLeft: Spacing.sm, color: selected ? ComeYaColors.primary : theme.text }}>{issue.label}</ThemedText>
              ...
              {selectedIssue === "other" && (
                <TextInput
                  style={[s.textArea, { backgroundColor: theme.backgroundSecondary, color: theme.text, borderColor: theme.border }]}
            ...
            <View style={[s.modalFooter, { borderTopColor: theme.border }]}>
              <Pressable
                style={[s.submitButton, { backgroundColor: selectedIssue ? ComeYaColors.error : theme.backgroundSecondary }]}
                onPress={handleSubmitDispute}
                disabled={!selectedIssue || loading}
              >
                {loading
                  ? <ActivityIndicator color="#fff" />
                  : <ThemedText type="body" style={{ color: selectedIssue ? "#fff" : theme.textSecondary, fontWeight: "700" }}>Enviar reporte</ThemedText>
                }
              </Pressable>
            </View>
          </View>
...
const s = StyleSheet.create({
  ...
  photoSection: { padding: Spacing.md, borderRadius: BorderRadius.lg, marginBottom: Spacing.md },
  photoBtn: { height: 120, borderRadius: BorderRadius.md, borderWidth: 2, borderStyle: "dashed", justifyContent: "center", alignItems: "center" },
  photoPreview: { position: "relative", borderRadius: BorderRadius.md, overflow: "hidden" },
  removePhoto: { position: "absolute", top: 8, right: 8, width: 28, height: 28, borderRadius: 14, backgroundColor: "rgba(0,0,0,0.6)", justifyContent: "center", alignItems: "center" },
