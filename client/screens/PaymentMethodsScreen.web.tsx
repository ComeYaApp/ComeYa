...
export default function PaymentMethodsScreen() {
  ...
  const handleSetDefault = async (id: string) => {
    ...
    const r = await apiRequest('GET', '/api/stripe/cards').then(r => r.json());
    setCards(r.cards || []);
  };
    ...
    <View style={[s.root, { backgroundColor: bg }]}>
      <MobileSidebarWrapper title="Métodos de Pago" sidebarStyle={[s.sidebar, { backgroundColor: card, borderRightColor: border }]}>
        <View style={[s.sideHeader, { borderBottomColor: border }]}>
          ...
          <Text style={[s.sideTitle, { color: text }]}>{isDriver ? 'Método de Pago' : 'Métodos de Pago'}</Text>
          <Text style={[s.sideSub, { color: sub }]}>
            {isDriver ? 'Configuración de pagos Stripe' : `${cards.length} tarjeta${cards.length !== 1 ? 's' : ''} guardada${cards.length !== 1 ? 's' : ''}`}
          </Text>
        </View>
        {!isDriver && (
      ...
      <ScrollView style={s.main} contentContainerStyle={s.content} showsVerticalScrollIndicator={false}>
        {loading ? (
          ...
          <>
            <View style={[s.infoBanner, { backgroundColor: PRIMARY + '10', borderColor: PRIMARY + '30' }]}>
              ...
              <View style={{ flex: 1, marginLeft: 12 }}>
                <Text style={[s.bannerTitle, { color: PRIMARY }]}>¿Cómo funcionan los pagos?</Text>
          ...
          <>
            {/* Aviso web */}
            <View style={[s.warnBanner, { backgroundColor: '#F59E0B15', borderColor: '#F59E0B30' }]}>
              <Feather name="info" size={16} color="#F59E0B" />
              <Text style={[s.warnText, { color: '#92400E' }]}>
                Para agregar tarjetas usa la app móvil. Aquí puedes ver tus tarjetas e historial.
              </Text>
            </View>
            ...
            {activeTab === 'cards' ? (
              cards.length === 0 ? (
                <View style={[s.empty, { backgroundColor: card, borderColor: border }]}>
                  ...
                  <Text style={[s.emptyTitle, { color: text }]}>No tienes tarjetas guardadas</Text>
                  <Text style={[s.emptySub, { color: sub }]}>Agrega una tarjeta durante el checkout para pagos más rápidos</Text>
                </View>
              ) : (
                cards.map(c => (
                  <View key={c.id} style={[s.cardItem, { backgroundColor: card, borderColor: border }]}>
                    <View style={[s.cardIconWrap, { backgroundColor: PRIMARY + '15' }]}>
                      <Feather name="credit-card" size={22} color={PRIMARY} />
                    </View>
                    <View style={{ flex: 1, marginLeft: 14 }}>
                      <Text style={[s.cardBrand, { color: text }]}>{c.brand} •••• {c.last4}</Text>
                      <Text style={[s.cardExp, { color: sub }]}>Vence {c.expMonth}/{c.expYear}</Text>
                    </View>
                    {c.isDefault && (
                      <View style={[s.defaultBadge, { backgroundColor: '#10B98120' }]}>
                        <Text style={{ color: '#10B981', fontSize: 12, fontWeight: '600' }}>Predeterminada</Text>
                      </View>
                    )}
                    <View style={s.cardActions}>
                      {!c.isDefault && (
                        <Pressable onPress={() => handleSetDefault(c.id)} style={[s.actionBtn, { backgroundColor: cardBg }]}>
                          <Feather name="check" size={14} color={PRIMARY} />
                          <Text style={{ color: PRIMARY, fontSize: 12, fontWeight: '600', marginLeft: 4 }}>Predeterminar</Text>
                        </Pressable>
                      )}
                      <Pressable onPress={() => handleDelete(c.id)} style={[s.actionBtn, { backgroundColor: '#EF444415' }]}>
                        <Feather name="trash-2" size={14} color="#EF4444" />
                        <Text style={{ color: '#EF4444', fontSize: 12, fontWeight: '600', marginLeft: 4 }}>Eliminar</Text>
                      </Pressable>
                    </View>
                  </View>
                ))
              ...
              history.length === 0 ? (
                <View style={[s.empty, { backgroundColor: card, borderColor: border }]}>
                  ...
                  <Text style={[s.emptyTitle, { color: text }]}>Sin historial de pagos</Text>
                </View>
              ) : (
                history.slice(0, 20).map((item: any) => (
                  <View key={item.payment.id} style={[s.historyItem, { backgroundColor: card, borderColor: border }]}>
                    <View style={[s.histIconWrap, { backgroundColor: cardBg }]}>
                      <Feather name={item.payment.method === 'card' ? 'credit-card' : 'dollar-sign'} size={18} color={sub} />
                    </View>
                    <View style={{ flex: 1, marginLeft: 12 }}>
                      <Text style={[s.histAmount, { color: text }]}>€{((item.payment.amount || 0) / 100).toFixed(2)}</Text>
                      <Text style={[s.histDate, { color: sub }]}>
                        {new Date(item.payment.createdAt).toLocaleDateString('es-ES', { day: 'numeric', month: 'short', year: 'numeric' })}
                      </Text>
                    </View>
                    <View style={[s.statusPill, { backgroundColor: item.payment.status === 'completed' ? '#10B98120' : '#F59E0B20' }]}>
                      <Text style={{ color: item.payment.status === 'completed' ? '#10B981' : '#F59E0B', fontSize: 12, fontWeight: '600' }}>
            ...
            <View style={[s.infoBanner, { backgroundColor: PRIMARY + '10', borderColor: PRIMARY + '30', marginTop: 16 }]}>
              <Feather name="shield" size={18} color={PRIMARY} />
              <View style={{ flex: 1, marginLeft: 12 }}>
                <Text style={[s.bannerTitle, { color: PRIMARY }]}>Pagos seguros con Stripe</Text>
                <Text style={[s.bannerSub, { color: PRIMARY }]}>Tus datos están protegidos con encriptación de nivel bancario.</Text>
              </View>
            </View>
          </>
        )}
...
const s = StyleSheet.create({
  ...
  warnBanner:   { flexDirection: 'row', alignItems: 'center', gap: 10, borderWidth: 1, borderRadius: 12, padding: 14, marginBottom: 20 },
  warnText:     { flex: 1, fontSize: 13 },
  infoBanner:   { flexDirection: 'row', alignItems: 'flex-start', gap: 12, borderWidth: 1, borderRadius: 14, padding: 16 },
  ...
  cardItem:     { flexDirection: 'row', alignItems: 'center', borderRadius: 14, borderWidth: 1, padding: 16, marginBottom: 12 },
  ...
  cardExp:      { fontSize: 13 },
  defaultBadge: { paddingHorizontal: 10, paddingVertical: 4, borderRadius: 8, marginRight: 8 },
  cardActions:  { flexDirection: 'row', gap: 8 },
  actionBtn:    { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 10, paddingVertical: 6, borderRadius: 8 },
  historyItem:  { flexDirection: 'row', alignItems: 'center', borderRadius: 12, borderWidth: 1, padding: 14, marginBottom: 10 },
  histIconWrap: { width: 40, height: 40, borderRadius: 20, justifyContent: 'center', alignItems: 'center' },
