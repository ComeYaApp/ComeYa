  ...
  useEffect(() => {
    ...
    const buildRoute = async (driverLat: number, driverLng: number) => {
      try {
        ...
        const dest = data.order.deliveryLatitude && data.order.deliveryLongitude
        ...
        <View style={[s.routeInfo, { backgroundColor: theme.card, bottom: insets.bottom + Spacing.lg }]}>
          {duration && (
            <View style={s.routeStat}>
              ...
              <ThemedText type="body" style={{ fontWeight: "700", marginLeft: Spacing.xs }}>{duration}</ThemedText>
            </View>
          )}
          {distance && (
            <View style={s.routeStat}>
              <Feather name="map-pin" size={16} color={ComeYaColors.primary} />
              <ThemedText type="body" style={{ fontWeight: "700", marginLeft: Spacing.xs }}>{distance}</ThemedText>
            </View>
          )}
          {activeOrder && (
            <ThemedText type="caption" style={{ color: theme.textSecondary, marginTop: Spacing.xs }} numberOfLines={1}>
